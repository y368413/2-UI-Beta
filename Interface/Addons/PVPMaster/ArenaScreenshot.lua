...
function EnemyTargetCheck()
	...
	if( nCount>= AS_Max_Enemies ) then
		if( sName == AS_PlayerName ) then
			AS_Frame:Show()
			AS_AlertFrame:AddMessage("[你]|cffFFFF40被 "..nCount.." 个敌人盯上了!|r")
			DEFAULT_CHAT_FRAME:AddMessage("[你]|cffFFFF40被 "..nCount.." 个敌人盯上了!|r")
			if AS_Sound then
				PlaySoundFile("Sound\\Creature\\Executus\\ExecutusSlay01.wav")
			end
		else
			AS_AlertFrame:AddMessage("|cffFFFF40" .. sName .. "|r [" .. sClass .. "]|cffFFFF40被 " .. nCount .. " 个敌人盯上了!|r")
			AS_AlertFrame:AddMessage("|cffFFFF40" .. sName .. "|r [" .. sClass .. "]|cffFFFF40被 " .. nCount .. " 个敌人盯上了!|r")
		end
	end
end
	...
	if( event == "VARIABLES_LOADED" ) then
		...
		SlashCmdList["ARENASCREENSHOT"] = function(msg)
			...
			if( cmd == "" ) then AS_Sound = not AS_Sound
				if AS_Sound then
					DEFAULT_CHAT_FRAME:AddMessage("/as , 打开声音")
				else
					DEFAULT_CHAT_FRAME:AddMessage("/as , 关闭声音")
				end
			end
		end
...
AS_Frame:RegisterEvent("UPDATE_WORLD_STATES")
