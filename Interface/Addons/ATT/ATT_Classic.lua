...
function ATTdbs:OnReceiveCooldownListUpdate(unitId, unitCooldows)
    ...
    for spellId, cooldownInfo in pairs(unitCooldows) do
        ATT:ProcessSync(anchor, spellId, cooldownInfo, unitId)
    end
end
...
function ATT:CreateAbilityEditor(frame)
  ...
  --  local cpanel = CreateFrame("Frame", "ATTFrame", frame, BackdropTemplateMixin and "BackdropTemplate, InsetFrameTemplate")
