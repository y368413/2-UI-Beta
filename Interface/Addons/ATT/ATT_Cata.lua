...
function ATT:InspectPlayer()
    ...
    if unit then self:ProcessInspect(unit, PlayerGUID, 1, true) end
end
...
function ATT:ProcessInspectQueue()
    ...
    for i, guid in pairs(inspect_queue) do
        ...
        if unit and not (InspectFrame and InspectFrame:IsShown()) and CheckInteractDistance(unit, 1) and CanInspect(unit) and (not self.insTime[guid] or self.insTime[guid] + 2 < cTime) then
            self.lastIns = guid
    ...
    if dbInspected and dbInspect[guid]["isInspected"] and class and dbImport[class] and type(db.isEnabledSpell[class] == "table") then
        ...
        if not db.bgMode then
            for abilityIndex, abilityTable in pairs(self:MergeTable(class, "abilities", true)) do
                ...
                if name and modif2 and dbInspected[modif2.mod] and dbInspected[modif2.mod] > 0 then
                    local newcd = dbInspected[modif2.mod]
                    cooldown = cooldown * (modif2.rank[newcd] or modif2.rank[1])
                end
                

                if name and glyphModif and dbInspected[glyphModif.mod] then cooldown = cooldown - glyphModif.cd end

                if name and bonusModif and dbInspected[bonusModif.mod] and dbInspected[bonusModif.mod] >= bonusModif.rank then
...
function ATT:TrackCooldown(anchor, ability, SentID, unit, guid, flag) --TODO per ID
    ...
    for k, icon in ipairs(anchor.icons) do
        ...
        if ShareCD[ability] and ShareCD[ability][icon.abilityID] and icon.inUse and not icon.active then icon.Start(ShareCD[ability][icon.abilityID] == 1 and icon.cooldown or ShareCD[ability][icon.abilityID], 1, rate) end
        ...
        if cooldownResetters[ability] then
            if type(cooldownResetters[ability]) == "table" and (cooldownResetters[ability][icon.abilityID] or cooldownResetters[ability]["ALL"]) and not icon.excluded and icon.inUse then
                local validReset = cooldownResetters[ability][icon.abilityID] == 1 or dbInspect[guid][cooldownResetters[ability][icon.abilityID]]
                
                if icon.ability ~= GetSpellInfo(23989) and validReset then --hunt fix
                  icon.Stop()
                end
...
function ATT:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    local unitDest = self:GetUnitByGUID(destGUID)
     ...
     if unit and (event == "SPELL_SUMMON") and not ATTdbs.dbBannedIds[SentID] then --fix ring of frost , maybe others?
...
function ATT:CHAT_MSG_BG_SYSTEM_NEUTRAL(text)
    ...
    if (instanceType == "arena" or instanceType == "pvp") and (string.find(text, "!$")) then
        C_Timer.After(1, function() ATT:InspectPlayer() ATT:UpdateGroup() end)
    end
end
...
local function CreatePopUpFrame(panel, name)
    local popUpFrame = CreateFrame("Frame", name, panel, "BackdropTemplate,UIPanelDialogTemplate"); --InsetFrameTemplate2
...
function ATT:CreateAbilityEditor(frame)
   ...
   -- cpanel:SetPoint("TOPLEFT", panel, "TOPLEFT", 20 , -290)
    ...
    local scrollframe = CreateFrame("ScrollFrame", "ATTScrollFrame", frame, "ScrollFrameTemplate")
