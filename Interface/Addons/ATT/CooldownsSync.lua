...
local function UpdatePlayerCooldownList()
    ...
    for abilityIndex, abilityTable in pairs(ATTdbs.dbRacials) do
...
local sendData = function(dataEncoded, channel)
    ...
    if (aceComm) then
        ...
        C_ChatInfo.SendAddonMessage("LRSATT", dataEncoded, channel)
    end
end
...
local sendDataLR = function(dataEncoded, channel)
    ...
    if (aceComm) then
        ...
        C_ChatInfo.SendAddonMessage("LRS", dataEncoded, channel)
    end
end
    ...
    if (flags) then
        if (bit.band(flags, "0x1")) then -- send to party
            if (IsInGroup() and not IsInRaid()) then
                ...
                if data == "F" and not hasLOR then
                    sendDataLR(dataEncoded, IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "PARTY")
                end
            end
        end
        ...
        if (bit.band(flags, "0x2")) then -- send to raid
            if (IsInRaid()) then
                ...
                if data == "F" and not hasLOR then
                    sendDataLR(dataEncoded, IsInRaid(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "RAID")
                end
            end
        end
...
C_ChatInfo.RegisterAddonMessagePrefix("LRSATT")
C_ChatInfo.RegisterAddonMessagePrefix("LRS")
