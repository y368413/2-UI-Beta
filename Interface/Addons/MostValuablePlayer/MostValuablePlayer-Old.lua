...
local function ShowMVP(theunit)
	...
	if not MVPvsFrame.modelHolder or not MVPvsFrame.modelHolder.isShow then
		MVPvsFrame.modelHolder = CreateFrame("Frame", nil,UIParent)
		MVPvsFrame.modelHolder.isShow = true
		MVPvsFrame.modelHolder:SetSize(512,512)
		MVPvsFrame.modelHolder:SetBackdrop(backdrop)
		MVPvsFrame.modelHolder:EnableMouse(true)
	    MVPvsFrame.modelHolder:SetMovable(true)
		MVPvsFrame.modelHolder:SetPropagateKeyboardInput(true)
		MVPvsFrame.modelHolder:SetPoint(cfg.MVPPoint["Point"],UIParent,cfg.MVPPoint["Relay"],cfg.MVPPoint["X"], cfg.MVPPoint["Y"])
		MVPvsFrame.modelHolder:Show()

	end	



	if not MVPvsFrame.playerModel or not MVPvsFrame.playerModel.isShow then
		MVPvsFrame.playerModel = CreateFrame("PlayerModel", nil, MVPvsFrame.modelHolder)
		MVPvsFrame.playerModel:SetPoint("CENTER", MVPvsFrame.modelHolder, "CENTER")
		MVPvsFrame.playerModel.isShow = true
		MVPvsFrame.playerModel:SetSize(GetScreenWidth() * 2, GetScreenHeight() * 2)
		MVPvsFrame.playerModel:SetScale(cfg.MVPPoint["Scale"])	
		MVPvsFrame.playerModel:ClearModel()
		MVPvsFrame.playerModel:SetUnit(theunit)
		MVPvsFrame.playerModel:SetFacing(6.5)
		MVPvsFrame.playerModel:SetPortraitZoom(0.05)
		MVPvsFrame.playerModel:SetCamDistanceScale(4.8)
		MVPvsFrame.playerModel:SetAlpha(1)
		MVPvsFrame.playerModel:SetAnimation(random(1,225))
		UIFrameFadeIn(MVPvsFrame.playerModel, 1, MVPvsFrame.playerModel:GetAlpha(), 1)
		MVPvsFrame.playerModel:Show()
    ...
    --------------关闭按钮-------------------
    if not MVPvsFrame.modelHolder.close then
		MVPvsFrame.modelHolder.close = CreateFrame("Button", nil, MVPvsFrame.modelHolder, "UIPanelCloseButton")
		MVPvsFrame.modelHolder.close:SetPoint("TOPRIGHT", 2, 1)
		MVPvsFrame.modelHolder.close:SetScript("OnClick", function(self, ...)
			cfg.MVPPoint["Point"],_,cfg.MVPPoint["Relay"],cfg.MVPPoint["X"],cfg.MVPPoint["Y"]=MVPvsFrame.modelHolder:GetPoint()
			MVPvsFrame.modelHolder:EnableMouse(false)
		    MVPvsFrame.modelHolder:SetMovable(false)
			MVPvsFrame.playerModel:SetUnit("none")
			MVPvsFrame.modelHolder:SetBackdropColor(0, 0, 0, 0)
			MVPvsFrame.playerModel:EnableMouseWheel(false);
	        MVPvsFrame.modelHolder:SetPropagateKeyboardInput(false)
	        MVPvsFrame.modelHolder:Hide()
	        MVPvsFrame.playerModel.isShow = false
	        MVPvsFrame.modelHolder.isShow = false
	...
	MVPvsFrame.modelHolder:SetScript("OnMouseDown", MVPvsFrame.modelHolder.StartMoving)
	MVPvsFrame.modelHolder:SetScript("OnMouseUp", MVPvsFrame.modelHolder.StopMovingOrSizing)
	MVPvsFrame.modelHolder:SetScript("OnKeyDown", function(self,key)
      if key == "ESCAPE" then
        cfg.MVPPoint["Point"],_,cfg.MVPPoint["Relay"],cfg.MVPPoint["X"],cfg.MVPPoint["Y"]=MVPvsFrame.modelHolder:GetPoint()
		MVPvsFrame.modelHolder:EnableMouse(false)
	    MVPvsFrame.modelHolder:SetMovable(false)
    ...
    MVPvsFrame.playerModel:SetScript("OnMouseWheel", function(self, direction)
	    if(direction > 0) then
...
function GetScoreData()
    ...
    for k, v in pairs(DamdgeData) do
        ...
        score[k] = round((v + HealData[k] - CombinedFails[k] * 3) / 10000, 1) + InterruptData[k] + DispelData[k] - DeathData[k] * 3
...
local function GetMVPData(name,level,RemainingTime,keystoneUpgradeLevels)
    ...
    for k, v in pairs(DamdgeData) do
        ...
        score[k] = round((v + HealData[k] - CombinedFails[k] * 3) / 10000, 1) + InterruptData[k] + DispelData[k] - DeathData[k] * 3 + (level + keystoneUpgradeLevels) * 10 + RemainingTime
