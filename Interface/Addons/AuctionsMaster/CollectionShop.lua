...
NS.modeNames = { MOUNTS, PETS, TOY, L["APPEARANCES"], "RECIPES" };
...
for i = 1, #NS.craftingProfessions do
	...
	if spellName then
		...
		NS.Print( string.format( "XXX:%d", NS.craftingProfessions[i] ) );
	end
end
NS.skills = {
	--
...
NS.SetMode = function( mode, noReset )
	...
	local craftedByProfession = { "craftedByProfession", "|cffaf8356" .. "Crafted by a Profession" .. FONT_COLOR_CODE_CLOSE, true };
	...
	elseif NS.mode == "APPEARANCES" then
		NS.modeFilters = {
			...
			{
				{ "itemRequiresLevels1", "Level 1-60", true },		-- Classic
				{ "itemRequiresLevels2", "Level 61-70", true },		-- TBC
				{ "itemRequiresLevels3", "Level 71-80", true },		-- WotLK
				{ "itemRequiresLevels4", "Level 81-85", true },		-- Cata
				{ "itemRequiresLevels5", "Level 86-90", true },		-- MoP
				{ "itemRequiresLevels6", "Level 91-100", true },		-- WoD
				{ "itemRequiresLevels7", "Level 101-110", true },	-- Legion
				{ "itemRequiresLevels8", "Level 111-120", true },	-- Battle
			},
			{
				requiresLevel,
				requiresProfession,
				{ "nonsetItems", "Non-set Items", true },
			},
			{ craftedByProfession },
		};
	elseif NS.mode == "RECIPES" then
		...
		NS.modeFilters = {
			...
			{},
			{
				requiresLevel,
				requiresProfession,
				{ "requiresProfessionSpec", RED_FONT_COLOR_CODE .. "Requires Profession Specialization" .. FONT_COLOR_CODE_CLOSE, false },
			},
			{ craftedByProfession },
		};
		-- Modify misc(5) requiresProfession(2) string(2) because recipes will only show up for learned professions anyways
		NS.modeFilters[5][2][2] = RED_FONT_COLOR_CODE .. "Requires Profession Level" .. FONT_COLOR_CODE_CLOSE;
	end
	...
	if NS.mode == "PETS" then
		AuctionFrameCollectionShop_FlyoutPanel_ToggleCategories:SetText( "Toggle Pet Families" );
	else
		AuctionFrameCollectionShop_FlyoutPanel_ToggleCategories:SetText( "Toggle Categories" );
	end
	-- Update and/or Reset
	if NS.mode == "MOUNTS" then
...
function NS.scan:Start( type )
	...
	elseif type == "SHOP" then
		...
		if NS.mode == "RECIPES" and not next( NS.playerProfessions ) then
			return self:Complete( RED_FONT_COLOR_CODE .. "You must have a primary profession or Cooking" .. FONT_COLOR_CODE_CLOSE );
		end
...
NS.Blizzard_AuctionUI_OnLoad = function()
	...
	NS.TextFrame( "_SelectedOwnerLabel", AuctionFrameCollectionShop_DialogFrame_BuyoutFrame, "¡ú", {
		size = { 50, 20 },
	...
	NS.InputBox( "_SelectedOwnerEditbox", AuctionFrameCollectionShop_DialogFrame_BuyoutFrame, {
		...
		tooltip = function( self )
			if self:GetText() == L["Unknown"] then
				return "Try reselecting the auction\nto load the seller's name.";
			end
		end,
		OnLoad = function( self )
	...
	NS.Button( "_BuyAllButton", AuctionFrameCollectionShop, L["Buy All"], {
		...
		tooltip = function()
			if not NS.buyAll then return end
		end,
		OnClick = function( self )
			if NS.buyAll then
				NS.buyAll = false;
	...
	NS.InputBox( "_NameSearchEditbox", AuctionFrameCollectionShop_FlyoutPanel, {
		...
		OnLoad = function( self )
			self.Instructions:SetText( "Item Name" );
		end,
	} );
