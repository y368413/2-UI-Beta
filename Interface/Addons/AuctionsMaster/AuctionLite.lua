...
function AuctionLite:ADDON_LOADED(_, name)
  ...
  elseif name == "Combuctor" then
  local index = 1;
	while (CombuctorItemButton) do
		_G["CombuctorItem1"]:SetScript("PreClick", ContainerFrameItemButton_OnClick_Hook);
		index = index + 1;
		_G["CombuctorItem1"] = _G["CombuctorItem"..index];
	end
