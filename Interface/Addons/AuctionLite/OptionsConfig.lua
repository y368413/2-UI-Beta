	...
	subFrameTabs = {
		...
		},
		{
			...
			Refresh			= function( SubFrame )
				...
				_G[sfn .. "AuctionsWonText"]:SetText( string.format( L["%s\n%sBuyout tracking is reset when closing %s|r"], #NS.auctionsWon .. " " .. GREEN_FONT_COLOR_CODE .. ( #NS.auctionsWon == 1 and BUYOUT or L["Buyouts"] ) .. FONT_COLOR_CODE_CLOSE .. ( NS.copperAuctionsWon > 0 and " (" .. NS.MoneyToString( NS.copperAuctionsWon, HIGHLIGHT_FONT_COLOR_CODE ) .. ")" or "" ), RED_FONT_COLOR_CODE, NS.title ) );
			end,
		},
		{
			-- GetAll Scan Data
