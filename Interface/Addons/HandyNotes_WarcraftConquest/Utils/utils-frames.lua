...
local function inviteFrame(inv)
  ...
  if C.LINK[inv .. "_Code"] -- builds the Discord Code entry on /wconquest inv discord
     then
  ...
  if C.LINK[inv] -- builds the Addon Site entry on /wconquest inv discord
     then
  ...
  if C.LINK[inv .. "_URL"] -- builds the Discord URL entry on /wconquest inv discord
    then
