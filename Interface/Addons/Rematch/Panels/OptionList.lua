...
panel.opts = {
	...
	{ "check", "LoadTeamsFromTarget", L["Load Teams From Target Button"], L["When a target is chosen after clicking the Target button above the loaded pets, automatically load any saved team associated with the target."] },
	{ "check", "UseOldTargetMenu", L["Use Old Target Button Menu"], L["When clicking the Target button above the loaded pets, use the old context menu rather than the scrollable list of targets to select from."], nil, true },
	...
	{ "check", "CollapseListsOnESC", L["Collapse Lists With ESC Key"], L["When a list that can expand or collapse is on screen, such as Options or Targets, make the ESCape key collapse the list if any part is expanded."], nil, true, true },
	...
	{ "check", "DontConfirmCaging", L["Don't Ask When Caging Pets"], L["Don't ask for confirmation when putting a pet in a cage."] },
	{ "check", "DontConfirmFillQueue", L["Don't Confirm To Fill Queue"], L["Don't ask for confirmation when using the 'Fill Queue' or 'Fill Queue More' menu items in the Queue menu."] },
	...
	{ "check", "HideMenuHelp", L["Hide Extra Help"], L["Hide the informational \"Help\" items found in many menus and on the pet card."], nil, true },
