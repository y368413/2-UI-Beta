...
function MISC:RareAlert()
	if MaoRUISettingDB["Misc"]["RareAlerter"] then
...
function MISC:Explosive_Update(...)
	...
	if index and M.GetNPCID(destGUID) == 120651 then
		...
		if overkill and overkill > 0 then
			...
			local cache = MaoRUISettingDB["Misc"]["ExplosiveCache"]
...
local function startCount()
	wipe(MaoRUISettingDB["Misc"]["ExplosiveCache"])
	M:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED", MISC.Explosive_Update)
end

local function endCount()
	local text
	for name, count in pairs(MaoRUISettingDB["Misc"]["ExplosiveCache"]) do
...
function MISC:ExplosiveAlert()
	if MaoRUISettingDB["Misc"]["ExplosiveCount"] then
...
function MISC:UunatAlert()
	if MaoRUISettingDB["Misc"]["UunatAlert"] then
