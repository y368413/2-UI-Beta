...
local TM = M:RegisterPlusModule("TalentManager")
------------------------------------
...
function TM:SaveSet(setName)
	...
	for key, data in pairs(self.db.sets[self.specID]) do
		if data.setName == setName then
			UIErrorsFrame:AddMessage(I.InfoColor..format("Already have a set named %s.", setName))
			return
		end
	end

	if #self.db.sets[self.specID] == MAX_SETS then
		UIErrorsFrame:AddMessage(I.InfoColor.."Too many sets here, please delete one of them and try again.")
		return
	end
...
P_EasyMenu = CreateFrame("Frame", "NDuiPlus_EasyMenu", UIParent, "UIDropDownMenuTemplate")
...
function TM:ShowContextText(button)
	...
	EasyMenu(menu, P_EasyMenu, "cursor", 0, 0, "MENU")
end
...
function TM:GetTalentName(tier, column)
	if column == 0 then
		return format(iconString, 134400, "Not set")
	else
...
local function CreateHeader(parent, title, width, height)
	...
	frame.Title = M.CreateFS(frame, 14, title, true, "CENTER", 0, 0)

	local line = frame:CreateTexture(nil, "ARTWORK")
	line:SetSize(frame:GetWidth(), R.mult)
...
function TM:CreateSaveFrame()
	...
	frame.Title = M.CreateFS(frame, 14, "", true, "TOP", 0, -8)

	local label = M.CreateFS(frame, 14, "Set Name".." :", true, "TOPLEFT", 10, -32)
	local editBox = M.CreateEditBox(frame, 160, 22)
	...
	for i = 1, MAX_TALENT_TIERS do
		...
		local bu = P_CreateButton(frame, 160, 22)
		if i == 1 then
		...
		bu:SetScript("OnClick", function(self)
			...
			if frame.PveIgnore[i] then
				bu:SetText(format("Tier %d ", i).."Ignored")
			else
	...
	for i = 1, 3 do
		...
		bu:SetScript("OnClick", function(self)
			...
			if frame.PvpIgnore[i] then
				bu:SetText(format("Slot %d ", i).."Ignored")
			else
				bu:SetText(self.TalentName)
			end
		end)
	...
	local save = P_CreateButton(frame, 160, 22, true)
	...
	save:SetScript("OnClick", function()
		...
		if setName and setName ~= "" then
			...
			UIErrorsFrame:AddMessage(I.InfoColor.."You must enter a set name.")
		end
	end)
...
function TM:CreateSetFrame()
	...
	local header = CreateHeader(frame, "Talent Set", 130, 22)
	header:SetPoint("TOP", 0, -20)
	header.Title:SetPoint("CENTER", 0, 4)
	frame.Header = header

	local newButton = P_CreateButton(frame, 130, 22, true)
	...
	frame.SetButtons = {}
	for i = 1, MAX_SETS do
		local button = P_CreateButton(frame, 130, 22)
		if i == 1 then
		...
		button:HookScript("OnLeave", M.HideTooltip)

		button:Hide()
		frame.SetButtons[i] = button
	end

	local toggle = P_CreateButton(PlayerTalentFrameTalents, 100, 22, true)
	toggle:SetPoint("BOTTOMRIGHT", -16, -16)
	toggle:SetText("Talent Manager")
...
function TM:TalentUI_Load(addon)
	if addon == "Blizzard_TalentUI" then
		...
		TM:CreateSaveFrame()
		TM:TweakWarmode()
		M:UnregisterEvent(self, TM.TalentUI_Load)
	end
end

function TM:SetupCache()
	TM.db = MaoRUIPerDB["TalentManager"]
...
function TM:OnLogin()
	...
	if IsAddOnLoaded("Blizzard_TalentUI") then
		...
		M:RegisterEvent("ADDON_LOADED", TM.TalentUI_Load)
	end
	...
	M:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED", function(_, unit)
	...
	M:RegisterEvent("PLAYER_TALENT_UPDATE", TM.UpdateSaveFrame)
end