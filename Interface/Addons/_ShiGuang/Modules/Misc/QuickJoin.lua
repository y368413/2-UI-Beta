...
function MISC:QuickJoin_ShowTips()
	local quickJoinInfo = {
		text = U["QuickJoinInfo"],
	...
	LFGListFrame.SearchPanel.SignUpButton:HookScript("OnShow", function()
		if not MaoRUIDB["Help"]["QuickJoin"] then
...
function MISC:AddDungeonsFilter()
	local mapData = {
	...
	local function toggleAll()
		...
		UIDropDownMenu_Refresh(M.EasyMenu)
		LFGListSearchPanel_DoSearch(searchPanel)
	end
	...
	searchPanel.RefreshButton:HookScript("OnMouseDown", function(self, btn)
		...
		EasyMenu(menuList, M.EasyMenu, self, 25, 50, "MENU")
	end)

	searchPanel.RefreshButton:HookScript("OnEnter", function()
		GameTooltip:AddLine(I.RightButton.._G.SPECIFIC_DUNGEONS)
		GameTooltip:Show()
	end)
