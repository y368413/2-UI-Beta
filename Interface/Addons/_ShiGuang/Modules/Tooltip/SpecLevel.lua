...
local specPrefix = SPECIALIZATION..": "..I.InfoColor
local levelPrefix = STAT_AVERAGE_ITEM_LEVEL..": "..I.InfoColor
...
function TT:InspectOnUpdate(elapsed)
	...
	if self.elapsed > frequency then
		...
		if currentUNIT and UnitGUID(currentUNIT) == currentGUID then
			M:RegisterEvent("INSPECT_READY", TT.GetInspectInfo)
...
function TT:GetInspectInfo(...)
	...
	elseif self == "INSPECT_READY" then
		...
		M:UnregisterEvent(self, TT.GetInspectInfo)
	end
end
M:RegisterEvent("UNIT_INVENTORY_CHANGED", TT.GetInspectInfo)
...
function TT:SetupSpecLevel(spec, level)
	...
	for i = 2, GameTooltip:NumLines() do
		...
		if text and strfind(text, specPrefix) then levelLine = line end
	end

	spec = specPrefix..(spec or isPending)
	level = levelPrefix..(level or isPending)
	if levelLine then
		levelLine:SetText(spec.."|r  "..level)
	else
		GameTooltip:AddLine(spec.."|r  "..level)
	end
end
			...
			if itemTexture then
				...
				if not itemLink then
					...
					if (not quality) or (not level) then
						...
						if unit ~= "player" then
							level = M.GetItemLevel(itemLink) or level
							if i < 16 then
								total = total + level
...
function TT:InspectUnit(unit, forced)
	...
	if UnitIsUnit(unit, "player") then
		...
		if not MaoRUISettingDB["Tooltip"]["SpecLevelByShift"] and IsShiftKeyDown() then forced = true end
...
function TT:InspectUnitSpecAndLevel()
	if MaoRUISettingDB["Tooltip"]["SpecLevelByShift"] and not IsShiftKeyDown() then return end
