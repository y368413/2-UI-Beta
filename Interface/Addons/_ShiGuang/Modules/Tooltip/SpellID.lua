  ...
  if (IsShiftKeyDown() or IsControlKeyDown() or IsAltKeyDown()) then
	...
	if type == types.item then
		if GetItemCount(id, true) and GetItemCount(id, true) - GetItemCount(id) > 0 then
			self:AddDoubleLine(BAGSLOT.."/"..BANK..":", format(I.InfoColor.."%s|r", GetItemCount(id).."/"..GetItemCount(id, true) - GetItemCount(id)))
		elseif GetItemCount(id) > 0 then
			self:AddDoubleLine(BAGSLOT..":", format(I.InfoColor.."%s|r", GetItemCount(id)))
		end
		if select(8, GetItemInfo(id)) and select(8, GetItemInfo(id)) > 1 then
			self:AddDoubleLine("Stack Cap"..":", format(I.InfoColor.."%s|r", select(8, GetItemInfo(id))))
		end
	end
	self:AddDoubleLine(type, format(I.InfoColor.."%s|r", id))
	self:Show()
  end
end
...
hooksecurefunc(GameTooltip, "SetToyByItemID", function(self, id) if id then addLine(self, id, types.item) end end)
