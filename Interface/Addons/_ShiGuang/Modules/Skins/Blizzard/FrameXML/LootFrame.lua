...
tinsert(R.defaultThemes, function()
	...
	hooksecurefunc("LootFrame_UpdateButton", function(index)
		...
		if not ic.bg then
			...
			local bd = M.CreateBDFrame(bu, .25)
			bd:SetPoint("TOPLEFT")
			bd:SetPoint("BOTTOMRIGHT", 114, 0)

			ic.bg = M.ReskinIcon(ic)
		end
	...
	M.ReskinPortraitFrame(LootFrame)
	M.ReskinArrow(LootFrameUpButton, "up")
	M.ReskinArrow(LootFrameDownButton, "down")
	...
	do
		...
		local bg = M.ReskinIcon(frame.SpecIcon)
		hooksecurefunc("BonusRollFrame_StartBonusRoll", function()
			bg:SetShown(frame.SpecIcon:IsShown())
		end)

		M.ReskinIcon(frame.PromptFrame.Icon)
		frame.PromptFrame.Timer.Bar:SetTexture(I.normTex)
		M.SetBD(frame)
		M.CreateBDFrame(frame.PromptFrame.Timer, .25)
	...
	hooksecurefunc("GroupLootFrame_OpenNewFrame", function()
		for i = 1, NUM_GROUP_LOOT_FRAMES do
			...
			if not frame.styled then
				...
				frame.Background:SetAlpha(0)
				frame.bg = M.CreateBDFrame(frame, nil, true)
