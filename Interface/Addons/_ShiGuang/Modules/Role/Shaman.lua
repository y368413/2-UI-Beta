...
local function TotemsGo()
	...
	for i = 1, 4 do
		...
		M.CreateIF(totem[i], true)
		totem[i].Icon:SetTexture(GetSpellTexture(icons[i]))
		totem[i]:SetAlpha(.3)
		if MaoRUISettingDB["Auras"]["DestroyTotems"] then
	...
	M.Mover(Totembar, "图腾助手", "Totems", R.Auras.TotemsPos, 140, 32)
end

-- Function
local f = MaoRUI:EventFrame({"PLAYER_ENTERING_WORLD", "PLAYER_TOTEM_UPDATE"})
f:SetScript("OnEvent", function(self)
	if not MaoRUISettingDB["Auras"]["Totems"] then
