...
local function StaggerGo()
	...
	M.Mover(bar, "Ì¹É®ÖúÊÖ", "Stagger", R.Auras.StaggerPos, bar:GetWidth(), 20)
end

local f = MaoRUI:EventFrame({"PLAYER_LOGIN", "PLAYER_TALENT_UPDATE"})
f:SetScript("OnEvent", function(self, event)
	if not MaoRUISettingDB["Auras"]["Stagger"] then
	...
	if event == "PLAYER_LOGIN" or event == "PLAYER_TALENT_UPDATE" then
		...
		do
			...
			bar:SetValue(Per)
			bar.Count:SetText(I.InfoColor..M.Numb(value).." "..I.MyColor..M.Numb(Per).."%")
