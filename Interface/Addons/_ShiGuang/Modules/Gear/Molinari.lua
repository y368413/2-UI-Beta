...
function Molinari:Apply(itemLink, spell, r, g, b, isItem)
	...
	local modifier = modifiers[defaults.modifier][2]
...
function Molinari:UpdateModifier()
	RegisterStateDriver(self, 'visible', modifiers[defaults.modifier][1] .. ' show; hide')
end
...
GameTooltip:HookScript('OnTooltipSetItem', function(self)
	...
	if(defaults.modifier == 'CTRL' and not IsControlKeyDown()) then return end
	if(defaults.modifier == 'SHIFT' and not IsShiftKeyDown()) then return end
	...
	if(not itemID or defaultBlacklist.Molinariitems[itemID]) then
		return
	end
