...
--- Local Functions ---
	...
	local function isCurrentTab(self)
		if self.tooltip and IsCurrentSpell(self.tooltip) then self:SetChecked(true)
...
OCES:SetScript("OnEnter", function()
    if OCES.itemID then
        GameTooltip:SetOwner(OCES);
        GameTooltip:SetItemByID(OCES.itemID);
    end;
end);
OCES:SetScript("OnLeave", function()
    GameTooltip:Hide();
end);
