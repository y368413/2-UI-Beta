...
local function pauseit() spot=0 Bt_TrainAll() end

function Bt_TrainAll()
	if done==true then spot=0 return end
	Numskills=GetNumTrainerServices()
	found=false
	while found==false do
		spot=spot+1
		_,cani,_=GetTrainerServiceInfo(spot)
		if cani=="available" then
			BuyTrainerService(spot)
			C_Timer.After(0.3,pauseit)
			found=true
		end
		if spot>=Numskills then done=true break end
	end
end
...
local function createit()
	...
	local Enable=false
		for i=1, GetNumTrainerServices() do
			local _,cani,_=GetTrainerServiceInfo(i)
			if cani=="available" then
...
TrainAllevent:SetScript("OnEvent", function(_,_,name)
...
MerchantFrame:HookScript("OnShow", function() MerchantFrame_SetFilter(MerchantFrame, GetMerchantFilter()) end)

----------------------------------------------------------------[[ Cloudy TradeSkill Copyright (c) 2016, Cloudyfa All rights reserved. ]]
local numTabs = 0
--- Create Frame ---
...
--- Local Functions ---
	...
	local function isUseable(id) return IsUsableSpell(GetSpellInfo(id)) end
...
local function registerMerchant()
	local OCES = CreateFrame("Button", "TradeSkillCreateScrollButton", TradeSkillFrame, "MagicButtonTemplate");
    OCES:SetPoint("TOPRIGHT", TradeSkillFrame.DetailsFrame.CreateButton, "TOPLEFT");
    OCES:SetScript("OnClick", function()
    if (IsShiftKeyDown() and OCES.itemID) then
        if ChatEdit_GetActiveWindow() then
            local _,link = GetItemInfo(OCES.itemID);
  ...
  OCES:SetScript("OnEnter", function()
    if OCES.itemID then GameTooltip:SetOwner(OCES); GameTooltip:SetItemByID(OCES.itemID); end;
  end);
  OCES:SetScript("OnLeave", function() GameTooltip:Hide(); end);
  OCES:SetMotionScriptsWhileDisabled(true);
  OCES:Hide();

  hooksecurefunc(TradeSkillFrame.DetailsFrame, "RefreshButtons", function(self)
	if C_TradeSkillUI.IsTradeSkillGuild() or C_TradeSkillUI.IsNPCCrafting() or C_TradeSkillUI.IsTradeSkillLinked() then OCES:Hide();
	else
        ...
        if recipeInfo and recipeInfo.alternateVerb then
            ...
            if enchantingTradeSkillNames[tradeSkillName] then
                ...
                TradeSkillCreateScrollButton:SetText(string.format("%s (%d)", "&", numScrollsAvailable));
                if numScrollsAvailable == 0 then numCreateable = 0; end
		...
		CloudyTradeSkill:UnregisterEvent("ADDON_LOADED")
end

local CloudyTradeSkill = CreateFrame("Frame")
CloudyTradeSkill:SetScript("OnEvent",function(self,event)
...
CloudyTradeSkill:RegisterEvent("ADDON_LOADED")