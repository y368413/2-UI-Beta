...
local info = module:RegisterInfobar(R.Infobar.SystemPos)
...
info.onEnter = function(self)
	...
	if GetCVarBool("scriptProfile") then
		...
		local maxAddOns = IsShiftKeyDown() and #usageTable or min(R.Infobar.MaxAddOns, #usageTable)
		...
		if not IsShiftKeyDown() then
			for i = (R.Infobar.MaxAddOns + 1), #usageTable do
				hiddenUsage = hiddenUsage + usageTable[i][2]
			end
			if #usageTable > R.Infobar.MaxAddOns then
				local numHidden = #usageTable - R.Infobar.MaxAddOns
				GameTooltip:AddDoubleLine(format("%d %s (%s)", numHidden, U["Hidden"], U["Shift"]), hiddenUsage.." Ms/s", .6,.8,1, .6,.8,1)
			end
		end
		GameTooltip:AddLine(" ")
	end
	...
	--GameTooltip:AddDoubleLine(" ", I.RightButton..U["CPU Usage"]..": "..(GetCVarBool("scriptProfile") and "|cff55ff55"..VIDEO_OPTIONS_ENABLED or "|cffff5555"..VIDEO_OPTIONS_DISABLED).." ", 1,1,1, .6,.8,1)
