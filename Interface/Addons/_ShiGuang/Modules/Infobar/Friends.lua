...
local function CanCooperateWithUnit(gameAccountInfo)
	return gameAccountInfo.playerGuid and (gameAccountInfo.factionName == I.MyFaction) and (gameAccountInfo.realmID ~= 0)
end
...
local function setupFriendsFrame()
	...
	M.CreateBD(friendsFrame, .7)
	M.CreateSD(friendsFrame)
	M.CreateTex(friendsFrame)
	...
	M.CreateFS(friendsFrame, 16, "|cff0099ff"..FRIENDS_LIST, nil, "TOPLEFT", 15, -10)
	friendsFrame.numFriends = M.CreateFS(friendsFrame, 14, "-/-", nil, "TOPRIGHT", -15, -12)
	...
	M.CreateFS(friendsFrame, 13, I.LineString, false, "BOTTOMRIGHT", -12, 42)
	--local whspInfo = I.InfoColor..I.RightButton..U["Whisper"]
	--M.CreateFS(friendsFrame, 13, whspInfo, false, "BOTTOMRIGHT", -15, 26)
	local invtInfo = I.InfoColor..I.RightButton..U["Whisper"].."   "..I.InfoColor.."ALT +"..I.LeftButton..U["Invite"]
	M.CreateFS(friendsFrame, 13, invtInfo, false, "BOTTOMRIGHT", -15, 10)
end
...
local function createInviteMenu()
	...
	menuList[1] = {text = U["Join or Invite"], isTitle = true, notCheckable = true}
end
...
local function createRoster(parent, i)
	...
	M.CreateBD(M.CreateBG(button.gameIcon))
