	...
	if event == "ADDON_LOADED" then
		...
		--if info == "ChatLoot" then
			...
			if ShiGuangPerDB then
				self.settings = ShiGuangPerDB
			else
				self.settings = self:CopyTable(self.defaults.global, false, true)
				ShiGuangPerDB = self.settings
			end
			
			if CHATLOOTCHAR then
				self.charSettings = CHATLOOTCHAR
			else
				self.charSettings = self:CopyTable(self.defaults.char, false, true)
				CHATLOOTCHAR = self.charSettings
			end
...
function ChatLoot:ApplyDelay()
	if not self.delayTimer then
		...
		self.delayTimer.t1:SetScript("OnFinished", function() self:LoadAddon() end)
