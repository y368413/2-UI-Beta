...
local function RenderEncounterJournalEncounterBossInfo(index)
  ...
  table.foreach(info, function(name)
    if string.find(name, "怒風") then n1 = name end
  end)
...
local EncounterJournalPlusInstanceInfoFrame = CreateFrame("FRAME");
EncounterJournalPlusInstanceInfoFrame:RegisterEvent("ADDON_LOADED");
EncounterJournalPlusInstanceInfoFrame:RegisterEvent("UPDATE_INSTANCE_INFO");
EncounterJournalPlusInstanceInfoFrame:SetScript("OnEvent",function(self, event, arg1)
  if event == "ADDON_LOADED" and arg1 == "Blizzard_EncounterJournal" then
    hooksecurefunc(EncounterJournal, "Show", function()
      local dungeonsTab, raidsTab = GetEncounterJournalInstanceTabs();
      if not dungeonsTab:IsEnabled() or not raidsTab:IsEnabled() or EncounterJournal.encounter:IsShown() then
        ResetEncounterJournalScrollInstancesInfo();
        RequestRaidInfo();
      end
    end);
