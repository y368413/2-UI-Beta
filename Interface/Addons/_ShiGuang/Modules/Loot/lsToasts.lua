...
local function ResetToast(toast)
	...
	toast:Hide()
	toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-default")
...
local function CreateBaseToastButton()
	...
	bg:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-default")
	bg:SetTexCoord(1 / 256, 225 / 256, 1 / 64, 49 / 64)
	toast.BG = bg

	local border = toast:CreateTexture(nil, "BACKGROUND", nil, 1)
	border:SetAllPoints()
	border:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-border")
	...
	textBG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-text-bg")
...
local function GetToast(toastType)
	...
	if toastType == "item" then
		...
		if not toast then
			...
			iconBorder:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-icon-border")
...
do
	...
	toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-archaeology")
...
do
	...
	if isUpgraded then
		...
		toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-upgrade")
...
do
	local function Toast_SetUp(name, subtypeID, textureFilename, moneyReward, xpReward, numItemRewards, isScenario, isScenarioBonusComplete)
		...
		if isScenario then
			...
			toast.Title:SetText(_G.SCENARIO_COMPLETED)
			...
			toast.Title:SetText(_G.DUNGEON_COMPLETED)
			toast.soundFile = "LFG_Rewards"
		end
	toast.Text:SetText(name)
	toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-dungeon")
		...
		if isItem then
			if link then
				...
				if factionGroup then
					toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-"..factionGroup)
				end
				...
				if isUpgraded then
					if baseQuality and baseQuality < quality then
						title = string.format(_G.LOOTUPGRADEFRAME_TITLE, "%s%s|r"):format(color.hex, _G["ITEM_QUALITY"..quality.."_DESC"])
					else
				...
				if isLegendary then
					title = _G.LEGENDARY_ITEM_LOOT_LABEL
					soundFile = "UI_LegendaryLoot_Toast"

				toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-legendary")
					toast.Dragon:Show()
			end

				if isStorePurchase then
					title = _G.BLIZZARD_STORE_PURCHASE_COMPLETE
				...
				if CFG.colored_names_enabled then
					...
					if rollType then
						if rollType == _G.LOOT_ROLL_TYPE_NEED then
							toast.Title:SetFormattedText(TITLE_NEED_TEMPLATE, _G.YOU_WON_LABEL, roll)
						elseif rollType == _G.LOOT_ROLL_TYPE_GREED then
							toast.Title:SetFormattedText(TITLE_GREED_TEMPLATE, _G.YOU_WON_LABEL, roll)
						elseif rollType == _G.LOOT_ROLL_TYPE_DISENCHANT then
							toast.Title:SetFormattedText(TITLE_DE_TEMPLATE, _G.YOU_WON_LABEL, roll)
						end
					end
...
do
	local function Toast_SetUp(recipeID)
		...
		if tradeSkillID then
			...
			toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-recipe")
...
do
	local function Toast_SetUp(questID, name, moneyReward, xpReward, numCurrencyRewards, isInvasion, isInvasionBonusComplete)
		...
		if isInvasion then
			...
			toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-legion")
			...
			toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-worldquest")
			toast.Border:SetVertexColor(color.r, color.g, color.b)
			toast.IconBorder:SetVertexColor(color.r, color.g, color.b)
...
do
	...
	local function Toast_SetUp(sourceID, isAdded)
		...
		if isAdded then
			toast.Title:SetText("新增幻化收藏")
		else
			toast.Title:SetText("幻化收藏移除")
		end

		toast.Text:SetText(name)
		toast.BG:SetTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\IsToasts\\toast-bg-transmog")
...
local function CreateToastConfigLine(panel, params)
	...
	local dndToggle = CreateConfigCheckButton(panel, {
		...
		tooltip_text = "Toasts in DND mode won't be displayed in combat, but will be queued up in the system instead. Once you leave combat, they'll start popping up.",
		get = params.dnd_get,
		set = params.dnd_set,
	})
...
local function PopulateConfigPanels()
	...
	subtext:SetText("包括成就、拾取、R、要塞相关、专业、任何和事件等框体的美化.")
	...
	local colorToggle = CreateConfigCheckButton(panel, {
		...
		text = "名称染色",
		tooltip_text = "Colours item, follower names by quality, and world quest, mission titles by rarity.",
	...
	panel.name = "类型"
	panel.parent = "|cff8080ff[美化]|r提示框体"
	panel:Hide()
	...
	subtext:SetText("更详细的设置每一类型提示的开关...")
	...
	local layout = {
		...
		[6] = {
			...
			text = "拾取 (特殊)",
			toast_tooltip = "Toasts triggered by special loot events, e.g. won rolls, legendary drops, personal loot, etc.",
		...
		[7] = {
			...
			text = "拾取 (普通)",
			toast_tooltip = "Toasts triggered by chat events, e.g. greens, blues, some epics, everything that isn't handled by special loot toasts.",
...
function dispatcher:ADDON_LOADED(arg)
	if arg ~= "_ShiGuang" then return end
