local RelicInspector = {}  -- Author: Thoralie
local defaults = {
	profile = {
		enabled = true,
...
-- ArtifactData.lua
local RelicInspector_RelicTypes = {
...
local RelicInspector_RelicSlots = {
...
local RelicInspector_Artifacts = {
	[127829] = 577, -- Havoc DH
...
-- TraitSpellIDs.lua
local RelicInspector_SpellIDByRelic = {
	-- Burden of Vigilance
...
local RelicSpells = RelicInspector_SpellIDByRelic
local RelicTypeNames = RelicInspector_RelicTypes
local RelicSlotsByArtifact = RelicInspector_RelicSlots
local SpecByArtifact = RelicInspector_Artifacts
local ArtifactBySpec = invertTable(RelicInspector_Artifacts)
...
local function DecorateArtifact(self)
	...
	if type(link) == 'string' and defaults.profile.enabled == true then
		...
		if (upgradeID == '256' or upgradeID == '16777472') and nil ~= RelicSlotsByArtifact[tonumber(itemID)] then
			...
			if(self:GetName() == "ItemRefTooltip") then
				-- Item Links
				if invLinkOptions[defaults.profile.linkTraitNames] == 2 then
					showTraitNames = true
				end
				if invLinkOptions[defaults.profile.linkTraitDesc] == 2 then
					showTraitDesc = true
				end
			else -- Game or Shopping Hover Tooltips
				if invHoverOptions[defaults.profile.hoverTraitNames] == 3 or
					(invHoverOptions[defaults.profile.hoverTraitNames] == 2 and modifierPressed ~= 0) then
					showTraitNames = true
				end
				if invHoverOptions[defaults.profile.hoverTraitDesc] == 3 or
					(invHoverOptions[defaults.profile.hoverTraitDesc] == 2 and modifierPressed ~= 0) then
					showTraitDesc = true
				end
			end
			...
			for i = 1,#relics do 
				if '' ~= relics[i] then
					...
					if type(gemLink) == 'string' then
						...
						--[[if invItemLevelOptions[defaults.profile.itemLevelDisplay] == 2 then
...
local function DecorateRelic(self)
	...
	if type(link) == 'string' and defaults.profile.enabled == true then
		...
		if invRelicOffspecOptions[defaults.profile.offspecsToShow] > 1 then
		...
		if invRelicSpecOptions[defaults.profile.relicSpecs] > 1 then
			...
			if invRelicSpecOptions[defaults.profile.relicSpecs] > 3 then
				for q = 1,#specKeys do
					...
					if invRelicSpecOptions[defaults.profile.relicSpecs] > 4 then
						local traitDesc = GetSpellDescription(traitSpellID)
						if nil ~= traitDesc then
							traitDesc = string.gsub(traitDesc,string.char(10),"")
							traitDesc = string.gsub(traitDesc,string.char(13),"")
				...
				for q = 1,#specKeys do
					...
					local quantity = "" 
					if invRelicSpecOptions[defaults.profile.relicSpecs] == 3 and usableSpecs[specKeys[q]] > 1 then
						quantity = format(' (%d)',usableSpecs[specKeys[q]])
					end
