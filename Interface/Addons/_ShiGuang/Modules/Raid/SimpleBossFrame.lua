...
local UpdateBuffs do
	...
	UpdateBuffs = function(frame)
		...
		-- filter = "HELPFUL"
		index = 1
		if not ShiGuangDB.SimpleBossFrameauras then
...
function SimpleBossFrame:Enable()
	...
	anchorFrame:Hide()
	if ShiGuangDB.SimpleBossFrameanchor and ShiGuangDB.SimpleBossFrameanchor[1] and ShiGuangDB.SimpleBossFrameanchor[4] and ShiGuangDB.SimpleBossFrameanchor[5] then
		anchorFrame:ClearAllPoints()
		anchorFrame:SetPoint(unpack(ShiGuangDB.SimpleBossFrameanchor))
		self.bossFrame:ClearAllPoints()
		self.bossFrame:OrgSetPoint(unpack(ShiGuangDB.SimpleBossFrameanchor))
	else
		anchorFrame:ClearAllPoints()
		anchorFrame:SetPoint(unpack(self.defaultAnchor))
	end
	if ShiGuangDB.SimpleBossFramescale then
		anchorFrame:SetScale(ShiGuangDB.SimpleBossFramescale)
		BossFrames_SetScale(ShiGuangDB.SimpleBossFramescale)
	end
...
function anchorFrame:OnMouseWheel(delta)
	if delta < 0 then
		commands.scale(SimpleBossFrame, (ShiGuangDB.SimpleBossFramescale or SimpleBossFrame.defaultScale) - BF.scale_delta)
	else
		commands.scale(SimpleBossFrame, (ShiGuangDB.SimpleBossFramescale or SimpleBossFrame.defaultScale) + BF.scale_delta)
	end
end
...
function anchorFrame:OnUpdate(elapsed)
	if not InCombatLockdown() and self.moving then
		ShiGuangDB.SimpleBossFrameanchor = {self:GetPoint(1)}
		SimpleBossFrame.bossFrame:ClearAllPoints()
		SimpleBossFrame.bossFrame:OrgSetPoint(unpack(ShiGuangDB.SimpleBossFrameanchor))
	end
end
...
function events:PLAYER_LOGIN(...)
	...
	BF.texturefile = BF.texture and SM3 and SM3:Fetch("statusbar", BF.texture, true) or "Interface\\RaidFrame\\Raid-Bar-Hp-Fill" --nil
...
function buff_prototype:SetStyle()
	self.cooldown:SetScale(1 / (ShiGuangDB.SimpleBossFramescale or BF.scale))
	self.cooldown:ClearAllPoints()
	self.cooldown:SetPoint("LEFT", self.cooldown:GetParent(), "RIGHT", BF.buff.offset.x + ((BF.percent.enabled and 53 or 0) * (ShiGuangDB.SimpleBossFramescale or BF.scale)) + (self.index - 1) * (BF.buff.size.W + 2), BF.buff.offset.y)
...
function commands:sb(arg)
	DEFAULT_CHAT_FRAME:AddMessage(
		...
		"  /sb power\n" ..
		"  /sb mana\n" ..
		"  /sb auras: "..tostring(ShiGuangDB.SimpleBossFrameauras and "Disabled" or "Enabled"),
		1, 1, 0
	)
end
...
function commands:reset(arg)
	...
	ShiGuangDB.SimpleBossFramescale = nil
	anchorFrame:ClearAllPoints()
	anchorFrame:SetPoint(unpack(self.defaultAnchor))
	self.bossFrame:ClearAllPoints()
	self.bossFrame:OrgSetPoint(unpack(self.defaultAnchor))
	ShiGuangDB.SimpleBossFrameanchor = nil
	ShiGuangDB.SimpleBossFrameauras = nil
end
...
function commands:scale(arg)
	...
	local anchor = ShiGuangDB.SimpleBossFrameanchor or {unpack(self.defaultAnchor)}
	...
	ShiGuangDB.SimpleBossFramescale = scale
end

function commands:aura()
	ShiGuangDB.SimpleBossFrameauras = not ShiGuangDB.SimpleBossFrameauras
end
