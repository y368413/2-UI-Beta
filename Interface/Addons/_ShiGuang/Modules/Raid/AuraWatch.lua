...
local AuraList, Aura, UnitIDTable, IntTable, IntCD = {}, {}, {}, {}, R.InternalCD
local MaxFrame = 12	-- Max Tracked Auras

-- Init
local function ConvertTable()
	if not MaoRUISettingDB["AuraWatchList"] then return end
	...
	for _, v in pairs(R.AuraWatchList["ALL"]) do
...
local function BuildAuraList()
	AuraList = R.AuraWatchList["ALL"] and R.AuraWatchList["ALL"] or {}
	for key, _ in pairs(R.AuraWatchList) do
		if key == I.MyClass then
			for _, value in pairs(R.AuraWatchList[I.MyClass]) do
...
local function MakeMoveHandle(Frame, Text, key, Pos)
	...
	M.CreateBD(MoveHandle)
	M.CreateFS(MoveHandle, 12, Text)
	if not MaoRUISettingDB["AuraWatch"][key] then 
		MoveHandle:SetPoint(unpack(Pos))
	else
		MoveHandle:SetPoint(unpack(MaoRUISettingDB["AuraWatch"][key]))
	end
	...
	MoveHandle:SetScript("OnDragStop", function(self)
		...
		MaoRUISettingDB["AuraWatch"][key] = {AnchorF, "UIParent", AnchorT, X, Y}
	end)
	MoveHandle:Hide()
...
local function BuildICON(iconSize)
	...
	Frame.Spellname = M.CreateFS(Frame, 12, "", false, "BOTTOM", 0, -3)

	if MaoRUISettingDB["AuraWatch"]["Hint"] then
...
-- Event
local f = MaoRUI:EventFrame({"PLAYER_ENTERING_WORLD", "COMBAT_LOG_EVENT_UNFILTERED"})
f:SetScript("OnEvent", function(self, event, ...)
	if not MaoRUISettingDB["AuraWatch"]["Enable"] then return end
...
SlashCmdList.AuraWatch = function(msg)
	 ...
	 if TestFlag then
		...
		for _, value in pairs(Aura) do
			for i = 1, 6 do
				...
				if value[i].Icon then value[i].Icon:SetTexture("Interface\\ICONS\\inv_misc_bearcubbrown") end  --GetSpellTexture(21)
	...
	elseif msg:lower() == "reset" then
		wipe(MaoRUISettingDB["AuraWatch"])
		ReloadUI()
	else
