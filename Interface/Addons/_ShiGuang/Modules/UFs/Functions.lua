...
function UF:RefreshColorDots()
	wipe(colorDots)
	M.SplitList(colorDots, R.db["Nameplate"]["ColorDots"])
end
...
function UF.CustomFilter(element, unit, button, name, _, _, _, _, _, caster, isStealable, _, spellID, _, _, _, nameplateShowAll)
	...
	if R.db["Nameplate"]["ColorByDot"] and style == "nameplate" and caster == "player" and colorDots[spellID] then
...
function UF:CreateAuras(self)
	...
	elseif mystyle == "raid" then
		...
		bu.size = 18*R.db["UFs"]["SMRScale"]/10
		bu.numTotal = 1
