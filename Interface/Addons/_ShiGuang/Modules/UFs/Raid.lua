...
function UF:CreateRaidDebuffs(self)
	...
	bu.count = M.CreateFS(bu, 12, "", false, "BOTTOMRIGHT", 6, -3)
...
function UF:InterruptIndicator(self)
	...
	local horizon = MaoRUIPerDB["UFs"]["HorizonParty"]
	local otherSide = MaoRUIPerDB["UFs"]["PWOnRight"]
...
function UF:CreatePartyAltPower(self)
	if not MaoRUIPerDB["UFs"]["PartyAltPower"] then return end
