...
local floor, sformat, band = math.floor, string.format, bit.band
...
local UpdatePositions = function()
	if charges and Ressesbars[1] then
		Ressesbars[1]:SetPoint("TOPRIGHT", RaidCDAnchor, "TOPRIGHT", -2, -8)
		Ressesbars[1].id = 1
		for i = 1, #bars do
		...
		for i = 1, #bars do
			bars[i]:ClearAllPoints()
			if i == 1 then
				bars[i]:SetPoint("TOPRIGHT", RaidCDAnchor, "TOPRIGHT", -2, -8)
			else
				if raidcd_upwards == true then
...
local OnMouseDown = function(self, button)
	if button == "LeftButton" then
		if self.isResses then
			SendChatMessage(sformat("战复剩余: ".."%d, "..L_COOLDOWNS_NEXTTIME.."%s.", currentNumResses, self.right:GetText()), CheckChat())
		else
			SendChatMessage(sformat("%s - %s: %s", self.name, C_Spell.GetSpellLink(self.spellId), self.right:GetText()), CheckChat)
		end
...
local CreateBar = function()
	...
	bar.icon = CreateFrame("Button", nil, bar, "BackdropTemplate")
