...
function CDS:RaidCD()
   if MaoRUIPerDB["Misc"]["RaidCD"] then
    ...
    M.Mover(RaidCDAnchor, U["Raid CD"], "RaidInterrupt", {"RIGHT", UIParent, "RIGHT", -210, 120})
		...
		M:RegisterEvent("ZONE_CHANGED_NEW_AREA", OnEvent)
		M:RegisterEvent("ENCOUNTER_END", OnEvent)
	else
		M:UnregisterEvent("PLAYER_ENTERING_WORLD", OnEvent)
		M:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED", OnEvent)
		M:UnregisterEvent("ZONE_CHANGED_NEW_AREA", OnEvent)
		M:UnregisterEvent("ENCOUNTER_END", OnEvent)
	end
end
