		...
		for _, b in pairs(a) do
			for _, c in pairs(b.List) do
				if c.AuraID then
					...
					if not exists then print("|cffFF0000 XXX spellID:|r "..c.AuraID) end
				end
			end
		end
	end
...
local function MakeMoveHandle(Frame, Text, key, Pos)
	...
	if not MaoRUISettingDB["AuraWatchMover"] then MaoRUISettingDB["AuraWatchMover"] = {} end
	if not MaoRUISettingDB["AuraWatchMover"][key] then 
		MoveHandle:SetPoint(unpack(Pos))
	else
		MoveHandle:SetPoint(unpack(MaoRUISettingDB["AuraWatchMover"][key]))
	end
	...
	MoveHandle:SetScript("OnDragStop", function(self)
		...
		MaoRUISettingDB["AuraWatchMover"][key] = {AnchorF, "UIParent", AnchorT, X, Y}
	end)
	MoveHandle:Hide()
...
local function BuildICON(iconSize)
	...
	if not MaoRUISettingDB["AuraWatch"]["ClickThrough"] then
...
local function onEvent(event, ...)
	...
	if event == "PLAYER_ENTERING_WORLD" then
		...
		M:UnregisterEvent(event, onEvent)
	else
		UpdateInt(event, ...)
	end
end
M:RegisterEvent("PLAYER_ENTERING_WORLD", onEvent)
M:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED", onEvent)
...
StaticPopupDialogs["RESET_AURAWATCH_MOVER"] = {
	text = "Reset AuraWatch Mover Confirm",
	button1 = OKAY,
	button2 = CANCEL,
	OnAccept = function()
