...
local function BloodyHell()
	...
	M.CreateSB(bar, true, .14, .5, 1)
	M.SmoothBar(bar)
	bar:SetMinMaxValues(0, 10)
	bar:SetValue(0)

	bar.Power = M.CreateFS(bar, 18, "", false, "CENTER", 0, -6)

	bar.Shield = M.CreateFS(bar, 18, "")
	bar.Shield:ClearAllPoints()
	bar.Shield:SetPoint("RIGHT", bar, "LEFT", -5, 8)
	bar.Shield:SetTextColor(1, .8, 0)

	bar.Count = M.CreateFS(bar, 18, "")
	...
	M.Mover(bar, "血DK助手", "BloodyHell", R.Auras.BHPos, bar:GetWidth(), 30)
end
...
local function AddSpellGroup(i, buff, cd, showValue)
	...
	if showValue then
		if name then
			icons[4].Count:SetText(M.Numb(value))
		else
...
local function updateSpells()
	...
	if hasShield then
		bar.Shield:SetText(M.Numb(value))
...
local function checkSpec(event)
	if GetSpecializationInfo(GetSpecialization()) == 250 then
		BloodyHell()
		bar:SetAlpha(.1)
		M:RegisterEvent("UNIT_POWER_FREQUENT", updatePower)
		M:RegisterEvent("RUNE_POWER_UPDATE", updateRune)
		M:RegisterEvent("SPELL_UPDATE_COOLDOWN", updateSpells)
		M:RegisterEvent("UNIT_AURA", updateSpells, "player")
	else
		if bar then bar:Hide() end
		M:UnregisterEvent("UNIT_POWER_FREQUENT", updatePower)
		M:UnregisterEvent("RUNE_POWER_UPDATE", updateRune)
...
function A:BloodyHell()
	if not MaoRUIPerDB["Auras"]["BloodyHell"] then return end
