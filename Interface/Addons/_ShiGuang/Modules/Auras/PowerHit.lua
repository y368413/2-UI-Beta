...
function A:ClassRecoure()
	if not MaoRUIPerDB["Auras"]["ClassRecourePlace"] then return end
	ClassRecourePlace()
end
...
function A:Energy()
	if not MaoRUIPerDB["Auras"]["EnergyBar"] then return end
	RiphieEnergy()
...
BlinkComboHelperFrame:SetScript("OnEvent", function(self, event, ...)
  if not MaoRUIPerDB["Auras"]["BlinkComboHelper"] then return end
