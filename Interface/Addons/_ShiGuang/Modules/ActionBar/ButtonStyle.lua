...
function Bar:UpdateHotKey()
	...
	if hotkey and hotkey:IsShown() and not R.db["Actionbar"]["Hotkeys"] then
		hotkey:Hide()
		return
	end
...
function Bar:UpdateEquipItemColor()
	...
	if R.db["Actionbar"]["EquipColor"] and IsEquippedAction(self.action) then
