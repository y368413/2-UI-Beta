...
if not IsAddOnLoaded('Bartender4') or ShiGuangloadDB then --return; end
--[[
@author 图图
设计目标：
...
local anchors={
    {"TOPLEFT", Minimap, "BOTTOMLEFT", -1, 3 }, --左上角锚点
    {"TOPRIGHT", Minimap, "BOTTOMRIGHT", 1, 3 } --右上角锚点
    --{"TOPLEFT", ActionButton1, "BOTTOMLEFT", 0, 0 }, --左上角锚点
    --{"TOPRIGHT", ActionButton12, "BOTTOMRIGHT", 0, 0 } --右上角锚点
}
TuBarCfg = {}
local cfg = TuBarCfg
local barHeight = 5
...
-----------------------------------事 件  响 应------------------------------------
function onEvent(self, evt)
    if evt == "ADDON_LOADED" then
        cfg = TuBarCfg
        if cfg.showText ==nil then cfg.showText=true end --默认显示
    end
    ...
    if GetWatchedFactionInfo() then--追踪了声望
        ...
        if isMaxLevel then
            ...
            TuXP_Box.expText:SetText(cfg.showText and string.format("%.1f%% + |cffb3e1ff%d%%|r", perXP, perRest or 0) or "")
                if HasArtifactEquipped() then
                local _, _, _, _, totalArtifactXP, pointsSpent, _, _, _, _,_, _, Artifacttier  = C_ArtifactUI.GetEquippedArtifactInfo()
		            local _, Artifactxp, ArtifactxpForNextPoint = MainMenuBar_GetNumArtifactTraitsPurchasableFromXP(pointsSpent, totalArtifactXP, Artifacttier)
                   TuXP_Box.ArtifactText:SetText(ARTIFACT_POWER..":"..M.Numb(Artifactxp).."/"..M.Numb(ArtifactxpForNextPoint) or "")
...
TuXP_Box.ArtifactText:SetShadowOffset(1, -1)
--事件注册
...
TuXP_Box:RegisterEvent("ARTIFACT_XP_UPDATE")--神器
TuXP_Box:RegisterEvent("ARTIFACT_UPDATE")--神器
...
TuXP_Box:SetScript("OnEnter", function()
    ...
    --end
     if HasArtifactEquipped() then
        local _, _, _, _, totalArtifactXP, pointsSpent, _, _, _, _,_, _, Artifacttier = C_ArtifactUI.GetEquippedArtifactInfo()
		            local _, Artifactxp, ArtifactxpForNextPoint = MainMenuBar_GetNumArtifactTraitsPurchasableFromXP(pointsSpent, totalArtifactXP, Artifacttier)
        GameTooltip:AddLine(string.format(ARTIFACT_POWER_BAR, Artifactxp, ArtifactxpForNextPoint) or "")    
     end    
    GameTooltip:Show()
end)
