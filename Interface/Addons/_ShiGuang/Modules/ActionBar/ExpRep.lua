...
local function UpdateBar(bar)
	...
	elseif C_AzeriteItem.HasActiveAzeriteItem() then
		...
		bar:SetValue(xp)
		bar:Show()
		bar.ArtifactText:SetText(string.format('|c00FF68CC%d%%|r',(xp)/(totalLevelXP)*100).."  "..M.Numb(xp).."/"..M.Numb(totalLevelXP).." -|c00FF68CC"..currentLevel.."|r- ".." "..ORDER_HALL_SHAMAN)
...
local function UpdateTooltip(bar)
	...
	if C_AzeriteItem.HasActiveAzeriteItem() then
		...
		GameTooltip:AddDoubleLine(ARTIFACT_POWER, M.Numb(xp).."/"..M.Numb(totalLevelXP).." ("..floor(xp/totalLevelXP*100).."%)", .6,.8,1, 1,1,1)
	end
	...
	if HasArtifactEquipped() then
		...
		if C_ArtifactUI.IsEquippedArtifactDisabled() then
			...
			GameTooltip:AddDoubleLine(ARTIFACT_POWER, M.Numb(totalXP)..numText, .6,.8,1, 1,1,1)
			if xpForNextPoint ~= 0 then
				local perc = " ("..floor(xp/xpForNextPoint*100).."%)"
				GameTooltip:AddDoubleLine(M.Numb(xp).."/"..M.Numb(xpForNextPoint)..perc, .6,.8,1, 1,1,1)
			end
		end
	end
	GameTooltip:Show()
...
function sendReports() 
   if GetNumSubgroupMembers() > 0 then sendReport("PARTY") end
   if GetNumGroupMembers() > 0 then sendReport("raid") end
   if IsInGuild() then sendReport("guild") end
...
function module:SetupScript(bar)
	...
	hooksecurefunc(StatusTrackingBarManager, "UpdateBarsShown", function() UpdateBar(bar) end)  	
end
...
function module:Expbar()
	...
	bar:SetPoint("TOP", Minimap, "BOTTOM", 0, 3)
	bar:SetSize(186, 3)
	bar:SetHitRectInsets(0, 0, 0, -10)
	M.CreateSB(bar)
    bar.ArtifactText=bar:CreateFontString("ShowArtifactText", "OVERLAY")
    bar.ArtifactText:SetFont(STANDARD_TEXT_FONT, 11, "OUTLINE")
    bar.ArtifactText:SetShadowColor(0, 0, 0)
    bar.ArtifactText:SetShadowOffset(1, -1)
    bar.ArtifactText:SetPoint("topright", Minimap,"topleft",-16, -1)  
