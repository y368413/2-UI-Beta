...
function module:OnLogin()
	...
	function Backpack:OnInit()
		...
		f.main = MyContainer:New("Main", {Columns = MaoRUISettingDB["Bags"]["BagsWidth"], Bags = "bags"})
		f.main:SetFilter(onlyBags, true)
		f.main:SetPoint("BOTTOMRIGHT", -50, 150)

		f.azeriteItem = MyContainer:New("AzeriteItem", {Columns = MaoRUISettingDB["Bags"]["BagsWidth"], Bags = "azeriteitem"})
		f.azeriteItem:SetFilter(bagAzeriteItem, true)
		f.azeriteItem:SetParent(f.main)

		f.equipment = MyContainer:New("Equipment", {Columns = MaoRUISettingDB["Bags"]["BagsWidth"], Bags = "equipment"})
		f.equipment:SetFilter(bagEquipment, true)
		f.equipment:SetParent(f.main)

		f.consumble = MyContainer:New("Consumble", {Columns = MaoRUISettingDB["Bags"]["BagsWidth"], Bags = "consumble"})
		f.consumble:SetFilter(bagConsumble, true)
		f.consumble:SetParent(f.main)

		f.bank = MyContainer:New("Bank", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "bank"})
		f.bank:SetFilter(onlyBank, true)
		f.bank:SetPoint("BOTTOMRIGHT", f.main, "BOTTOMLEFT", -20, 0)
		f.bank:Hide()

		f.bankAzeriteItem = MyContainer:New("BankAzeriteItem", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "bankazeriteitem"})
		f.bankAzeriteItem:SetFilter(bankAzeriteItem, true)
		f.bankAzeriteItem:SetParent(f.bank)

		f.bankLegendary = MyContainer:New("BankLegendary", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "banklegendary"})
		f.bankLegendary:SetFilter(bankLegendary, true)
		f.bankLegendary:SetParent(f.bank)

		f.bankEquipment = MyContainer:New("BankEquipment", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "bankequipment"})
		f.bankEquipment:SetFilter(bankEquipment, true)
		f.bankEquipment:SetParent(f.bank)

		f.bankConsumble = MyContainer:New("BankConsumble", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "bankconsumble"})
		f.bankConsumble:SetFilter(bankConsumble, true)
		f.bankConsumble:SetParent(f.bank)

		f.reagent = MyContainer:New("Reagent", {Columns = MaoRUISettingDB["Bags"]["BankWidth"], Bags = "bankreagent"})
	...
	function MyButton:OnCreate()
		...
		self.Quest = M.CreateFS(self, 30, "!", false, "LEFT", 3, 0)
		...
		if MaoRUISettingDB["Bags"]["Artifact"] then
	...
	function MyContainer:OnCreate(name, settings)
		...
		if name == "Main" or name == "Bank" or name == "Reagent" then
			self:SetScale(MaoRUISettingDB["Bags"]["BagsScale"])
			M.CreateMF(self)
		elseif name:match("^Bank%a+") then
			M.CreateMF(self, f.bank)
		else
			M.CreateMF(self, f.main)
		end
		...
		if label then M.CreateFS(self, 14, label, true, "TOPLEFT", 8, -8) return end
		...
		tagDisplay:SetFont(unpack(I.Font))
		tagDisplay:SetPoint("RIGHT", infoFrame, "RIGHT",0,0)
		M.CreateFS(infoFrame, 14, SEARCH, true, "LEFT", 0, 1)

		local SortButton = M.CreateButton(self, 60, 20, BAG_CLEANUP_BAGS)
		...
		local closebutton = M.CreateButton(self, 20, 20, "X")
		...
		if name == "Main" or name == "Bank" then
			...
			local bagToggle = M.CreateButton(self, 60, 20, BAGSLOT)
			...
			if name == "Bank" then
				...
				local switch = M.CreateButton(self, 70, 20, REAGENT_BANK)
		...
		elseif name == "Reagent" then
			local deposit = M.CreateButton(self, 100, 20, REAGENTBANK_DEPOSIT)
			deposit:SetPoint("LEFT", SortButton, "RIGHT", 6, 0)
			deposit:SetScript("OnClick", DepositReagentBank)

			local switch = M.CreateButton(self, 70, 20, BANK)
