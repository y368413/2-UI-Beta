...
local moads = {FOGLIGHT_HYBIRD, FOGLIGHT_DISABLE, FOGLIGHT_ALL, FOGLIGHT_NONE}
local menu = CreateFrame('frame', 'foglightmenu', WorldMapFrame.UIElementsFrame, 'L_UIDropDownMenuTemplate')
...
local function setMode(mode)
	if not moads[mode] then mode = 4 end
	L_UIDropDownMenu_SetSelectedID(foglightmenu, mode)
...
L_UIDropDownMenu_Initialize(foglightmenu, initmenu)
L_UIDropDownMenu_SetSelectedID(foglightmenu, 4)
L_UIDropDownMenu_SetWidth(foglightmenu, 85)

menu:SetScript('OnEvent', function(_, _, ...)
	FoglightMode = FoglightMode and moads[FoglightMode] and FoglightMode or 4
...
[1669]="argus 1" -- argus is missing a ton of terrain textures, and since we can't detect that, hide them
}
local OverlayInfo = { -- Generated on August 30th, 2017 by Semlar
