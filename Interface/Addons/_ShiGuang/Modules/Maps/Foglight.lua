------------------------## Author: Semlar## Version: 3.0.2## SavedVariables: FoglightMode
...
---[[
local FoglightMode = 4
local showTerrain = false

local FoglightMoads = {"混合", "禁用", "全地貌", "无地貌"}
local FoglightMenu = CreateFrame("frame", "foglightmenu", WorldMapFrame.UIElementsFrame, "Lib_UIDropDownMenuTemplate")
FoglightMenu:SetPoint('BOTTOMLEFT', -19, -6)
--FoglightMenu:SetFrameLevel(WORLDMAP_POI_FRAMELEVEL + 2)
FoglightMenu:SetAlpha(0.8)
FoglightMenu:SetFrameLevel(1000)

local function SetFoglightMode(FoglightStyle)
	if not FoglightMoads[FoglightStyle] then FoglightStyle = 4 end
	UIDropDownMenu_SetSelectedID(foglightmenu, FoglightStyle)
	if FoglightStyle == 1 then -- hybrid
		overlayFrame:Show()
		for _,tx in pairs(terrainTextures) do tx:SetDrawLayer('ARTWORK', -2) end
		terrainFrame:SetShown(showTerrain)
	elseif FoglightStyle == 2 then -- disabled
		overlayFrame:Hide()
	elseif FoglightStyle == 3 then -- all terrain
		overlayFrame:Show()
		for _,tx in pairs(terrainTextures) do tx:SetDrawLayer('ARTWORK', 2) end
		terrainFrame:SetShown(showTerrain)
	elseif FoglightStyle == 4 then -- no terrain
		terrainFrame:Hide()
		overlayFrame:Show()
	end
	FoglightMode = FoglightStyle
end

local function menuOnClick(self)
	SetFoglightMode(self:GetID())
end

local function initmenu()
	for i,v in ipairs(FoglightMoads) do
...
FoglightMenu:SetScript("OnEvent", function(_, _)
	FoglightMode = FoglightMode and FoglightMoads[FoglightMode] and FoglightMode or 4
	SetFoglightMode(FoglightMode)
end)
FoglightMenu:RegisterEvent("ADDON_LOADED")
--]]
...
local OverlayInfo = { -- Generated on April 4th, 2017 by Semlar
...
["valsharah"]={"andutalah/241/240/587/250","blackrookhold/250/253/262/175","bradensbrook/311/244/259/275","dreamgrove/294/364/283/0","gloamingreef/239/301/136/274","groveofcenarius/171/150/457/351","lorlathil/177/156/467/413","mistvale/274/344/610/18","moonclawvale/254/281/549/380","shalanir/326/360/419/0","smolderhide/341/188/324/480","templeofelune/216/219/459/240","thastalah/218/168/342/416"},
["brokenshore"]={"thelosttemple/308/244/632/169","felragestrand/332/276/596/100","tombofsargeras/312/301/500/0","theweepingterrace/276/213/350/13","brokenvalley/338/322/254/84","soulruin/338/270/389/180","deadwoodlanding/182/245/220/260","deliverancepoint/387/314/312/302"},
["highmountain"]={"bloodhunthighlands/297/250/307/75","feltotem/256/326/172/31","frosthoofwatch/186/213/391/408","ironhornenclave/288/258/452/410","nightwatchersperch/344/295/0/244","pinerockbasin/217/148/323/249","riverbend/214/308/314/360","rockawayshallows/207/302/469/45","shipwreckcove/283/170/331/0","skyhorn/311/229/357/179","stonehoofwatch/341/328/494/236","sylvanfalls/445/326/0/342","thundertotem/244/199/332/302","trueshotlodge/172/204/249/236","cavea/110/98/445/190"},
["brokenshorepaladin"]={"thelosttemple/376/364/0/212"},
						...
						for j=1,numTexturesTall do
							...
							for k=1,numTexturesWide do
								...
								if not overlayTextures[texNum] then
									...
									texture:SetVertexColor(0.7, 0.7, 0.7, 1)
