...
---------------------------------- ExtraQuestButton, by p3lim-- NDui MOD--------------------------------
...
ExtraQuestButton:SetScript("OnEvent", function(self, event, ...)
	if(self[event]) then self[event](self, event, ...)
	elseif(self:IsEnabled()) then self:Update() end
end)
...
function ExtraQuestButton:SetItem(itemLink, texture)
	...
	if(self.itemID) then
		...
		if MaoRUISettingDB["Actionbar"]["Enable"] then M.UpdateHotKey(self) end
