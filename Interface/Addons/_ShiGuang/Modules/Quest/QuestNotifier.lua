...
local function completeText(link)
	PlaySound(SOUNDKIT.ALARM_CLOCK_WARNING_3, "Master")  --PlaySoundFile("Interface\\Addons\\_ShiGuang\\Media\\Sounds\\QuestNotifier.ogg", "Master")
...
function module:QuestNotifier()
	...
	if not MaoRUISettingDB["Misc"]["QuestNotifier"] then return end

	FindQuestComplete()
	initComplete = true

	M:RegisterEvent("QUEST_ACCEPTED", FindQuestAccept)
	M:RegisterEvent("QUEST_LOG_UPDATE", FindQuestComplete)
	M:RegisterEvent("QUEST_TURNED_IN", FindWorldQuestComplete)
	if MaoRUISettingDB["Misc"]["QuestSchedule"] then
		M:RegisterEvent("UI_INFO_MESSAGE", FindQuestProgress)
	end
end