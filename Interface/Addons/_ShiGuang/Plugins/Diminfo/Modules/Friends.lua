...
if cfg.Friends == true then
  ...
  Text:SetShadowOffset(1, -1)
	...
	local menuFrame = CreateFrame("Frame", "FriendRightClickMenu", UIParent, "Lib_UIDropDownMenuTemplate")
	local menuList = {
	...
	local function Update(self, event, arg1)
		...
		Text:SetText(format(cfg.ColorClass and "%d"..init.Colored.."%s" or "%d%s",onlineFriends + numBNetOnline,"^-^"))
		dataValid = false
	end
	...
	Stat:SetScript("OnMouseUp", function(self, btn)
		...
		Lib_EasyMenu(menuList, menuFrame, self, 0, -10, "MENU", 2)
	end)
