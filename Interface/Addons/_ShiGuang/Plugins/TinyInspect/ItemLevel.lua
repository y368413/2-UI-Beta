    ...
    if (addonName == "Blizzard_GuildUI") then
        ...
        hooksecurefunc("GuildNewsButton_SetText", function(button, text_color, text, text1, text2, ...)
            ...
            if (text2 and type(text2) == "string") then
                ...
                if (link and GuildNewsItemCache[link]) then
                    button.text:SetFormattedText(text, GuildNewsItemCache[link] .. text1, text2, ...)
                elseif (link) then
                    local _, level, _, _, quality  = LibItemInfo:GetItemInfo(link)
                    if (level > 0 and quality) then
                        GuildNewsItemCache[link] = format("|c%s%s|r", select(4,GetItemQualityColor(quality)) or "ffffd200", level)
                        button.text:SetFormattedText(text, GuildNewsItemCache[link] .. text1, text2, ...)
                    end
                end
            end
...
local function SetPaperDollItemLevel(self, unit)
    ...
    local frame = GetItemLevelFrame(self, "PaperDoll")
         if (self:GetID() <= 5 or self:GetID() == 9 or self:GetID() == 15) then
            self.ItemLevelFrame.levelString:SetPoint("LEFT", self, "RIGHT", 3, 6)
         elseif (self:GetID() == 16) then
            self.ItemLevelFrame.levelString:SetPoint("RIGHT", self, "LEFT", -3, 8)
         elseif (self:GetID() == 17) then
            self.ItemLevelFrame.levelString:SetPoint("LEFT", self, "RIGHT", 3, 8)
         else
            self.ItemLevelFrame.levelString:SetPoint("RIGHT", self, "LEFT", -3, 6)
         end
