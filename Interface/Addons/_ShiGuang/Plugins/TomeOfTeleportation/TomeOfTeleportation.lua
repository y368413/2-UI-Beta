--## Author: Remeen
if GetLocale() == "zhCN" then TOMEOFTELEPORTATIONTITLE = "传送大师"; TITLEREADME = "点击图标打开传送大师";
elseif GetLocale() == "zhTW" then TOMEOFTELEPORTATIONTITLE = "傳送大師"; TITLEREADME = "點擊圖標打開傳送大師";
else TOMEOFTELEPORTATIONTITLE = "Tome of Teleportation"; TITLEREADME = "Click icon to open Tome of Teleportation";
end
...
local AddonTitle = TOMEOFTELEPORTATIONTITLE
...
_G["BINDING_NAME_TOMEOFTELEPORTATIONSHOW"] = "    "..DISPLAY
...
function Teleporter_OnEvent(self, event, ...)
	if event == "ADDON_LOADED" then
		local loadedAddon = ...
		if string.upper(loadedAddon) == string.upper("_ShiGuang") then Teleporter_OnAddonLoaded()	end
...
local function InitTeleporterOptionsMenu(frame, level, menuList, topLevel)
	...
	elseif menuList== "Sort" then
		...
		info.owner = frame
		info.checked = function(info) sortMode = GetOption("sort"); return sortMode == nil or sortMode == 2; end
		MSA_DropDownMenu_AddButton(info, level)
		...
		info.owner = frame
		info.checked = function(info) return GetOption("sort") == 1; end
		MSA_DropDownMenu_AddButton(info, level)
