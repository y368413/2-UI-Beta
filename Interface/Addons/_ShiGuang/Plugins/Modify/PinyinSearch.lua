...
-------------------------------------------------------
--              Pinyin lib                           --
--    Copy from https://github.com/MissinA/pinyin    --
-------------------------------------------------------
local data = {
    a = "阿啊呵腌嗄锕錒",
...
--获取单个字符长度
function PinyinSearch.GetCharSize(char)
    if not char then
        return 0
...
--获取中文字符长度
function PinyinSearch.UTF8Len(str)
    local len = 0
    local currentIndex = 1
    while currentIndex <= #str do
        local char = string.byte(str, currentIndex)
        currentIndex = currentIndex + PinyinSearch.GetCharSize(char)
        len = len + 1
    end
    return len
end
...
--截取中文字符串
function PinyinSearch.UTF8Sub(str, startChar, numChars)
    local startIndex = 1
    while startChar > 1 do
        local char = string.byte(str, startIndex)
        startIndex = startIndex + PinyinSearch.GetCharSize(char)
...
for k, v in pairs(data) do
    C_Timer.After(delay/5*0.05, function()
        for i = 1, PinyinSearch.UTF8Len(v) do
            local char = PinyinSearch.UTF8Sub(v, i, 1)
...
function PinyinSearch.Pinyin(chars)
    local results = {}
    chars = string.lower(chars or "")

    local charCount = PinyinSearch.UTF8Len(chars)
    for i = 1, charCount do
        results[i] = {}
        local char = PinyinSearch.UTF8Sub(chars, i, 1)
        if string.len(char) == 1 then
            table.insert(results[i], char)
        else
            local result = pyTable[char]
            if result then
...
local function OnAutoCompleteButtonClick(self)
    local point, relativeTo = PinyinSearch.AutoCompleteFrame:GetPoint(1)
...
PinyinSearch.AutoCompleteFrame = autoCompleteFrame
...
local function FoundResult()
    local editbox = PinyinSearch.Task.EditBox
    if not editbox then return end
    ...
    if text and strlen(text) >= 2 then
        ...
        if #complePinyins > 0 then
            ...
            if source.GetSrc then
                ...
                if not source then
                    PinyinSearch.AutoCompleteFrame:Hide()
                    return
                end
            end

            local count, maxCount = 0, PinyinSearch.AutoCompleteFrame.buttonNum
    ...
    PinyinSearch.AutoCompleteFrame:Update(editbox, results)
end

local function OnTextChanged(self, userInput)
    PinyinSearch.Task:Cancel()

    if userInput then
        PinyinSearch.Task:Start(self, FoundResult)
    else
...
local function OnEscapePressed(self)
    if PinyinSearch.AutoCompleteFrame:IsVisible() then
        PinyinSearch.AutoCompleteFrame:Hide()
    end
end
...
local function OnEnterPressed(self)
    if PinyinSearch.AutoCompleteFrame:IsVisible() then
        local value = PinyinSearch.AutoCompleteFrame:GetSelectedValue()
        if value then
            self:SetText(value)
        end
    end
...
local function OnTabPressed(self)
    if PinyinSearch.AutoCompleteFrame:IsVisible() then
        PinyinSearch.AutoCompleteFrame:ChangeSelectedItem()
    end
...
function PinyinSearch:EnableFeature(type)
...
local function Update(self, ...)
    if not PinyinSearch.LibLoaded then return end
...
local function createFeature(type)
    local feature = PinyinSearch[type]
    if not feature then
        feature = {}
        ...
        PinyinSearch[type] = feature
    end
    return feature
end

local function saveData(src, name)
    local completePinyins, firstLetterPinyins = PinyinSearch.Pinyin(name)
...
PinyinSearch:EnableFeature("Container")
...
local addons = {
    Blizzard_Collections                    = function()
        PinyinSearch:EnableFeature("Pet")
        PinyinSearch:EnableFeature("Mount")
        PinyinSearch:EnableFeature("Toy")
        PinyinSearch:EnableFeature("Appearance")
    end,
    Blizzard_GuildBankUI                    = function()
        PinyinSearch:EnableFeature("GuildBank")
    end,
    Blizzard_TradeSkillUI                   = function()
        PinyinSearch:EnableFeature("TradeSkill")
    end,
    Bagnon                                  = function()
        -- hook searchframe
        ...
        Bagnon.SearchFrame.New = function(self, parent)
            ...
            if parentName and parentName:match("BagnonInventoryFrame") then
                PinyinSearch.AttachEditBox(searchFrame, PinyinSearch["Container"])
            end
            return searchFrame
        end
    ...
    --[[Combuctor                               = function()
        ...
        Combuctor.Frame.New = function(self, id)
            ...
            if frameName and frameName:match("CombuctorInventoryFrame") then
                PinyinSearch.AttachEditBox(frame.searchBox, PinyinSearch["Container"])
                frame:HookScript("OnShow", function()
                    PinyinSearch["Container"]:Update()
                end)
            end
...
frame:SetScript("OnEvent", function(self, event, param1)
    if event == "ADDON_LOADED" then
        PinyinSearch:CompactAddons(param1)
    elseif event == "BANKFRAME_OPENED" then
        PinyinSearch["Container"]:Update()
    elseif event == "TRANSMOG_COLLECTION_UPDATED" or event == "TRANSMOG_COLLECTION_ITEM_UPDATE" or "TRANSMOG_SEARCH_UPDATED" then
        PinyinSearch["Appearance"]:Update()
    end
end)

EventRegistry:RegisterCallback("ContainerFrame.AllBagsOpened", function() PinyinSearch["Container"]:Update() end, frame)
EventRegistry:RegisterCallback("PinyinSearchLib.Complete", function() PinyinSearch.LibLoaded = true end, frame)