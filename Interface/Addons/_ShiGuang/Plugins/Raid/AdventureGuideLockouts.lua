--## Author: alex_chrome, Meivyn  ## IconTexture: Interface\EncounterJournal\UI-EJ-PortraitIcon  ## Version: 1.5.0
local AdventureGuideLockouts = {}

AdventureGuideLockouts.worldBosses = {
    {
...
function AdventureGuideLockouts:RequestWarfrontInfo()
...
function AdventureGuideLockouts:GetSavedWorldBossInfo(instanceIndex)
...
function AdventureGuideLockouts:GetSavedWorldBossEncounterInfo(instanceIndex, encounterIndex)
    local encounter = self.worldBosses[instanceIndex].encounters[encounterIndex]
    local bossName
...
function AdventureGuideLockouts:GetInstanceLockout(instanceIndex)
...
---@param instanceIndex number
---@return table @ instanceLockout
function AdventureGuideLockouts:GetWorldBossLockout(instanceIndex)
...
function AdventureGuideLockouts:UpdateSavedInstances()
...
function AdventureGuideLockouts:CreateStatusFrame(button, orderIndex, difficulty)
...
---@param orderIndex number
function AdventureGuideLockouts:UpdateStatusFramePosition(orderIndex)
    local numVisible = 0
...
function AdventureGuideLockouts:UpdateInstanceStatusFrame(button, elementData)
...
local function UpdateFrames()
    EncounterJournal.instanceSelect.ScrollBox:ForEachFrame(function(frame, elementData)
        AdventureGuideLockouts:UpdateInstanceStatusFrame(frame, elementData)
    end)
end
...
frame:SetScript("OnEvent", function(_, event, arg1)
    if event == "ADDON_LOADED" then
        if arg1 == "_ShiGuang" then
            ...
            AdventureGuideLockouts.worldBosses[5].encounters[4].encounterID = playerFaction == "Horde" and 2212 or 2213
            AdventureGuideLockouts.worldBosses[5].encounters[4].questID =  playerFaction == "Horde" and 52848 or 52847
            AdventureGuideLockouts.worldBosses[5].encounters[8].encounterID = playerFaction == "Horde" and 2329 or 2345
            AdventureGuideLockouts.worldBosses[5].encounters[8].questID =  playerFaction == "Horde" and 54896 or 54895
            AdventureGuideLockouts.playerFaction = playerFaction
    ...
    elseif event == "UPDATE_INSTANCE_INFO" then
        AdventureGuideLockouts:RequestWarfrontInfo()
        AdventureGuideLockouts:UpdateSavedInstances()
    end
end)
