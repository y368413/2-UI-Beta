...
daftAuction:SetScript("OnEvent", function(self, event)
	if event == "AUCTION_ITEM_LIST_UPDATE" and not IsAddOnLoaded("AuctionLite") then -- the auction list was updated or sorted
		if (selectedItem ~= nil) then -- an item was placed in the auction item box
			...
			for i=1, batch do -- SCAN CURRENT PAGE
				local postedItem, _, count, _, _, _, _, minBid, _, buyoutPrice, _, _, _, owner = C_AuctionHouse.GetReplicateItemInfo(i);
