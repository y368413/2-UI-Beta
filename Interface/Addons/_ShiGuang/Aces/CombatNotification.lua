...
local clocks_update = function(self,t)
	...
	if isActive > 0 then
		if not canSend then
			...
			if E.SendDefault then DEFAULT_CHAT_FRAME:AddMessage('|cffff3333=>|r·'.. INFO_WOWTIME_TIP,1,1,1) else SendChatMessage('=>·'.. INFO_WOWTIME_TIP, "GUILD", nil, nil) end
		end
	end
end
...
function f:lfgMsg()
	...
	for i = 1, GetNumRandomDungeons() do
		local _, name = GetLFGRandomDungeonInfo(i)--GetLFGDungeonInfo(462)
	--local name = GetLFGDungeonInfo(462);
	end
	if not name then name = ''; end
	local canSend = (IsInGroup() and IsInInstance()) or IsInRaid() and IsInInstance()
	local eligible, forTank, forHealer, forDamage, itemCount, money, xp = GetLFGRoleShortageRewards(576, 1)
	if eligible and (itemCount > 0) then
		if forTank then 
			str = str.. COMBATNOTIFICATIONINFO_Tank 
			strg = strg.. COMBATNOTIFICATIONINFO_Tank_G
		end
		if forHealer then 
			str = str.. COMBATNOTIFICATIONINFO_Healer 
			strg = strg.. COMBATNOTIFICATIONINFO_Healer_G
		end
		if forDamage then 
			str = str.. COMBATNOTIFICATIONINFO_DPS 
			strg = strg.. COMBATNOTIFICATIONINFO_DPS_G
		end
		if not canSend then
			ElvUIAlertRun(name.. str.. COMBATNOTIFICATIONINFO_need)
			if E.SendGuild then SendChatMessage('=>·'.. name.. strg.. COMBATNOTIFICATIONINFO_need, "GUILD", nil, nil) end
			if E.SendDefault then DEFAULT_CHAT_FRAME:AddMessage('|cffff3333=>|r·'.. name.. str.. COMBATNOTIFICATIONINFO_need,1,1,1) end
		end
		str, strg = "",""
