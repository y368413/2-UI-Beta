...
Castbars.BaseTickDuration =
{
    ...
    [GetSpellInfo(198590) or ""] = 1, -- Drain Soul
    ...
    [GetSpellInfo(173330) or ""] = -1, -- Mana Tea (not modified by haste)
}
...
function Castbars:FrameTimerRestore(frame, adjustTextWidth)
    ...
    elseif (frame.value and frame.timer) then -- MirrorTimer
        ...
        frame.value = maxValue and frame.value > maxValue and maxValue or frame.value;
...
function Castbars:FrameIconRestore(frame)
    if (frame.icon) then
        ...
        if (frame.shield and frame.shield:IsShown() and self.db.profile[frame.configName]["ShowShield"]) then
            ...
            frame.icon:SetHeight(1.1 * height);
            frame.icon:SetWidth(1.1 * height);
            frame.icon:show();
        elseif (self.db.profile[frame.configName]["ShowIcon"]) then
            ...
            frame.icon:SetHeight(1.2 * height);
            frame.icon:SetWidth(1.2 * height);
            frame.icon:show();
        else
            frame.icon:hide();
        end
...
function Castbars:FrameLatencyQueueRestore(frame)
    if (frame.latency and self.db.profile[frame.configName]["ShowLatency"] and not frame.mergingTradeSkill and frame.spellInSpellBook) then
        ...
        local latency = (roundtrip / 1000) / scale;
...
function Castbars:FrameCustomize(frame)
    ...
    if (frameName:sub(1, 11) == "MirrorTimer") then
        ...
        frame.friendlyName = "游泳/疲劳 " .. index;
        if (index == 1) then
            frame.dragable = true;
        end
    ...
    if (frame.dragable) then
        ...
        self.OnEnter = self.OnEnter or function(frame)
            if (self.ConfigMode and not frame:IsDragging()) then
                ...
                GameTooltip:SetText("|cFFFFFFFF鼠标拖动.\n|cFFCCCCCC可以使用↑/↓/←/→进行微调.");
            end
        end
    ...
    if (frameName == "CastingBarFrame") then
        ...
        frame.gcd:SetScript("OnUpdate", function(frameGcd, elapsed)
            ...
            if (frameGcd.elapsed > 0.1) then
                frameGcd.elapsed = 0;
                if (frame:IsVisible()) then frame.gcd.border:SetColorTexture(0, 0, 0, 0) else frame.gcd.border:SetColorTexture(0, 0, 0, 0.5) end
            end
    ...
    if (frameType == "Castbar") then
        frame.setHeight = function(self, height)
            ...
            if (frame.shield) then
                frame.shield:SetHeight(3.6 * height);
                frame.shield:SetWidth(3.6 * height);
                frame.shield:ClearAllPoints();
                frame.shield:SetPoint("LEFT", frame.icon, "LEFT", -0.6 * height, 0);
                frame.shieldborder:SetHeight(3.6 * height);
            end
        ...
        args =
        {
            general =
            {
                ...
                args =
                {
                    ...
                    player = self:GetOptionsTableForBar("CastingBarFrame", "玩家/载具", 2),
                    pet = self:GetOptionsTableForBar("PetCastingBarFrame", "宠物", 3),
                    target = self:GetOptionsTableForBar("TargetCastingBarFrame", "目标", 4),
                    focus = self:GetOptionsTableForBar("FocusCastingBarFrame", "焦点", 5),
                    mirror = self:GetOptionsTableForBar("MirrorTimer", "游泳/疲劳", 6),
                },
            },
    ...
    self.db = LibStub("AceDB-3.0"):New("CastbarsDB",
    {
        profile =
        {
            ...
            PetCastingBarFrame =
            {
                ...
                FontSize = 9,
                Border = "Blizzard Tooltip",
                Position = {point = "CENTER", relpoint = "CENTER", x = 12, y = -265},
            },
            TargetCastingBarFrame =
            {
                ...
                Height = 21,
                Border = "Blizzard Tooltip",
                Position = {point = "CENTER", relpoint = "CENTER", x = 14, y = -155},
            },
            FocusCastingBarFrame =
            {
                ...
                Height = 18,
                Border = "Blizzard Tooltip",
                Position = {point = "CENTER", relpoint = "CENTER", x = 21, y = -130},
            },
            MirrorTimer =
            {
                Width = 210,
                Height = 12,
                Position = {point = "TOP", relpoint = "TOP", x = 0, y = -45},
            },
        }
    });
    ...
    self.CastingBarFrame_OnEvent = function(frame, event, ...)
        ...
        if (event == "UNIT_SPELLCAST_SENT") then				
            ...
            if (unit == "player") then
                ...
                frame.sentCastGUID = castGUID;
            end
            return;
        elseif (event == "ACTIONBAR_UPDATE_COOLDOWN") then
            local startTime, duration = GetSpellCooldown(783); -- Travel Form - works for all classes
        ...
        if ((event == "UNIT_SPELLCAST_START") or (event == "UNIT_SPELLCAST_CHANNEL_START")) then
            ...
            if (unit == frame.unit) then
                ...
                if (unit == "player") then
                    ...
                    if (castGUID == frame.sentCastGUID) then
                    ...
                    frame.gcd.border:SetColorTexture(0, 0, 0, 0);
                    if (frame.channeling) then
