...
do -- temporary? fix for bug clearing the icon when zoning in 7.2
	...
	SequencesFrame:SetScript('OnUpdate', function(self, elapsed)
		if DoUpdate then	
			DoUpdate = false
			self:Hide()
			for name, sequence in pairs(Sequences) do
	...
	SequencesFrame:SetScript('OnEvent', function(self, event, slot)
	...
	SequencesFrame:RegisterEvent('ACTIONBAR_SLOT_CHANGED')
end
