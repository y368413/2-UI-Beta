...
function MISC:DoTrim(trimX, trimY)
	...
	if mover then
		...
		MaoRUISettingDB[mover.__key][mover.__value] = {point, "UIParent", point, x, y}
	end
end
...
function MISC:Mover_OnClick(btn)
	...
	elseif IsControlKeyDown() and btn == "RightButton" then
		...
		MaoRUISettingDB[self.__key][self.__value] = nil
	end
...
function MISC:Mover_OnDragStop()
	...
	MaoRUISettingDB[self.__key][self.__value] = {orig, "UIParent", tar, x, y}
	MISC.UpdateTrimFrame(self)
	updater:Hide()
end
