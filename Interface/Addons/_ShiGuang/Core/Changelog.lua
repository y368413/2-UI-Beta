...
local quest = {
   ...
   "[删除]部分无用的材质和声音，精简整体大小",
   "[删除]删除了上次新加的FloFlyout，有需要自己下吧",
   "[调整]现在不再根据组队情况通报打断\驱散，而是改回最初的直接说出来",
   "[修正]重写了收邮件辨识逻辑，不再偶发有时候邮件收起来特别卡的bug",
   "[修复]Blz自己的一个著名的UIdropdown的报错",
   "[修复]快捷对话插件一个偶发的报错",
   "[添加]拾取框通报按钮(四个按钮颜色对应频道)",
   "[添加]增强幻化预览框，现在可以逐件脱装备",
   "[吐槽]Blz新的改动，导致现在跨种族幻化预览功能太南了",
   "[添加]一键折叠所有任务(大地图右侧框)",
   "[干掉]“你有新的PVP天赋，巴拉巴拉”提示框",
   "[调整]背包整理速度，并修正了一个背包导致游戏突然变卡一下下的bug",
   "[更新]传送小助手添加最新的炉石",
   "[添加]现在你未收集满的套装，也会显示在你待幻化套装列表里",
   "[更新]幻化助手、世界任务助手、艾泽里特特质评分、Maxdps、大地图节日物品标记",
