...
function M:CreateSD(size, override)
	if not override and not MaoRUISettingDB["Skins"]["Shadow"] then return end
...
function M:CreateBD(a)
	...
	M:PixelBorders(self)
	M:SetBackdrop(self, a or MaoRUISettingDB["Skins"]["SkinAlpha"])
...
function M:CreateGradient()
	...
	if MaoRUISettingDB["Skins"]["FlatMode"] then
...
local function Menu_OnMouseUp(self)
	self.bg:SetBackdropColor(0, 0, 0, MaoRUISettingDB["Skins"]["SkinAlpha"])
end
...
function M:ReskinPortraitFrame()
	...
	if closeButton then M.ReskinClose(closeButton) end
	return bg
end
