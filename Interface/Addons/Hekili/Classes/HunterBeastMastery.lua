...
if UnitClassBase( "player" ) == "HUNTER" then
    ...
    spec:RegisterSetting( "barbed_shot_grace_period", 0.5, {
        name = "|T2058007:0|t倒刺射击的缓冲期",
        desc = "如果此项大于0，插件在使用默认优先级配置或|cFFFFD100barbed_shot_grace_period|r表达式时，将提前1个GCD推荐|T2058007:0|t倒刺射击。",
        icon = 2058007,
    ...
    spec:RegisterSetting( "avoid_bw_overlap", false, {
        name = "避免|T132127:0|t狂野怒火重叠",
        desc = "如果勾选，当已拥有此BUFF，插件不会推荐使用|T132127:0|t狂野怒火。",
        type = "toggle",
        width = "full"
    } )

    spec:RegisterSetting( "check_pet_range", true, {
        name = "监测宠物|T132176:0|t杀戮命令的攻击范围",
        desc = function ()
            return "如果勾选，当你的目标不在宠物的攻击范围内，插件不会推荐|T132176:0|t杀戮命令。\n\n" ..
            "必须启用|c" .. ( state.settings.petbased and "FF00FF00" or "FFFF0000" ) .. "使用宠物范围监测"
        end,
        type = "toggle",
        width = "full"
    } )
