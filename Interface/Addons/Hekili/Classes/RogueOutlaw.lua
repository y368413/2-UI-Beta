...
if UnitClassBase( "player" ) == "ROGUE" then
    ...
    spec:RegisterSetting( "mfd_points", 3, {
        name = "|T236340:0|t死亡标记的连击点数",
        desc = "只有你的连击点数小于等于此项设定值是，插件才会推荐使用|T236364:0|t死亡标记。",
        type = "range",
        min = 0,
        max = 5,
        step = 1,
    ...
    spec:RegisterSetting( "dirty_gouge", false, {
        name = "深邃诡计下使用|T132155:0|t凿击",
        desc = "如果勾选，插件会在深邃诡计天赋支撑下，并且你没有足够连击点数时推荐使用|T132155:0|t凿击。" ..
        "这很可能会因|cFFFFD100身位错误|r导致失败，因为凿击技能需要你位于目标正面。\n\n" ..
        "此项默认状态为不勾选。",
        type = "toggle",
        width = "full",
    } )
    ...
    spec:RegisterSetting( "solo_vanish", true, {
        name = "允许独自战斗时使用|T132331:0|t消失",
        desc = "如果不勾选，插件将不会推荐在你独自战斗时使用|T132331:0|t消失（避免脱离战斗）。",
        type = "toggle",
        width = "full"
    } )

    spec:RegisterSetting( "allow_shadowmeld", nil, {
        name = "允许使用|T132089:0|t影遁",
        desc = "如果勾选，插件将在条件满足时推荐身为暗夜精灵的你使用种族技能|T132089:0|t影遁。即使你的动作条没有任何改变，但你确实可以在影遁时使用你原本需要潜行才能使用的技能。\n" ..
        "影遁只会在BOSS战和团队战斗时被推荐（避免脱离战斗）。",
        type = "toggle",
        width = "full",
