...
-- 由于不同的背包插件使用的框体都不一样。对所有背包插件做适配显然不是一个好主意，因此只能修改打开背包的过程。
-- 不同的背包插件，对OpenBag(BagID)和CloseBag(BagID)的操作不同。例如有些只响应OpenBag(BagID)而不响应CloseBag(BagID)。
-- 因此对OpenAllBagsMatchingContext()使用前钩和后钩都不合适。
-- 虽然可以使用ItemButtonUtil.CloseFilteredBags(ChallengesKeystoneFrame)，但这会照成意料之外的背包显示，这不coooooool。
-- 基于上面几点，我直接重写OpenAllBagsMatchingContext(frame)函数，在源头关闭背包打开的动作。
-- 这样既不需要做插件的适配，也不用担心打扰到其它插件。这并不算是一个好办法，但很有效。
-- 可能会增加CPU的调用时间，导致卡顿。这里需要注意测试。
function OpenAllBagsMatchingContext(frame)
	local count = 0;
	for i = 0, NUM_BAG_FRAMES do
        -- 如果找到匹配的物品则打开背包（并且过滤）
        -- ItemButtonUtil.ItemContextMatchResult.Match的值为1， 则匹配； 2为不匹配； 3为不适合
		if ItemButtonUtil.GetItemContextMatchResultForContainer(i) == ItemButtonUtil.ItemContextMatchResult.Match then
            -- 如果背包未打开，则打开背包
			if not IsBagOpen(i) then
                if frame ~= ChallengesKeystoneFrame then
                    OpenBag(i); -- 就是这里做出了打开背包的处理！
                    count = count + 1;
                end
			end
		end
...
-- 来自Ermad的插件AngryKeystones
-- 将钥石放入插槽，即使不打开背包也是可以很好的使用
local function SlotKeystone()
	for container=BACKPACK_CONTAINER, NUM_BAG_SLOTS do
		local slots_num = GetContainerNumSlots(container) -- 背包栏的格子数量
		for slot=1, slots_num do -- 遍历背包中的所有物品
			...
			if slotLink and slotLink:match("|Hkeystone:") then
				PickupContainerItem(container, slot) -- 选中背包中的某个物品（将物品放在光标上）
				if CursorHasItem() then -- 如果鼠标光标当前持有一个物品则返回1
					C_ChallengeMode.SlotKeystone() -- 将钥石放入
				end
			end
		end
	end
...
-- 斜杠处理命令，大多数时候只是用来测试。
SLASH_KEYSTON1, SLASH_KEYSTPN2 = "/KS", "/ks"
SlashCmdList["KEYSTON"] = keep_tracker_state