...
local BloodVial = Class('BloodVial', TheWarWithin.reward.Item, {item = 225952})

function BloodVial:GetStatus()
    local collected = select(11, C_MountJournal.GetMountInfoByID(2222))
    return collected and TheWarWithin.status.Green(L['known']) or
               TheWarWithin.status.Red(L['missing'])
end
