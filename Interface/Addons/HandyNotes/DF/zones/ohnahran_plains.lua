...
map.nodes[20001600] = Treasure({ -- required 70392, 70391
    quest = nil,
    requires = Dragonflight.requirement.Item(198843), -- Emerald Gardens Explorer's Notes
...
map.nodes[32423817] = Treasure({ -- required 67046, 67049
    ...
    requires = Dragonflight.requirement.Item(194540), -- Nokhud Armorer's Notes
...
map.nodes[70603543] = Treasure({
    ...
    requires = Dragonflight.requirement.Item(195453), -- Ludo's Stash Map
    rewards = {
...
function Lizi.getters:note()
    local function status(i)
        if C_QuestLog.IsQuestFlaggedCompleted(self.quest[i]) then
            return Dragonflight.status.Green(i)
        else
            return Dragonflight.status.Red(i)
        end
    end
