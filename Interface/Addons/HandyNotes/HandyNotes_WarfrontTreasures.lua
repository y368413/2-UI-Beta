local HandyNotes_WarfrontTreasures = {}
...
HandyNotes_WarfrontTreasures_HL = HL
...
local function work_out_texture(point)
    ...
    if HandyNotes_WarfrontTreasures_db.icon_item then
        if point.item then
...
local get_point_info_by_coord = function(uiMapID, coord)
    return get_point_info(HandyNotes_WarfrontTreasures_points[uiMapID] and HandyNotes_WarfrontTreasures_points[uiMapID][coord])
end
...
local function handle_tooltip(tooltip, point)
    if point then
        ...
        if point.quest and HandyNotes_WarfrontTreasures_db.tooltip_questid then
        ...
        if (HandyNotes_WarfrontTreasures_db.tooltip_item or IsShiftKeyDown()) and (point.item or point.npc) then
...
local handle_tooltip_by_coord = function(tooltip, uiMapID, coord)
    return handle_tooltip(tooltip, HandyNotes_WarfrontTreasures_points[uiMapID] and HandyNotes_WarfrontTreasures_points[uiMapID][coord])
end
...
local function hideNode(button, uiMapID, coord)
    HandyNotes_WarfrontTreasures_hidden[uiMapID][coord] = true
    HL:Refresh()
end
...
do
    ...
    local function generateMenu(button, level)
        ...
        if (level == 1) then
            ...
            info.isTitle      = 1
            info.text         = "HandyNotes_WarfrontTreasures"
            info.notCheckable = 1
    ...
    function HLHandler:OnClick(button, down, uiMapID, coord)
        ...
        local point = HandyNotes_WarfrontTreasures_points[currentZone] and HandyNotes_WarfrontTreasures_points[currentZone][currentCoord]
...
do
    ...
    local function iter(t, prestate)
        ...
        while state do -- Have we reached the end of this zone?
            if value and HandyNotes_WarfrontTreasures_should_show_point(state, value, currentZone, isMinimap) then
                local label, icon, _, _, _, scale, alpha = get_point_info(value)
                scale = (scale or 1) * (icon and icon.scale or 1) * HandyNotes_WarfrontTreasures_db.icon_scale
                return state, nil, icon, scale, HandyNotes_WarfrontTreasures_db.icon_alpha * alpha
            end
    ...
    function HLHandler:GetNodes2(uiMapID, minimap)
        currentZone = uiMapID
        isMinimap = minimap
        if minimap and HandyNotes_WarfrontTreasures_map_spellids[uiMapID] then
            if HandyNotes_WarfrontTreasures_map_spellids[mapFile] == true then
                return iter
            end
            if UnitHasBuff("player", HandyNotes_WarfrontTreasures_map_spellids[mapFile]) then
                return iter
            end
        end
        return iter, HandyNotes_WarfrontTreasures_points[uiMapID], nil
    end
end
...
function HL:OnInitialize()
    -- Set up our database
    self.db = LibStub("AceDB-3.0"):New("HandyNotes_WarfrontTreasuresDB", HandyNotes_WarfrontTreasures_defaults)
    HandyNotes_WarfrontTreasures_db = self.db.profile
    HandyNotes_WarfrontTreasures_hidden = self.db.char.hidden
    -- Initialize our database with HandyNotes
    HandyNotes:RegisterPluginDB("HandyNotes_WarfrontTreasures", HLHandler, HandyNotes_WarfrontTreasures_options)
...
HandyNotes_WarfrontTreasures_defaults = {
    profile = {
        show_on_world = true,
...
HandyNotes_WarfrontTreasures_options = {
    type = "group",
    name = "HandyNotes_WarfrontTreasures",
    get = function(info) return HandyNotes_WarfrontTreasures_db[info[#info]] end,
    set = function(info, v)
        HandyNotes_WarfrontTreasures_db[info[#info]] = v
        HandyNotes_WarfrontTreasures_HL:SendMessage("HandyNotes_NotifyUpdate", "HandyNotes_WarfrontTreasures")
    end,
    args = {
        icon = {
            type = "group",
        ...
        display = {
            ...
            args = {
                ...
                unhide = {
                    ...
                    func = function()
                        for map,coords in pairs(HandyNotes_WarfrontTreasures_hidden) do
                            wipe(coords)
                        end
                        HandyNotes_WarfrontTreasures_HL:Refresh()
                    end,
                    order = 50,
                },
            },
...
HandyNotes_WarfrontTreasures_should_show_point = function(coord, point, currentZone, isMinimap)
    if isMinimap and not HandyNotes_WarfrontTreasures_db.show_on_minimap and not point.minimap then
        return false
    elseif not isMinimap and not HandyNotes_WarfrontTreasures_db.show_on_world then
        return false
    end
    if point.level and point.level ~= currentLevel then
        return false
    end
    if HandyNotes_WarfrontTreasures_hidden[currentZone] and HandyNotes_WarfrontTreasures_hidden[currentZone][coord] then
        return false
    end
    if HandyNotes_WarfrontTreasures_outdoors_only and IsIndoors() then
        return false
    end
    if point.junk and not HandyNotes_WarfrontTreasures_db.show_junk then
        return false
    end
    if point.faction and point.faction ~= player_faction then
        return false
    end
    if (not HandyNotes_WarfrontTreasures_db.found) then
        if point.quest then
    ...
    -- if (not HandyNotes_WarfrontTreasures_db.repeatable) and point.repeatable then
    --     return false
    -- end
    if not point.follower then
        if point.npc then
            if not HandyNotes_WarfrontTreasures_db.show_npcs then
                return false
            end
        else
            -- Not an NPC, not a follower, must be treasure
            if not HandyNotes_WarfrontTreasures_db.show_treasure then
                return false
            end
        end
    end
    if point.hide_before and not HandyNotes_WarfrontTreasures_db.upcoming and not allQuestsComplete(point.hide_before) then
        return false
    end
    return true
end
...
HandyNotes_WarfrontTreasures_merge = merge
...
HandyNotes_WarfrontTreasures_path = path

HandyNotes_WarfrontTreasures_map_spellids = {
    -- [862] = 0, -- Zuldazar
    -- [863] = 0, -- Nazmir
    -- [864] = 0, -- Vol'dun
...
HandyNotes_WarfrontTreasures_points = {
    --[[ structure:
...
local merge = HandyNotes_WarfrontTreasures_merge
local path = HandyNotes_WarfrontTreasures_path

merge(HandyNotes_WarfrontTreasures_points[14], { --Arathi
    --World Boss
