...
function WR_ShamanRestoration()
	...
	then
		if UnitExists("focus") and not UnitCanAttack("player","focus") and FocusHP<=0.50 then	--焦点是友方 并且 焦点血量低于70%
...
function WR_ShamanRestoration_ZLZY(Text)
	...
	if not WR_Shaman_Restoration() then return false end
...
function WR_ShamanRestoration_YDTT()
	...
	if WR_SpellUsable("倾盆大雨") then return false end	--技能不可用
...
function WR_Shaman_Restoration()
	if WR_Shaman_RestorationIsTrue then return true end
	if ShowColorFramePass~=nil and ShowColorFramePass()~=nil then
		...
		if result ==7560 or result ==24000 or result ==17280 or result ==241920 then
			WR_Shaman_RestorationIsTrue = true
			return true
		end
	end
...
function WR_ShamanRestoration_ZLZQTT()
	...
	and UnitAffectingCombat("player")	--玩家战斗中
	and WR_RangeCountPR(40,0.90)>=3	--40码内血量低于90%血量的队友数量>=3
	and
	(
		not Totem_ZLZQ	--治疗之泉图腾 不存在
		or
		WR_RangeCountPR(40,0.70)>=3	--40码内血量低于70%血量的队友数量>=3
	)
	and WR_Shaman_Restoration()
	then
...
function WR_ShamanRestoration_ZLL(Text)
	...
	then
		...
		(
			...
			and (WR_GetSpellCharges("治疗之泉图腾")~=nil and WR_GetSpellCharges("治疗之泉图腾")<2)	--可用次数<2
			and not PlayerMove --玩家没有移动
			and WR_RangeCountPR(40,0.85)>=4	--40码内血量低于85%血量的队友数量>=4
			and WR_GetUnitBuffTime("player","浪潮汹涌")>GCD	--浪潮汹涌存在
			and FocusLostHealth > ZLL_Value	--焦点损失血量 > 治疗链 治疗量
			and WR_GetUnitBuffCount("player","潮汐使者")>=1	--潮汐使者 BUFF层数>=1
		)
		or
		(
			Text=="使者"
		...
		(
			Text=="迅捷"
			and (WR_GetSpellCharges("治疗之泉图腾")~=nil and WR_GetSpellCharges("治疗之泉图腾")<2)	--可用次数<2
			and WR_GetUnitBuffTime("player","自然迅捷")>GCD	--潮汐使者 BUFF存在
