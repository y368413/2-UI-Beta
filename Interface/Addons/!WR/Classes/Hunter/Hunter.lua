...
function WR_HunterSettingsSave()
	if WRSet~=nil then
		...
		WRSet.SW_SFSD=UIDropDownMenu_GetSelectedID(SW_SFSD_Dropdown) --施法速度
		...
		WRSet.SC_SFSD=UIDropDownMenu_GetSelectedID(SC_SFSD_Dropdown) --施法速度
	end
...
function WR_Hunter_Function_JS()
	...
	(
		...
		or
		WR_GetRangeSpellTime(45,EscapeSpellName,"player")<1.5	--对我施法<=1.5秒
		or
		(
			IsPlayerSpell(459517)	--已学 紧急药膏
			and
			WR_GetUnitDebuffInfo("player","虚空裂隙")~=0	--虚空裂隙 DEBUFF存在
		)
	)
	then
...
function WR_Hunter_DMSJ(Unit,Text)
	...
	(
		...
		or
		WR_GetUnitBuffTime("player","干掉他们")>GCD	--生存猎
		or
		WR_GetUnitBuffTime("player","捕掠猎获")>GCD	--兽王猎
		or
		WR_GetUnitBuffTime("player","夺命打击")>GCD	--通用天赋
	)
	and
	(
		Text==nil
	...
	then
		if Unit=="target" then
			if WR_ColorFrame_Show("CN8",(Text or "") .. "夺命T") then return true end
		elseif Unit=="mouseover" then
			if WR_ColorFrame_Show("CN9",(Text or "") .. "夺命M") then return true end
		elseif Unit=="party1target" then
			if WR_ColorFrame_Show("AN7",(Text or "") .. "夺命P1T") then return true end
		elseif Unit=="party2target" then
			if WR_ColorFrame_Show("AN8",(Text or "") .. "夺命P2T") then return true end
		elseif Unit=="party3target" then
			if WR_ColorFrame_Show("AN9",(Text or "") .. "夺命P3T") then return true end
		elseif Unit=="party4target" then
			if WR_ColorFrame_Show("AN0",(Text or "") .. "夺命P4T") then return true end
		end
	end
	return false
end
