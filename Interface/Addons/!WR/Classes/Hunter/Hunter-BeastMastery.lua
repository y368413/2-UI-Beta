...
function WR_HunterBeastMastery()
	...
	if not WR_FastPass then	--如果不是快速通过,则执行下列数据初始化过程
		ShiFaSuDu=0.05+WRSet.SW_SFSD*0.05
		WR_Initialize()	--初始化
...
function WR_HunterBeastMastery_FZSJ(Unit)
	...
	and WR_GetCastInterruptible(Unit,InterruptTime,WR_HunterBeastMastery_FZSJ_Important)	--指定"单位"施法剩余时间小于设定
	then
		if Unit=="target" then
...
function WR_HunterBeastMastery_Function_FZSJ()
	...
	if WRSet.SW_FZSJ<=4 then
		WR_HunterBeastMastery_FZSJ_Important = false
	else
		WR_HunterBeastMastery_FZSJ_Important = true
	end
	
	if WRSet.SW_FZSJ==9 then	--禁用
		return false
	elseif (WRSet.SW_FZSJ==1 or WRSet.SW_FZSJ==5)	--智能
	or
	(
		(WRSet.SW_FZSJ==4 or WRSet.SW_FZSJ==8)	--焦点
		and
		(
	...
	elseif (WRSet.SW_FZSJ==2 or WRSet.SW_FZSJ==6) then	--目标
		if WR_HunterBeastMastery_FZSJ("target") then return true end
	elseif (WRSet.SW_FZSJ==3 or WRSet.SW_FZSJ==7) then	--指向
		if WR_HunterBeastMastery_FZSJ("mouseover") then return true end
	elseif (WRSet.SW_FZSJ==4 or WRSet.SW_FZSJ==8) then	--焦点
...
function WR_HunterBeastMastery_HSJ(Text)
	if GCD<=ShiFaSuDu
	and TargetInRange40 --当前目标在40码
	and IsPlayerSpell(466932)	--已学 黑蚀箭
	--and zhandoumoshi==1	--爆发 开启
