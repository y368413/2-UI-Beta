...
--
	...
	function ZGV_DefaultSkin_StepLine_Mixin:ShowAsGrind(goal,text,briefhidden)
		self.goal=goal
		self.isheader = nil
		self.tipgoal = nil
		self.back:Hide()
	...
	function ZGV_DefaultSkin_StepLine_Mixin:ShowGoalIcon()
		...
		if ZGV.db.profile.goalicons then
			...
			elseif goal.loadguide then
				self:SetIcon(ZGV.actionicons['loadguide'])
	...
	function ZGV_DefaultSkin_StepLine_Mixin:ShowAsLoadguide(goal,text,briefhidden)
		self.goal=goal
		self.isheader = nil
		self.tipgoal = nil
		self.back:Hide()
...
--ZGV_DefaultSkin_Frame_Mixin
	...
	function ZGV_DefaultSkin_Frame_Mixin:ShowSpecialState()	
		...
		elseif state=="select" then
			...
			if not InCombatLockdown() then ZGV.ActionBar.Frame:Hide() end
...
function ZGV_DefaultSkin_TitleButton_Mixin:SetButtonColor(r,g,b,a)
	for _,tx in ipairs{self:GetNormalTexture(),self:GetPushedTexture(),self:GetDisabledTexture()} do
...
ZygorSpecialButton_Mixin = {}
function ZygorSpecialButton_Mixin:SetTextureInsetsFromSkin()
	local insetx = self.forceinsetx or SkinData("TitleButtonInset") or 0
	local insety = self.forceinsety or  SkinData("TitleButtonInset") or 0
...
-- inherits ZGV_DefaultSkin_TitleButton. Override functions in proper objects
function ZygorSpecialButton_Mixin:OnEnter() end
function ZygorSpecialButton_Mixin:OnLeave() end
function ZygorSpecialButton_Mixin:OnClick() end