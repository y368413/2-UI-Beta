...
---@class LibRover
local Lib = LibStub:NewLibrary(LIB_MAJOR, LIB_MINOR)  if not Lib then return end
AceTimer:Embed(Lib)
---@class LibRover
local LibRover=Lib
...
local tinsert,tremove,ipairs,pairs,next,debugprofilestop,tonumber,table,wipe,type,assert,error,pcall,print,debugstack
    = tinsert,tremove,ipairs,pairs,next,debugprofilestop,tonumber,table,wipe,type,assert,error,pcall,print,debugstack
local IsSpellKnown,UnitLevel,IsQuestFlaggedCompleted,GetSpellCooldown
    = IsSpellKnown,UnitLevel,C_QuestLog.IsQuestFlaggedCompleted,C_Spell.GetSpellCooldown or GetSpellCooldown
...
local TAXI_FROWN_UNKNOWNS = 500

local INVOLUNTARY_MAX=0.5
local INVOLUNTARY_CUT=0.2
...
if Lib.IsClassic or Lib.IsClassicCATA then
	...
	if Lib.IsClassicCATA then
		Mixin(MAPENUM,{
			["TWILIGHTHIGHLANDS"]=241,
			["STRANGLETHORNVALE"]=224,
			["NORTHERNSTRANGLETHORN"]=1434,
			["THECAPEOFSTRANGLETHORN"]=210,
			["VASHJIR"]=203,
			["ABYSSALDEPTHS"]=204,
			["KELPTHARFOREST"]=201,
...
function Lib:UpdateConfig(profile)
	if profile.travelmode == 1 then
		...
		Lib.cfg.use_hearth = false
		Lib.cfg.use_item_teleports = false
		Lib.cfg.use_whistle = false
	...
	elseif profile.travelmode == 3 then
		...
		Lib.cfg.avoid_portals = false
		Lib.cfg.use_hearth = true
		Lib.cfg.use_item_teleports = true
		Lib.cfg.use_whistle = true
		Lib.cfg.use_spells = true
		Lib.cfg.use_astral_recall = true
		Lib.cfg.use_mage_teleport = true
	else -- custom
		Lib.cfg.forcedirect = false
		Lib.cfg.pathfinding_comfort=profile.pathfinding_comfort
		Lib.cfg.avoid_portals = profile.travelavoidportals
		...
		Lib.cfg.use_spells = profile.travelusespells
...
function Lib:GetCooldownWithoutGCD(what,id)
	...
	if what=="spell" then 
		...
		if type(start)=="table" then start,dur,active = start.startTime,start.duration,start.isEnabled end
...
local cond_env = setmetatable({  -- some fancy functions for node conditions to use
	...
	hasbuff = function(spell) return ZGV.Parser.ConditionEnv.hasbuff(spell) end,
},{__index=_G})
...
local function _StartupThread()
	...
	do -- INITIALIZE WALLS
		local segment="walls"
		for zone,zdata in ZGV.OrderedPairs(Lib.data.walls or {}) do repeat
			local zname,f = zone:match("^(.-)%s*/%s*(.-)$")
			local mapid,floor = Lib:GetMapByNameFloor(zname or zone,f)
			if not mapid then break end --continue
	...
	ZGV:SendMessage("LIBROVER_INITIALISED")
...
function Lib:SetupInitialQuickTravel(startnode)
	...
	for i,port in ipairs(Lib.data.portkeys) do repeat
		...
		local reject_reason = (function()
			...
			if port.mode=="hearth" and not Lib:GetCFG("use_hearth") then  return "use_hearth off"  end  -- obviously
			if port.mode=="ghearth" and not Lib:GetCFG("use_item_teleports") then  return "use_item_teleports off"  end  -- obviously
			if port.mode=="dhearth" and not Lib:GetCFG("use_item_teleports") then  return "use_item_teleports off"  end  -- obviously
			...
			if port.spell and not Lib:GetCFG("use_spells") then  return "use_spells off"  end
...
function Lib:IsDestinationImpossible(mymap,destmap)
	...
	if destmap==-1 then return true,"FORCE_REJECT","map is known not to exist" end
	...
	local SP_DRAGONRIDING_BASICS = IsSpellKnown(376777) and (SP_JOURNEYMAN_RIDING or SP_EXPERT_RIDING or SP_ARTISAN_RIDING or SP_MASTER_RIDING)

	local canfly = SP_EXPERT_RIDING or SP_ARTISAN_RIDING or SP_MASTER_RIDING or SP_DRAGONRIDING_BASICS
	...
	elseif cont==1550 and expansion<EXPANSION_SL  then
...
function Lib:BoatLockDisable(mode)
	--"manual"
	--"arrival"
	--"rocking"
	--"menu"
	--"loading"
	--"stepchange"
	Lib:Debug("Disabling boatlock from "..(mode or "no mode"))

	LibRover.BoatLockPreventTimed = false 
	self:CancelTimer(LibRover.BoatLockPreventTimer)

	if ZGV.db.profile.debug_librover_boat_timedprevent then
		if mode=="menu" then
			Lib:Debug("Boatlock starting BoatLockPreventTimed")
			LibRover.BoatLockPreventTimed = true
			LibRover.BoatLockPreventTimer = self:ScheduleTimer(function() 
				LibRover.BoatLockPreventTimed = false 
				Lib:Debug("Boatlock clearing BoatLockPreventTimed")
			end,30)
		end
	end
	LibRover.BoatLock = false
end

function Lib:BoatLockEnable(mode)
	--"manual"
	--"arrival"
	--"rocking"
	if LibRover.BoatLockPreventTimed then 
		Lib:Debug("Boatlock prevented by BoatLockPreventTimed")
		return
	end

	Lib:Debug("Enabling boatlock from "..(mode or "no mode"))
	LibRover.BoatLock = true
end
...
function Lib:FindPath(am,ax,ay,bm,bx,by, handler, extradata, force_new, quiet)
	...
	if self.BoatLock then Lib:Debug("No recalc, boatlocked") return false end
	...
	self:Debug("&_PUSH |cff00ff88FindPath|r:  |cffaaff88%s|r##%d |cffffffff%.1f|r,|cffffffff%.1f|r --> |cffaaff88%s|r##%d |cffffffff%.1f|r,|cffffffff%.1f|r, %s",
		...
		extradata.direct and "direct" or "pathing")
...
function Lib:InitializePath_Threaded() --called in as part of StepForever_Threaded only.
	...
	for ni=1,#all do  local node=all[ni]
	...
	local all=Lib.nodes.all
	for ni=1,#all do  local node=all[ni]
		for i=1,#temp_fields_i do  local field=temp_fields_i[i]
			node[field]=nil
		end
		count=count+1
...
local MAPENUM_ISLEOFDORN=2248
local MAPENUM_DORNOGAL=2339
local MAPENUM_RINGINGDEEPS=2214
local MAPENUM_HALLOWFALL=2215
local MAPENUM_AZJKAHET=2255
local MAPENUM_AZJKAHET_LOWER=2256
local MAPENUM_CITYOFTHREADS=2213
local MAPENUM_CITYOFTHREADS_LOWER=2216
local IsKhazMap = {[MAPENUM_DORNOGAL]=1,[MAPENUM_ISLEOFDORN]=1,[MAPENUM_RINGINGDEEPS]=1,[MAPENUM_HALLOWFALL]=1,[MAPENUM_AZJKAHET]=1,[MAPENUM_AZJKAHET_LOWER]=1,[MAPENUM_CITYOFTHREADS]=1,[MAPENUM_CITYOFTHREADS_LOWER]=1}
...
function Lib:StepPath()  -- THE WORKHORSE.
	...
	local CFG_avoid_highlevel_zones = Lib:GetCFG("avoid_highlevel_zones")
	local CFG_pathfinding_comfort = Lib:GetCFG("pathfinding_comfort")
	local CFG_prefer_taxi = Lib:GetCFG("prefer_taxi")
	...
	local maxspeed,runspeed,flyspeed,normalflyspeed = unpack(speeds)
	if Lib:GetCFG("prefer_taxi") and not Lib:GetCFG("forcedirect") and normalflyspeed then -- in comfort mode use normal flying mount speed for dragonriding
		flyspeed = max(flyspeed,normalflyspeed) * 0.8
	end
	...
	local current_istaxiknown = current:IsTaxiKnown()
	...
	local lib_opti_neighcond = Lib.opti_neighcond

	--for i,neighpair in ipairs(current.n) do
	local current_n = current.n
	for ni=1,#current_n do  local current_n_ni=current_n[ni]  local neigh,neighlink=current_n_ni[1],current_n_ni[2]
		...
		--]]
			...
			lib_debug_time.neigh_count = lib_debug_time.neigh_count + 1
		...
		local neightype=neigh.type
		local cached_neighlink = lib_opti_neighcond and tempstore[neighlink]
		local cached_neigh = lib_opti_neighcond and tempstore[neigh]


		
		local border_opt_reject
		--[[
		if neigh.status~="closed" and neigh.status~="reject" and Lib.do_border_opti then
		...
		local neighlink_passed_cond,neigh_passed_cond=true,true
		if neigh.status~="closed" and neigh.status~="reject" then
			if lib_opti_neighcond then
				if neighlink.cond_fun then
					if cached_neighlink.passed_cond==nil then
						neighlink_passed_cond = not neighlink.cond_fun or neighlink.cond_fun()  -- and aren't failing a condition
						cached_neighlink.passed_cond = not not neighlink_passed_cond
					else
						neighlink_passed_cond = cached_neighlink.passed_cond
					end
				end
				if neigh.cond_fun then
					if cached_neigh.passed_cond==nil then
						neigh_passed_cond = not neigh.cond_fun or neigh.cond_fun()  -- and aren't failing a condition
						cached_neigh.passed_cond = not not neigh_passed_cond
					else
						neigh_passed_cond = cached_neigh.passed_cond
					end
				end
			else
				neighlink_passed_cond = Lib.ignore_travel_conditions or not neighlink.cond_fun or neighlink.cond_fun()
				neigh_passed_cond = Lib.ignore_travel_conditions or not neigh.cond_fun or neigh.cond_fun()
			end
		end

		-- try to open neighbours who:
		if neigh.status~="closed"  -- aren't closed already
		and neigh.status~="reject"  -- aren't rejected
		and neighlink_passed_cond
		and not border_opt_reject
		--and neigh_passed_cond -- NO. Taxis may still be flied over even if failed a cond.
		...
		then
			...
			if cached_neigh and neigh.type=="taxi" and not cached_neigh.taxi_known_cached then cached_neigh.taxi_known,cached_neigh.taxi_desc,cached_neigh.taxi_couldbe = neigh:IsTaxiKnown()  cached_neigh.taxi_known_cached=true  end
			...
			--[[  DO NOT ADD PENALTIES HERE, THIS IS THE TRUE TRAVEL TIME ]]--

			-- if it's border-in-flight, make sure not to default to "walk". Player may fly. Just use whatever brought us here.
			if current.border_in_flight and neigh==current.border and current.parentlink then mode=current.parentlink.mode end
			...
			elseif mode=="ship" or mode=="zeppelin" then
				...
				speed = (mode=="fly" and flyspeed or runspeed) * BASE_MOVEMENT_SPEED
			...
			if mode=="taxi" then -- apply bonuses
				if IsKhazMap[current.m]
				and LibTaxi:PlayerHasKhazAlgarFlightMasterAchievement() then -- flight time was 25% reduced
					mytime = mytime * 0.75
					if cost_debugging then costdesc = costdesc .. "KAFM achi; " end
				elseif IsSpellKnown(117983) then  -- Guild Perk Ride like the Wind.
					mytime = mytime * 0.8
			...
			lib_debug_time.scoring1 = lib_debug_time.scoring1 + debugprofilestop() - debug_time_scoring_1

			local debug_time_scoring_2 = debugprofilestop()
			...
			if neighlink.direct then mycost = self.COST_FORCED end
			...
			-- Penalize uncomfortable modes of travel.
				...
				then
					...
					if mode=="walk" then
						local mod = 1 + (CFG_pathfinding_comfort * 1)
						mycost = mycost * mod
						if cost_debugging then costdesc = costdesc .. ("cmft=%d%%,*%.2f; "):format(CFG_pathfinding_comfort*100,mod) end
					elseif mode=="taxi" then
						...
						local mod = 1 - (CFG_pathfinding_comfort * COSTMOD_COMFORT_TAXI)
						mycost = mycost * mod
						if cost_debugging then costdesc = costdesc .. ("cmft=%d%%,*%.2f; "):format(CFG_pathfinding_comfort*100,mod) end
					end

					-- mode change penalty
					local mod = 1 + (CFG_pathfinding_comfort * changed_modes * 0.2)
					mycost = mycost * mod
					if cost_debugging then costdesc = costdesc .. ("modechange %d*%.2f; "):format(changed_modes,mod) end
				end

			-- Promote comfortable modes of travel.
				if mode=="taxi" and CFG_prefer_taxi then
					local mod = 1 - COSTMOD_COMFORT_TAXI
					mycost = mycost * mod
					if cost_debugging then costdesc = costdesc .. ("prefer_taxi,*%.2f; "):format(mod) end
				end
			-- ==
			...
			if (mode=="walk" or mode=="fly") and current.parentlink and current.parentlink.mode=="taxi" and current_istaxiknown==false then -- walking from an unknown taxi, means we have LANDED on it. Penalize!
				mycost=COST_FAILURE+1
			...
			if mode=="portal" then
				...
				if Lib:GetCFG("frown_on_portals") then mycost = mycost + 10 end	-- Usually portals decrease travel time significantly, so increasing their cost has no effect. But short portal hops are confusing so this makes short hops happen more rarely.
				-- Note, this HAS to be an added penalty, not a multiplier, or else EVERY portal gets ridiculously penalized.
			end

			-- ==
			if neightype=="taxi" and (mode=="walk" or mode=="fly") then -- departing from conditioned taxis is bad
				local known,desc,couldbe
				if cached_neigh then known,desc,couldbe = cached_neigh.taxi_known,cached_neigh.taxi_desc,cached_neigh.taxi_couldbe
				else known,desc,couldbe = neigh:IsTaxiKnown() end
				if known==false then
					mycost=mycost+COST_FAILURE+20 -- absolutely don't fly from known unknowns
				elseif known==nil and couldbe then
					mycost=mycost+TAXI_FROWN_UNKNOWNS -- and discourage maybes
				end
			...
			if not neigh_passed_cond then
				mycost=mycost+COST_FAILURE+21
				if cost_debugging then costdesc = costdesc .. "failed cond_fun; " end
				-- don't fail the node, it might be reachable from a different angle
			end
			...
			-- === FAILED NODES 
				...
				-- Ban nodes by quest/faction.
					if (neigh.factionid and select(3,GetFactionInfoByID(neigh.factionid))<neigh.factionstanding) then
						...
						if neightype~="taxi" then neigh.status="reject" end
			...
			if lib_debug_onodes or (lib_debug_nodes_any and lib_debug_nodes[neigh.num]) then
				Lib:Debug ("|cffffffffFrom [%d]: %s%s|r (%s %.1f), c=|cffddffdd%.1f|r",current.num, colors[neightype] or "",neigh:tostring(), mode,mycost, cost)
			end
			...
			if not neigh.cost or cost<neigh.cost then
				if lib_debug_onodes or (lib_debug_nodes_any and lib_debug_nodes[neigh.num]) then
					if neigh.cost then
						Lib:Debug ("Updating node: %s%s|r (%s), c=%.1f>%.1f", colors[neightype] or "",neigh:tostring(), mode, neigh.cost,cost)
					else
						--Lib:Debug (("Opening node: %s%s|r, c=|cffddffdd%.1f|r"):format(colors[neigh.type] or "",neigh:tostring(),cost))
						Lib:Debug ("Opening node: %s%s|r (%s), c=%.1f", colors[neightype] or "",neigh:tostring(), mode, cost)
					end
				...
				lib_debug_time.scoring2 = lib_debug_time.scoring2 + debugprofilestop() - debug_time_scoring_2
			...
			if updated then
				...
				elseif neigh.status=="reject" then
					-- ignore those. Do not open, ever.
				else
	...
	-- PREPARE NODES FOR DISPLAY. Assign titles based on situation.
		for n=1,#results do repeat
			...
			local text = text
				...
				:gsub("{spell}",(node.spell and getSpellName(node.spell) or "Teleport"))
...
Lib.involuntary_movement = 0

local ot,om,ox,oy,ospdel
ospdel=0
function Lib:OnUpdate(elapsed)
	...
	if self.calculating and self.thread then
		...
		if code=="PENDING" then
			...
			.." cheapest |cffffffff%.2f|r, initial |cffffffff%.2f|r, overhead |cffffffff%.2f|r, blah |cffffffff%.2f|r, scoring1 |cffffffff%.2f|r, scoring2 |cffffffff%.2f|r, adding |cffffffff%.2f|r ms    \n"
			...
			Lib.debug_time.cheapest, Lib.debug_time.initial, Lib.debug_time.neighoverhead, Lib.debug_time.neighblah, Lib.debug_time.scoring1, Lib.debug_time.scoring2, Lib.debug_time.adding, --Lib.debug_time.closed, Lib.debug_time.final,
	...
	-- detect player involuntary movement (boats etc.)

	ospdel = ospdel+elapsed
	if ospdel>0.1 then
		local x,y,m = self:GetPlayerPosition()
		local t = GetTime()
		if om and ox and oy then
			local dist = Mdist(om,ox,oy,m,x,y)
			if dist then
				self.actual_speed = dist/(t-ot)
			else
				self.actual_speed = nil
			end
		else
			self.actual_speed = nil
		end
		local isGliding = C_PlayerInfo and C_PlayerInfo.GetGlidingInfo and C_PlayerInfo.GetGlidingInfo()
		local inv_mov = (GetUnitSpeed("player")==0 and not isGliding and self.actual_speed and self.actual_speed>0)
		if inv_mov then
			self.involuntary_movement = self.involuntary_movement + ospdel
			if self.involuntary_movement > INVOLUNTARY_MAX then self.involuntary_movement = INVOLUNTARY_MAX end
		else
			self.involuntary_movement = self.involuntary_movement - ospdel/4
			if self.involuntary_movement < 0 then self.involuntary_movement = 0 end
			--if self.involuntary_movement>0.3 then print("I'M BEING KIDNAPPED "..self.actual_speed) end
		end
		self.is_involuntary_movement = self.involuntary_movement>=INVOLUNTARY_CUT
		--print (self.is_involuntary_movement and "BOAT" or "----",self.involuntary_movement)
		ot,om,ox,oy=t,m,x,y
		ospdel=0
	end
...
function Lib.HasBuff(name,id)
	if UnitBuff then
		...
		local aura = C_UnitAuras.GetPlayerAuraBySpellID(tonumber(name))
...
function Lib:CheckMaxSpeeds()
	...
	local dragonspeed = SP_DRAGONRIDING_BASICS and FLIGHTMODE_DYNAMIC and 10
	...
	if ZGV and ZGV.db then  -- debug overrides
		...
		if ZGV.db.profile.debug_librover_flightazeroth~=nil then canfly_in_azeroth=ZGV.db.profile.debug_librover_flightazeroth end
		if ZGV.db.profile.debug_librover_flightnorthrend~=nil then canfly_in_northrend=ZGV.db.profile.debug_librover_flightnorthrend end
	...
	Lib.speeds = { -- groundspeed, dragonspeed, steadyspeed
	...
	Lib.debug_maxspeeds = {
		...
		['SP_LICENCE_WOTLK']=SP_LICENCE_WOTLK,
		['SP_LICENCE_CATA']=SP_LICENCE_CATA,
		['SP_LICENCE_PANDARIA']=SP_LICENCE_PANDARIA,
		['SP_DRAGONRIDING_BASICS']=SP_DRAGONRIDING_BASICS,
...
local function onEvent(this, event, arg1, arg2, arg3, arg4, arg5)
	...
	elseif event=="NEW_WMO_CHUNK" then
		...
		if tm-Lib.last_zone_event>MAX_ZONE_CHANGE_FREQ then
			Lib:Debug("Got event %s, updating",event)
			Lib:UpdateNow("quiet",ZGV.db.profile.pathfinding_speed/3) -- hard force update, but quiet
			Lib.last_zone_event=tm
			Lib:ScheduleTimer(function()
			...
			Lib:Debug("Got event %s, too hot (%ds of %ds), waiting",event,tm-Lib.last_zone_event,MAX_ZONE_CHANGE_FREQ)
			Lib.last_zone_event=tm
		end
...
function LibRover:DebugNodes(n1,n2,...)
	...
	if n1 and n2 then
		...
		local function debugit(n1,n2)
			...
			if walkwhy and walkwhy:find("different continent") then walkwhy=walkwhy..((" (%s,%s)"):format(n1.c,n2.c)) end
...
do -- TaxiWhistle predictor
	...
	function TWP:AnnouncePrediction()
		...
		if self:IsOnValidMap() then
			...
			if C_Item.GetItemCount(TAXI_WHISTLE_ITEM_ID)==0 then print("Actually, you don't HAVE the whistle!") end
		end
	...
	function MMH.FrameOnEvent(frame,event,arg1,arg2,arg3)
		if event=="UNIT_SPELLCAST_SUCCEEDED" then
			...
			elseif ZGV.DEV and MMH.last_mole and MMH.last_mole.time and time()-MMH.last_mole.time<30 then
				local spellname = getSpellName(arg3)
