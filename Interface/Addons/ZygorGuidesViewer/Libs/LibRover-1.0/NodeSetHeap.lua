...
local function HeapSwimMinUp(num)
	...
	while (num > 1 and heap[half].cost>heap[num].cost) do
...
local function SinkNodeDown()
	...
	local j
	while( (k*2) <= size) do
		j = k*2; -- J is always a left leaf since it is even.
		if (j<size and heap[j].cost>heap[j+1].cost) then j = j+1 end  -- j<size so there is a right leaf. compare left and right leafs. Smaller value is now j
		if heap[k].cost<=heap[j].cost then break end -- Compare node to the smaller of it's children. If it is smaller than swap them and repeat
