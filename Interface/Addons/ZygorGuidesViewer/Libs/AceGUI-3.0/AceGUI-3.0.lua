...
-- @name AceGUI-3.0
-- @release $Id$
local ACEGUI_MAJOR, ACEGUI_MINOR = "AceGUI-3.0-Z", 41
	...
	function(content, children)
		...
		for i = 1, #children do
			...
			if i == 1 then
				...
				frame:SetPoint("TOPLEFT", content, 0, (child.marginTop or 0))
				...
				if usedwidth == 0 or ((framewidth) + usedwidth > width) or child.width == "fill" then
					...
					rowstart:SetPoint("TOPLEFT", content, "TOPLEFT", 0, -(height + (rowoffset - rowstartoffset) + 3)+(child.marginTop or 0))
					height = height + rowheight + 3 + (child.marginTop or 0)
...
AceGUI:RegisterLayout("FlowTop", --sinus@zygor: keep it for Profiles
	function(content, children)
		if layoutrecursionblock then return end
		--used height so far
		local height = 0
		...
		for i = 1, #children do
			...
			if i == 1 then
				...
				-- put the control on the current row, adding it to the width and checking if the height needs to be increased
					...
					rowoffset = math_max(rowoffset, frameoffset)
					rowheight = math_max(rowheight, rowoffset + (frameheight / 2))
					frame:SetPoint("TOPLEFT", children[i-1].frame, "TOPRIGHT", 0, (child.marginTop or 0))
					usedwidth = framewidth + usedwidth
				end
			end
		...
		--anchor the last row, if its full height needs a special case since  its height has just been changed by the anchor
		if isfullheight then
			rowstart:SetPoint("TOPLEFT", content, "TOPLEFT", 0, 0)
		elseif rowstart then
			rowstart:SetPoint("TOPLEFT", content, "TOPLEFT", 0, -(height + (rowoffset - rowstartoffset) + 3))
		end
