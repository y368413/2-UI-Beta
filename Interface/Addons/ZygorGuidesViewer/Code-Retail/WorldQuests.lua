...
local faction_data = {
	...
	[2391] = {quest=-1, exp="bfa", engname="Rustbolt Resistance", filepath="INV_Faction_Rustbolt", zone=1462},			-- Rustbolt Resistance, no emissary, match by zone
	...
	[2465] = {quest=-1, exp="sha", engname="The Wild Hunt", filepath="inv_misc_sigil_ardenweald01"},			-- The Wild Hunt            
	[2410] = {quest=-1, exp="sha", engname="The Undying Army", filepath="inv_misc_sigil_maldraxxus01"},		-- The Undying Army            
	[2407] = {quest=-1, exp="sha", engname="The Ascended", filepath="inv_misc_sigil_bastion01"},			-- The Ascended
	[2413] = {quest=-1, exp="sha", engname="Court of Harvesters", filepath="inv_misc_sigil_revendreth01"},		-- Court of Harvesters

	[2503] = {quest=-1, exp="dra", engname="Maruuk Centaur", filepath="ui_majorfaction_centaur"},			-- Maruuk Centaur
	[2507] = {quest=-1, exp="dra", engname="Dragonscale Expedition", filepath="ui_majorfaction_expedition"},		-- Dragonscale Expedition
	[2510] = {quest=-1, exp="dra", engname="Valdrakken Accord", filepath="ui_majorfaction_valdrakken"},		-- Valdrakken Accord
	[2511] = {quest=-1, exp="dra", engname="Iskaara Tuskarr", filepath="ui_majorfaction_tuskarr"},			-- Iskaara Tuskarr
	...
	[2594] = {quest=-1, exp="tww", engname="The Assembly of the Deeps", filepath="ui_majorfactions_candle"}, 
	[2570] = {quest=-1, exp="tww", engname="Hallowfall Arathi", filepath="ui_majorfactions_flame"}, 
	[2590] = {quest=-1, exp="tww", engname="Council of Dornogal", filepath="ui_majorfactions_storm"},
	[2600] = {quest=-1, exp="tww", engname="The Severed Threads", filepath="ui_majorfactions_web"},
}
...
for i,faction in pairs(faction_data) do
	...
	faction.value = i
	faction.texture = (faction.filepath and "|TInterface\\Icons\\"..faction.filepath..":14|t " or "")
...
local function guide_exists(questID) 
	...
	if not guides then 
		local guide,labelstep = find_world_quest_step(questID,WorldQuests.current_mapid)
...
function WorldQuests:ShowTooltipQuest(row)
	...
	if ( questItem.worldQuest ) then
		...
		if info then 
			...
			local factionName = factionID and (C_Reputation.GetFactionDataByID(factionID) or _null).name
			if (factionName) then
