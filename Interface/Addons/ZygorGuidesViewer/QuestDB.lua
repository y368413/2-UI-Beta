...
function QuestDB:SetQuestForButton(questIdent)
	...
	if QuestMapFrame and QuestMapFrame:IsVisible() then
		...
		ZGV.QuestDB.SearchIcon:SetPoint("TOPRIGHT",QuestMapFrame.DetailsFrame,"TOPRIGHT",-10,-10)
	...
	elseif QuestLogDetailScrollChildFrame and QuestLogDetailScrollChildFrame:IsVisible() then
		ZGV.QuestDB.SearchIcon:ClearAllPoints()
		ZGV.QuestDB.SearchIcon:SetParent(QuestLogDetailScrollChildFrame)
		ZGV.QuestDB.SearchIcon:SetPoint("TOPRIGHT",QuestLogDetailScrollChildFrame,"TOPRIGHT",-5,-5)
	end
end
...
tinsert(ZGV.startups,{"Quest DB",function(self)
	...
	if ZGV.IsRetail then
		hooksecurefunc("QuestMapLogTitleButton_OnClick",function(self,button) ZGV.QuestDB:SetQuestForButton(self.questID,"QuestMapLogFrame") end)
	...
	if ZGV.IsClassicCATA then
		hooksecurefunc("QuestLog_SetSelection",function(questIndex) C_Timer.After(0,function() ZGV.QuestDB:SetQuestForButton(questIndex) end) end)
	end
