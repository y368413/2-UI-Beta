...
function widget:Initialise()
	...
	if not self.Floating then 
		self.UnregisteredChatEvent = true
...
function widget:InitialisePopup()
	...
	local text = CHAIN(self.popup:CreateFontString())
		...
		:SetTextColor(1,1,1,1)
		:SetJustifyH("LEFT")
		:SetWidth((self.popupwidth or ZGV.Widgets.Fader:GetWidth()))
		:SetHeight(20)
	.__END
	text:SetPoint("TOPLEFT",self.popuplines[lines_per_column],"BOTTOMLEFT",0,-20)
	if UnitLevel('player') == self.maxlevel then text:SetText("max Level") end

	self.popuplines.maxlevel = text

	local text_played = CHAIN(self.popup:CreateFontString())
		:SetFont(FONT,12) 
		:SetTextColor(1,1,1,1)
		:SetJustifyH("LEFT")
		:SetWidth((self.popupwidth or ZGV.Widgets.Fader:GetWidth()))
		:SetHeight(20)
	.__END
	text_played:SetPoint("TOPLEFT",self.popuplines[lines_per_column],"BOTTOMLEFT",0,0)

	self.popuplines.played = text_played
...
function widget:OnPopup()
	...
	played_text = ZGV.F.GetTimeFromSeconds(ZGV.db.char.timeperlevel.totalplayed,use_short)
	self.popuplines.played:SetText(L["widget_timeplayed_played"]:format(played_text))
end
...
function widget:OnEvent(event,total,level)
	...
	if pl < self.maxlevel then
		...
		ZGV.db.char.timeperlevel.totalplayed = total
	else
	...
	if self.UnregisteredChatEvent then
		ChatFrame1:RegisterEvent("TIME_PLAYED_MSG")
		self.UnregisteredChatEvent = false
	end
