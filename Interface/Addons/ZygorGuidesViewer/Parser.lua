...
--if not ZGV.ClassToNumber.DRUID then ZGV.ClassToNumber.DRUID=11 end -- fix for 2024-07-09 blizzard bug: druid MIA
...
local function ParseRanges(str,askeys)
	local chunks = {strsplit(",",str)}
	local objects = {}
	for _,object in ipairs(chunks) do
		if tonumber(object) then
			if askeys then
				objects[tonumber(object)] = true
			else
				table.insert(objects,tonumber(object))
			end
		else
			local from,to = object:match("(%d+)-(%d+)")
			for i=tonumber(from),tonumber(to) do
				if askeys then
					objects[tonumber(i)] = true
				else
					table.insert(objects,tonumber(i))
				end
			end
		end
	end
...
Parser.ParseRanges = ParseRanges
...
local ConditionEnv = {
	...
	races={
		"LightforgedDraenei","LFDraenei", "VoidElf", "DarkIronDwarf","DIDwarf", "HighmountainTauren","HMTauren", "ZandalariTroll","ZTroll", "Nightborne", "MagharOrc","MHOrc", "KulTiran","KTHuman", "Vulpera", "MGnome","Mechagnome", "Earthen","EarthenDwarf",  -- allied races first, because they're searched-and-replaced in conditions; wouldn't want to replace Troll in ZandalariTroll
		"NightElf","Dwarf","Human","Gnome","Draenei","Worgen",
		"Orc","Troll","Scourge","Tauren","BloodElf","Goblin",
		"Pandaren","Dracthyr",
		"Undead"
	},
	race_aliases = {undead='scourge', lfdraenei='lightforgeddraenei', hmtauren='highmountaintauren', ztroll='zandalaritroll', didwarf='darkirondwarf', mhorc='magharorc', kthuman='kultiran', mgnome='mechagnome', earthen='earthendwarf', },
	classes={},
	...
	hasbuff = function(query,count)
		if tonumber(query) and C_UnitAuras then -- wotlk doesn't have C_UnitAuras, so we need to fall back to looping
			...
			for i=1,50 do
				...
				if auraData and ((texture and auraData.icon==texture) or (spell and auraData.spellId==spell)) then
					if not count or auraData.applications>=count then return true end
					stacks = stacks + auraData.applications
					if stacks>=count then return true end
				end
				local auraData = GetAuraDataByIndex("player",i,"HARMFUL")
				if auraData and ((texture and auraData.icon==texture) or (spell and auraData.spellId==spell)) then
					if not count or auraData.applications>=count then return true end
					stacks = stacks + auraData.applications
					if stacks>=count then return true end
	...
	itemcount = function(...)
		...
		local count = select("#", ...)
		for i = 1, count do
			total = total + C_Item.GetItemCount(select(i, ...))
		end
		return total
	end,
	...
	inscenario = function(scenarioID,scenarioName) 
		...
		if scenarioID and scenarioID~="" and C_ScenarioInfo and C_ScenarioInfo.GetScenarioInfo then 
			local info = C_ScenarioInfo.GetScenarioInfo()
			return info.scenarioID==scenarioID and not info.isComplete
		end
		if scenarioName and scenarioName~="" and C_Scenario and C_Scenario.GetInfo then
			local name, currentStage, numStages,_,_,_,_,_,_,_,_,_,currentScenarioID = C_Scenario.GetInfo()
	...
	scenariogoal = function(...)
		...
		for criteriaIndex=1,numCriteria do
			...
			for i = 1, count do
				local id = select(i, ...)
				if criteriaInfo.criteriaID==id then return true end
			end
		end
		return false
	end,
	...
	outvehicle = function()
		return not UnitInVehicle("player") and not UnitOnTaxi("player")
	end,
	...
	--hardcorebuffs = {364001,364265,364268,364282,364487}, -- buffs are only present if selfmade. Why...
	hardcore = function()
		if type(ZGV.db.char.fakehardcore)~="nil" then return ZGV.db.char.fakehardcore end
		--for _,spell in ipairs(Parser.ConditionEnv.hardcorebuffs) do
		--	if C_UnitAuras.GetPlayerAuraBySpellID(spell) then return true end
		--end
		--return false
		return ZGV.IsClassicHardcore
	end,
	selfmade = function()
		if C_UnitAuras.GetPlayerAuraBySpellID(431567) then return true end
		return false
	end,
	pandariaremix = function() 
		return Parser.ConditionEnv.hasbuff(424143)
	end,
	widgetactive = function(widgetID,widgetSetID)
		local knownSets = {
			TopCenter = C_UIWidgetManager.GetTopCenterWidgetSetID(),
			PowerBar = C_UIWidgetManager.GetPowerBarWidgetSetID(),
			ObjectiveTracker = C_UIWidgetManager.GetObjectiveTrackerWidgetSetID(),
			BelowMinimap = C_UIWidgetManager.GetBelowMinimapWidgetSetID(),
			widgetSetID = widgetSetID
		}

		for setName,setID in pairs(knownSets) do
			local widgetset = C_UIWidgetManager.GetAllWidgetsBySetID(setID)
			for _, wData in ipairs(widgetset) do
	...
	machinewave = function() 
		local typeinfo = UIWidgetManager:GetWidgetTypeInfo(8)
		local widgetData = typeinfo.visInfoDataFunction(5573)
		if not widgetData then return 0 end

		local wave = (widgetData.text or ""):match("(%d+)")
...
function Parser:ParseEntry(guide,fully_parse,lastparsed)
	...
	local function apply_colours(text)
		text = text:gsub("_(.-)_","|cffffee88%1|r")
		text = text:gsub("{}","|r")

		text = text:gsub("{o}","|cffff7d40")
		text = text:gsub("{g}","|cff00ff00")
		text = text:gsub("{r}","|cffff0000")
		text = text:gsub("{b}","|cff3dbffb")
		text = text:gsub("{y}","|cffffcc00")
		text = text:gsub("{p}","|cffcf3dbf")
		return text
	end
			...
			for chunk in line:gmatch("%s*(.-)%s*|+") do if #chunk>0 then
				...
				if step and not cmd_parsed then
					...
					--]]
						...
						elseif cmd=="blizztooltip" then
							goal.blizztooltip = true

						elseif cmd=="autoacceptany" then
							goal.autoacceptany = ParseRanges(params,"askeys")
						elseif cmd=="autoturninany" then
							goal.autoturninany = ParseRanges(params,"askeys")

						elseif cmd=="mapicon" then -- override icon for |mapmarker
							goal.mapicon = params

						elseif cmd=="noautoaccept" then
							goal.noautoaccept = true

						elseif cmd=="noautogossip" then
							goal.noautogossip = true

						elseif cmd=="usebank" then
							goal.usebank = true
			...
			macroguide.body=macroguide.body:gsub(".-\n",function(par)
				...
				return par:gsub("##([0-9]*)",function(id)
					if not isItem then
						...
						id=spellData.name -- That's an easy part, really
					else
...
-- check labels within guide. cross guide labels are checked in zgv at the end of _StartupThread, once all guides are parsed
		...
		for si,step in ipairs(guide.steps) do
			for g,goal in ipairs(step.goals) do
				if goal.next and not goal.next:find("\\") and goal.next~="+1" then
					local jump = goal.next
					local sign=jump:sub(1,1)  if sign=="+" or sign=="-" then jump=jump:sub(2) end
					if not steplabels[jump] then
						return nil,"Next label missing: "..jump,"-",si,""
					end
				end
			end
		end
	...
	if not (guide.headerdata.poiloader or guide.headerdata.orientation or guide.headerdata.noendstep) then
...
if LocalizedClassList then
	ParseHeader_classes = LocalizedClassList()
else
...
function Parser:ParseHeaderError(msg,param,value)
	print("ParseHeaderError",msg,param,value)
	return
end
...
function Parser:ParseHeader(guide)
	...
	for cmd,params in pairs(guide.headerdata) do
		...
		if cmd=="pet" then
			...
			if not speciesId then return parseerror(guide.title,"bad pet number",cmd,params) end
			...
			if not fun then return parseerror(guide.title,err,cmd,params) end
		...
		elseif cmd=="condition_suggested" or cmd=="condition_valid" or cmd=="condition_invalid" or cmd=="condition_end" or cmd=="condition_visible" or cmd=="condition_suggested_race" then
			...
			if not fun then return parseerror(guide.title,err,cmd,tostring(params)) end
		...
		elseif cmd=="mounts" then
			...
			if ZGV.IsRetail then
				local mountID = C_MountJournal.GetMountFromSpell(params[1])
				if mountID then
					local DisplayID = C_MountJournal.GetMountInfoExtraByID(mountID) 
					if DisplayID then
						guide.headerdata.model = guide.headerdata.model or {DisplayID}
					end
				end
			end
			guide.mounts=params
		...
		elseif cmd=="class" then
			...
			if guide.class and guide.class~=params then return parseerror(guide.title,"can't specify multiple classes for a guide, make it general",cmd,params) end
			if not ParseHeader_classes[params] then return parseerror(guide.title,"class "..params.." is unknown to science",cmd,params) end
			--[[ unused?
		...
		elseif cmd=="spec" then
			...
			if not guide.headerdata.class then return parseerror(guide.title,"spec specified without a class, what do you mean?",cmd,params) end
			local spec=ZGV.SpecToNumber[guide.headerdata.class][params]
			if not spec then return parseerror(guide.title,"spec "..params.." unknown for class ".. tonumber(guide.class),cmd,params) end
		...
		elseif cmd=="model" and not (guide.headerdata.pet or guide.headerdata.mounts) then
			-- guide-wide
