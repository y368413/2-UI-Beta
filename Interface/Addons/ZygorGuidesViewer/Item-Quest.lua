...
function QuestItem:ShowQuestRewardGlow(index,reason)
	...
	if not self.GlowFrame then
		...
		.__END
		self.GlowFrame.texcoin = CHAIN(self.GlowFrame:CreateTexture())
			:SetSize(15,15)
		...
		ZGV.IconSets.ItemUpgrades.ARROW:AssignToTexture(self.GlowFrame.tex)
	end

	self.GlowFrame:SetPoint("LEFT",b,"LEFT",39,1)
	if reason=="money" then
		self.GlowFrame.tex:Hide()
		self.GlowFrame.texcoin:Show()
	elseif reason=="upgrade" then
		self.GlowFrame.tex:Show()
		self.GlowFrame.texcoin:Hide()
	else
		self.GlowFrame.tex:Hide()
		self.GlowFrame.texcoin:Hide()
	end
...
function QuestItem:GetQuestRewardIndex()
	...
	for index=1, totalrewards do 
		...
		local is_upgrade, slot, change, score, comment = ItemScore.Upgrades:IsUpgrade(itemlink,{keeptype=true,rawvalues=true})
		ZGV:Debug("%s %s %s upgrade %s because %s",index,itemlink,is_upgrade and "is" or "not", change, comment)
		if is_upgrade then 
			if change > best_upgrade_change then
				best_upgrade_change = change
				best_upgrade_index = index
				ZGV:Debug(index.." is new best by score change")
			end
		end
		...
		if sellPrice > best_sell_value then
			...
			ZGV:Debug(index.." is new best by selling price")
		end
	end

	if best_upgrade_index > 0 then
		return best_upgrade_index,"upgrade"
	elseif best_sell_index > 0 then
		return best_sell_index,"money"
	end
...
local function CreateQuestPopup()
	...
	F.OnAccept = function(self)
		...
		--equip it!
		C_Item.EquipItemByName(self.itemid)
	end
	...
	F.equippingQuestItemSettings = function(self)
		...
		local function OnOpen() -- If the guide is completed then we know all the needed trainings
			...
			if not questid or self.questid~=questid or self.itemid~=questitemid then
				...
				ZGV.NotificationCenterOld:RemoveButton(self:GetName())
			end
		end
	...
	F.equippingNormalItemSettings = function(self)
		...
		local function OnOpen() -- If the guide is completed then we know all the needed trainings
			if not ZGV.db.profile.questitemreplaced or C_Item.GetItemCount(ZGV.db.profile.questitemreplaced) < 1 then
				if ZGV.Testing.DontHidePopups then return end
				ZGV.NotificationCenterOld:RemoveButton(self:GetName())
			end
		end
...
function QuestItem:ReEquipNormalItem()
	...
	if normItemid == 0 or C_Item.GetItemCount(normItemid) < 1 then --They had no item when they started so don't do anything. or they got rid of the item they were using.
