...
if ZGV.IsRetail then
	virtual_continents = {
		...
		[C_KHAZALGAR] = {
			-- old conts just copied for lack of better idea
			[C_EASTERN] =   {  2.0, -1.4,  1.0 },
			[C_KALIMDOR] =  { -2.0, -1.4,  1.0 },
			[C_BROKENIS] =  {  0.5, -1.3,  1.0 },
			[C_KULTIRAS] =  {  1.0, -1.0,  1.0 },
...
function MapCoords:SetupMapButtons()
	...
	if MapCoords.MapButtonFrame then return end
	...
	MapCoords.MapButtonFrame = ZGV.ChainCall(ZGV.CreateFrameWithBG("FRAME","ZygorWorldMapMenu",WorldMapFrame.BorderFrame))
		...
		:SetScript("OnUpdate",function(self)
			if (MapCoords.MapButton:IsDragging()) then  -- yes, it's the inside button that's dragging the parent.
	...
	MapCoords.MapButton = ZGV.ChainCall(CreateFrame("BUTTON",nil,MapCoords.MapButtonFrame,"ZygorSpecialButton_Template"))
		:ApplySkin()
		:SetSize(32,32)
		:SetPoint("TOPLEFT", MapCoords.MapButtonFrame, "TOPLEFT", 5, -5)
		:SetFrameStrata("HIGH")
		:SetFrameLevel(611)
		:SetScript("OnClick", function() MapCoords:ShowMapMenu() end)
	...
	MapCoords.menu = CreateFrame("FRAME",nil,MapCoords.MapButtonFrame,"UIDropDownForkTemplate")
end
...
function MapCoords:ShowMapMenu()
	local L=ZGV.L

	if DropDownForkList1 and DropDownForkList1:IsShown() and DropDownForkList1.dropdown==MapCoords.menu then
		CloseDropDownForks()
		return
	end
	...
	UIDropDownFork_SetAnchor(MapCoords.menu, 0, 0, "BOTTOMLEFT", MapCoords.MapButtonFrame, "BOTTOMRIGHT")
	EasyFork(menu,MapCoords.menu,nil,0,0,"MENU",false)
	UIDropDownFork_SetWidth(MapCoords.menu, 300)
end
