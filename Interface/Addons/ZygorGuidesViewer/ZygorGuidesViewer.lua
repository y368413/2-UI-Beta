...
ZGV.StandAlone = C_AddOns.GetAddOnMetadata(addonName,"X-Standalone")
...
ZGV.registeredguides = {}  ---@type Guide[]
ZGV.registered_guide_types = {} ---@type { [GuideType]: Guide[] }
ZGV.registeredmapspotsets = {}
ZGV.guidesets = {}
ZGV.registered_groups = { groups={},guides={}}
ZGV.RegisteredGuidesTitles = {}
ZGV.RegisteredGuidesByIdent = {}
...
function ZGV:OnEnable()  --PLAYER_LOGIN
	...
	self:AddEventHandler("UPDATE_MOUSEOVER_UNIT",function() ZGV.HandleRaidmarker("mouseover") end)
	self:AddEventHandler("UNIT_TARGET",function() ZGV.HandleRaidmarker("target") end)
...
local function _StartupThread()
	...
	--[[
	-- startup gearfinder
	if ZGV.ItemScore.Upgrades:IsEnabled() then
...
function ZGV:SetGuide(name,step,hack,silent,source) --hack used for testing
	...
	ZGV.Tabs:UpdateCurrentTab("guidechange",step)
end

---@param typ GuideType
function ZGV:FindSuggestedGuides(typ)
...
function ZGV:ForeachInGuidesAsync2(guides,callback,progress,done)
...
function ZGV:FindSuggestedGuidesAsync(typ)
	local suggested={} ---@type { [GuideType]:Guide[]}
	local suggroups={} ---@type { [string]:Guide[]}
	local scope = typ and self.registered_guide_types[typ] or self.registeredguides
...
function ZGV:FindSuggestedGuidesAsync2(typ)
	local suggested={} ---@type { [GuideType]:Guide[]}
	local suggroups={} ---@type { [string]:Guide[]}
	local scope = typ and self.registered_guide_types[typ] or self.registeredguides
...
function ZGV:FocusStep(num,forcefocus)
	...
	ZGV.Tabs:UpdateCurrentTab("focus",num)
...
function ZGV:FindEvent(eventName)
	...
	local now = ZGV.F.GetSecondsFromTime(dateobject)
	...
	for event=1, numEvents do
		...
		if eventdata and eventdata.calendarType=="HOLIDAY" and --We don't care about any other events
		   (eventName==ZGV.WorldEventIDs[eventdata.eventID] or eventName==eventdata.title) then --Does the ring fit?
			local startTime = ZGV.F.GetSecondsFromTime(eventdata.startTime)
			local endTime = ZGV.F.GetSecondsFromTime(eventdata.endTime)
...
function ZGV:MagicRaidMarker(marker)
	...
	if UnitCanAttack("player","target") then
		...
		if icon~=raidmarker then
			...
			ZGV.UsedRaidMarkers[raidmarker] = true
		end
	else
...
function ZGV:SuggestGuideFromBlizzardIcon(object)
	...
	if ZGV.CurrentGuide==step_guide then
		...
		ZGV:Debug("&_SUB &worldquests loading %s",poiID)
		ZGV.Tabs:LoadGuideToTab(step_guide,step_label,"areapoiid")
	end
...
function ZGV:StoreTelemetryBasics()
	local _
...
function ZGV.HandleRaidmarker(unit)
	...
	if GetRaidTargetIndex(unit)~=marker then
		...
		ZGV.UsedRaidMarkers[marker] = true
	end
end
...
function ZGV.ClearRaidmarker()
	...
	if ZGV.UsedRaidMarkers[1] then ZGV.UsedRaidMarkers[1]=nil SetRaidTarget("player",1) end
	if ZGV.UsedRaidMarkers[6] then ZGV.UsedRaidMarkers[6]=nil SetRaidTarget("player",6) end
	if ZGV.UsedRaidMarkers[7] then ZGV.UsedRaidMarkers[7]=nil SetRaidTarget("player",7) end
	if ZGV.UsedRaidMarkers[8] then ZGV.UsedRaidMarkers[8]=nil SetRaidTarget("player",8) end
