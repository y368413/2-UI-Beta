...
function ZGV:Options_DefineOptionTables()
	...
	do
		...
		AddOptionSep()
		--AddOption('hidetracker',{ type = 'toggle', set = function(i,v) Setter_Simple(i,v)  ZGV.Replacements:UpdateVisibility("force")  end, _default=false, width=200,})
		AddOption('hideexptracker',{ type = 'toggle', set = function(i,v) Setter_Simple(i,v)  ZGV.Replacements:UpdateVisibility("force")  end, _default=false, })
		AddOptionSep()
		...
		AddOption('zgvhideindungeons',{ type = 'toggle', name="Hide viewer when entering dungeons and raids", width = "full", _default = false })
		AddOption('showafterdungeons',{ type = 'toggle', name="Show viewer again when leaving", indent = 20, width = "full", _default = true, disabled = function() return not self.db.profile.zgvhideindungeons end })
	...
	do
		...
		AddOption('autogossip',{ type = 'toggle', width = "full", _default = not ZGV.DEV })
		AddOptionSep()
	...
	do
		...
		AddOptionSpace()
		AddOption('arrowoutline',     { type = 'toggle', width = "double", _default=false,  set=function(i,v) Setter_Simple(i,v) ZGV.Pointer:SetFontSize(ZGV.db.profile.arrowfontsize) end, })
		...
		AddOptionSpace()
			...
			AddOption('arrowskinselect',{
				...
				values = function()
					local t={}
					for id,skin in pairs(self.Pointer.ArrowSkins) do
						local myOrder = skin.order or id
						t[myOrder]=skin.name
					end
					return t
				end,
		...
		AddSubgroup("travelmode_custom",{
			...
			AddOption('pathfinding_comfort',     { 
				...
				get = function()
					return ZGV.db.profile.pathfinding_comfort==1
				end,
				indent=20,
			})
	...
	do
		...
		AddOption('autogear',{ type = 'toggle',width="full", _default=true, set = function(i,v) 
			...
			ZGV.ItemScore.Upgrades:AttachBagButton() 
			ZGV.ItemScore.Upgrades:ScoreEquippedItems()
			ZGV.ItemScore.Upgrades:RefreshBags()
		end})
		...
		AddOptionSpace()
		AddOption('markupgrades',{ type = 'toggle',width="full", _default=true, set = function(i,v) Setter_Simple(i,v) ZGV.ItemScore.Upgrades:RefreshBags() end, disabled = function() return not self.db.profile.autogear end,})
		...
		AddOption('upgradeslot',{
			...
			set = function(i,v)
				...
				if v>0 then
					ZGV.db.profile.upgradeslot = slot_ids[v]
					if slot_ids[v]==INVSLOT_FINGER1 then print("ring") ZGV.db.profile.upgradeslot2=INVSLOT_FINGER2 end
					if slot_ids[v]==INVSLOT_TRINKET1 then print("trink") ZGV.db.profile.upgradeslot2=INVSLOT_TRINKET2 end
				else
			...
			get = function(i,v)
				for i,slot in ipairs(slot_ids) do
					if slot==ZGV.db.profile.upgradeslot then return i end
				end
				return 0
			end,				
			_default = false,
		...
		AddOption('autogearpopup',{ type = 'toggle', width="double", _default=true})
		...
		ZGV.ItemScore.modifierkeys = modifierkeys
		...
		ZGV.ItemScore.mousekeys = mousekeys
		...
		AddOption('',{ type = "description", name = L["opt_gear_sources_dungeons"]:format(), font=ZGV.font_dialog_gray, width="full", disabled=function() return not self.db.profile.autogear end })
			...
			if ZGV.IsRetail then
				...
				AddOption('gear_8',{ name="Mythic+",  type='toggle', width="100", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, })
		...
		AddOption('',{ type = "description", name = L["opt_gear_sources_raids"]:format(), font=ZGV.font_dialog_gray, width="full" })
			if ZGV.IsRetail then 
				...
				AddOption('gear_15',{ name=PLAYER_DIFFICULTY2, type='toggle', width="100", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.db.profile.gear_5=v ZGV.db.profile.gear_6=v ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, }) 
				AddOption('gear_16',{ name=PLAYER_DIFFICULTY6, type='toggle', width="100", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.db.profile.gear_7=v ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, }) 
			end
			if ZGV.IsClassicWOTLK or ZGV.IsClassicCATA then
				AddOption('gear_3',{ name=RAID_DIFFICULTY1, type='toggle', width="100", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, })
				...
				AddOption('gear_4',{ name=RAID_DIFFICULTY2, type='toggle', width="100", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, })
				--AddOption('gear_6',{ name=RAID_DIFFICULTY4, type='toggle', width="150", _default=false, set = function(i,v) Setter_Simple(i,v) ZGV.ItemScore.GearFinder:ClearResults() end, disabled=function() return not self.db.profile.autogear end, }) 
			end
		...
		if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK then
			for specnum,specdata in pairs(ZGV.ItemScore.Defaults[playerclass]) do
				AddOption('',{
					...
					font=ZGV.font_dialog,
					hidden=function() 
						local visibility = tonumber(ZGV.db.char.gear_active_build)==specnum and tonumber(ZGV.db.char.gear_selected_build)==specnum
	...
	do
		...
		AddOption('',{ type = 'description', name=L['opt_nc_types'], hidden=function() return not self.db.profile.nc_showall end, font=ZGV.font_dialog})
			AddOptionSep()
			...
			--AddOption('n_popup_gear_alt',{ type = 'toggle', width = "full", _default = true,disabled = function() return not (self.db.profile.nc_enable and self.db.profile.n_popup_gear) end, indent=20, hidden=not ZGV.DEV})
		...
		if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK then
			...
			AddOptionSep()
			AddOption('',{ type = 'description', name=L['opt_nc_skillstitle'], font=ZGV.font_dialog, indent=20, hidden=function() return not self.db.profile.nc_showall end, disabled = function() return not (self.db.profile.nc_enable and self.db.profile.nc_skills) end,})
				AddOptionSep()
				AddOption('',{type="description", name=L['opt_group_notify'], font=ZGV.font_dialog, indent=20, hidden=function() return not self.db.profile.nc_showall end, disabled = function() return not (self.db.profile.nc_enable and self.db.profile.nc_skills) end,})
				AddOption('nc_skills_optional',{ type = 'toggle', width = "single", indent=20, set = function(i,v) Setter_Simple(i,v) ZGV.Skills:RefreshSkillPopup() end, _default = true, hidden=function() return not self.db.profile.nc_showall end, disabled=function() return not (self.db.profile.nc_enable and self.db.profile.nc_skills) end })
				AddOption('nc_skills_future',{ type = 'toggle', width = "single", indent=20,   set = function(i,v) Setter_Simple(i,v) ZGV.Skills:RefreshSkillPopup() end, _default = true, hidden=function() return not self.db.profile.nc_showall end, disabled=function() return not (self.db.profile.nc_enable and self.db.profile.nc_skills) end })
				AddOptionSep()
				AddOption('',{type="description", name=L['opt_nc_skillstriggertitle'], hidden=function() return not self.db.profile.nc_showall end, font=ZGV.font_dialog })
	...
	do
		...
		AddOption('mouseovermarkers',{ type = 'toggle', width = "full", _default=true})
		...
		AddOption('sep00spam',{ type="header", name="Chat" })
			AddOption('spam_levelup',{
				...
				set = function(i,v)
					Setter_Simple(i,v)
					ZGV.db.profile.spam_levelup_emote = true
					ZGV:RefreshOptions()
				end,
				_default=true,
			})
	...
	--if ZGV.db.profile.debug then	---- OPTIONS: debug
		...
		do
			...
			do -- Fake Popups
				...
				AddOption('faketoast_1', { type = 'execute', width=160, 
					...
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("welcome","Welcome to Zygor Guides","This is a test welcome message",{cleartype=true,keeponclick=true}) 
					end
				})
				AddOption('faketoast_2', { type = 'execute', width=160, 
					name="Weekly Quests", 
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("weekly","Weekly Quests have reset on the server.","",{dontsave=true})
					end
				})
				AddOption('faketoast_2.5', { type = 'execute', width=160, 
					name="Daily Quests", 
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("daily","Daily Quests have reset on the server.","",{dontsave=true})
					end
				})
				AddOption('faketoast_3', { type = 'execute', width=160, 
					name="Events", 
					func=function(info,val) ZGV.NotificationCenter:AddEntry("event","The Noblegarden Event is in progress.","Click to see guides for it.",{folder="EVENTS\\Noblegarden"}) end
				})
				AddOption('faketoast_4', { type = 'execute', width=160, 
					name="Orientation Guide", 
					func=function(info,val) ZGV.NotificationCenter:AddEntry("orientation","Run Startup Orientation to improve guide directions","Click to run now",{cleartype=true}) end
				})
				AddOption('faketoast_5', { type = 'execute', width=160, 
					name="Guide Updates", 
					func=function(info,val) ZGV.NotificationCenter:AddEntry("guides","See new and updated guides","",{cleartype=true,keeponclick=true}) end
				})
				AddOption('faketoast_6', { type = 'execute', width=160, 
					name="Feature Updates", 
					func=function(info,val) ZGV.NotificationCenter:AddEntry("features","New in this update","",{cleartype=true,keeponclick=true}) end
				})
				...
				AddOption('fakepopup_6.5', { type = 'execute', width=185, 
					...
					func=function(info,val) ZGV.GuideFuncs:SuggestDungeonGuide(ZGV:GetGuideByTitle("Dungeon Guides\\Draenor Raids\\Highmaul - Normal/Heroic")) end
				})
				AddOptionSep()
				--[[
				...
				AddOption('faketoast_11', { type = 'execute', width=160, 
					...
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("pet","Baby Blizzard Bear","",{guide="PETSMOUNTS\\Battle Pets\\Beast Pets\\Baby Blizzard Bear"})
					end
				})
				AddOption('faketoast_12', { type = 'execute', width=160, 
					name="Find gear upgrades", 
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("gear","You have 2 new upgrades available","")
					end
				})
				AddOption('faketoast_13', { type = 'execute', width=160, 
					name="Cutscene", 
					func=function(info,val)
						ZGV.NotificationCenter:AddEntry("cutscene","A cutscene has been skipped.","")
					end
				})
			...
			do -- Fake goal status
				...
				AddOption('fakegoal_list',{
					...
					values = goal_status,
					set = function(i,v) 
						if not ZGV.CurrentStep or not ZGV.CurrentStep.goals[1] then return end
					...
					get = function() 
						if not ZGV.CurrentStep or not ZGV.CurrentStep.goals[1] then return 1 end
		...
		do
			...
			AddOption('sep00gossip',{ type="header", name="Gossip recorder" })
			AddOption('reportgossips',{
				name = "ReportGossips",
				desc = "",
				type = 'toggle',
				set = function(i,v)
					if v then 
						ZGV:Print("DEV: ReportGossips is running")
					else
						ZGV:Print("DEV: ReportGossips is disabled")
					end
					Setter_Simple(i,v)  
				end,
			})
