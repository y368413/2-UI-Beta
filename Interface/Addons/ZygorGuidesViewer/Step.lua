...
--- Step completes when:
--- - its logic_fun completes
--- - a completed |confirm line is found
--- - a completed |override line is found
--- - all, or any |or, or enough |or lines are complete
--- @return boolean complete, boolean possible, string ...
function Step:IsComplete()
	...
	if not self:AreRequirementsMet(nil,"strict") then return true,true,"req unmet" end

	if self.score then
		if ZGV.db.profile.ratings and ZGV.completionstreak<3 then
			return ZGV.db.char.scoredguides[self.parentGuide.title],true
		else
			return true,true,"score 2"
		end
	end
	...
	-- check for ORs and overrides
	local status
	for i,goal in ipairs(self.goals) do
		status = goal.status
		if status~="hidden" then
			if goal.override and status=="complete" then return true,true,"override complete",i end  ------------- COMPLETE
	...
	-- all hidden? die.
	for i,goal in ipairs(self.goals) do
		if goal.status~="hidden" and (goal.action or goal.tooltip) and not goal.temporary then allhidden=false end
	end
	if allhidden then return true,true,"all hidden" end
	...
	for i,goal in ipairs(self.goals) do
		...
		if status=="complete" or status=="incomplete" or status=="warning" then
			...
			if status~="complete" and not goal.optional and not goal.override  then  complete = false  end
	...
	return completeable and complete and not alloptional, steppossible, "normal comp"
end
...
function Step:GetJumpDestination(jump)
	...
	if type(jump)=="number" or jump:match("^%d+$") then
		...
		elseif jump:find("\\") then
			...
			return tag,guide
		elseif jump:match("id:") then
			local guide,tag = jump:match("id:(.*)::(.*)")
			if not tag then guide=jump:match("id:(.*)") end

			guide = ZGV.RegisteredGuidesByIdent[guide]
			tag = tonumber(tag) or tag or 1
...
function Step:CycleWaypoint(delta,nocycle,because)
	...
	local isValid = function(goal)  return goal and goal.x and not goal.action=="mapmarker" and not goal.force_noway and goal:IsVisible() and (not goal:IsCompleteable() or select(2,goal:IsComplete() --[[possible--]] ))  end

	local any_valid = false
	for i,goal in ipairs(self.goals) do  if isValid(goal) then any_valid=true break end  end
	...
	until not any_valid --[[ so all are good --]] or isValid(goal)
