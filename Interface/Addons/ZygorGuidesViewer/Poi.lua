	...
	if ZGV.db.char.activepoi then
		for _,pointset in pairs(Poi.Points) do
			for _,point in pairs(pointset) do
				if point.ident == ZGV.db.char.activepoi then
...
function Poi:LoadPoint(point,noswitch)
	...
	ZGV.db.char.activepoi = point.ident
...
local function EventHandler(self, event, ...)
	...
	or event=="CHAT_MSG_CURRENCY" then 
		...
		if ZGV:Throttler("refresh_all_points",0.1,refresh_all_points) then return end
		refresh_all_points()
	end
end
...
tinsert(ZGV.startups,{"POI hooks",function(self)
	ZGV:AddEventHandler("CRITERIA_UPDATE",EventHandler)
	ZGV:AddEventHandler("QUEST_LOG_UPDATE",EventHandler)
...
function DataProvider:RefreshAllData(force)
	...
	local ident = ZGV.db.char.activepoi
