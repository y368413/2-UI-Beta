...
function ZygorGuidesViewer_ArrowSkin_Mixin:CreateFrame()
	if not self.frame then
		self.frame = CreateFrame("BUTTON",nil,Pointer.ArrowFrameCtrl,self.options.template..",SecureHandlerStateTemplate")
		self.frame.skin_id = self.id
		self.frame.skin = self
		self.frame.options = self.options
		self.frame.icons = self.icons
...
function ZygorGuidesViewerFrame_Arrow_Mixin:SetupSprites()
	...
	self.special:SetTexture(self.icons.file)
	self.arrow.arr:SetTexture(opt.texture)
	if opt.texturespecular then self.arrow.arrspecular:SetTexture(opt.texturespecular) end
	...
	if opt.texturespecular then self.arrow.arrspecular.sprite_coords=self.arrow.arr.sprite_coords end
end
...
function ZygorGuidesViewerFrame_Arrow_Mixin:ShowText (title,dist,eta,status)
	...
	self.desc:SetText((disttxt and distcolor..disttxt.."|r" or "") ..  (etatxt or "") ..
