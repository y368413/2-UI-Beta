...
local function GetScenarioGoalData(scenariogoalid,count,stage)
	...
	for criteriaIndex=1,numCriteria do -- current stage only
		...
		if criteriaInfo and criteriaInfo.criteriaID==scenariogoalid then
			...
			if criteriaInfo.isWeightedProgress then -- weightedProgress go from 0 to 100, and quantity/totalQuantity are not reliable for them. quantityString is "50%", so parse progress out of it
				...
				if not criteriaInfo.isWeightedProgress and not criteriaInfo.isFormatted then
					quantityString = string.format("%d/%d %s", criteriaInfo.quantity, criteriaInfo.totalQuantity, criteriaString);
				end
			...
			return quantity,needed,needed-quantity,criteriaInfo.description,nil
		end
	end
	--return
...
local ParseRanges = ZGV.Parser.ParseRanges
...
GOALTYPES['talknpcs'] = {
	...
	onLibRoverResults = function(state, path, ext)
		...
		if last then
			local title = ZGV.L['stepgoal_talk to']:format(ZGV.Localizers:GetTranslatedNPC(tonumber(last.title),last.title) or "?")
			if last.w then
	...
	iscomplete = function(self)
		...
		if self.targets then
			for _,target in ipairs(self.targets) do
				if tonumber(target[2]) then
					got = got + C_Item.GetItemCount(target[2])
					if self.usebank then
						local inbank = ZGV.Inventory:CountBank(target[2])
						got = got + inbank
						bank = bank + inbank
			...
			if tonumber(self.targetid) then
				got = got + C_Item.GetItemCount(self.targetid)
				if self.usebank then
					local inbank = ZGV.Inventory:CountBank(self.targetid)
					got = got + inbank
					bank = bank + inbank
				end
			end
...
GOALTYPES['trash'] = {
	...
	iscomplete = function(self)
		if not self.targetid then return false,true end -- no known item... what the...
		if not self.count or self.count==0 then self.count=1 end  -- completable? has to have a count.
		local got = C_Item.GetItemCount(self.targetid)
...
GOALTYPES['goldcollect'] = {
	parse = function(self,params)
		...
		-- if only one target is given, set up targets with only that one
		if not self.targets then self.targets = {{self.target,self.targetid}} end
	end,
	...
	iscomplete = function(self)
		...
		for _,target in ipairs(self.targets) do
			got = got + C_Item.GetItemCount(target[2])
			local itemdata = target[3] or (ZGV.Gold.servertrends and ZGV.Gold.servertrends.items[self.targetid])
			local itemdemand = itemdata and (itemdata.sold or itemdata.q_md or (itemdata.q_lo + itemdata.q_hi)/2)
			if itemdemand then
				demand = demand + itemdemand
			end
			target[3] = itemdata
		end

		if not self.targets[1][3] then -- if we do not have trends for default item, never complete the farm
		...
		for i,goal in pairs(step.goals) do
			if goal.action == "goldcollect" then
				...
				for _,target in ipairs(goal.targets) do
					local numInBags = C_Item.GetItemCount(target[2]) or 0
					local _, _, itemRarity, _, _, _, _, _, _, _, itemSellPrice = ZGV:GetItemInfo(target[2])
					...
					elseif ZGV.Gold.servertrends and ZGV.Gold.servertrends.items[target[2]] and ZGV.Gold.servertrends.items[target[2]].p_lo then
						-- GetPrice only gets today's price, and there may be gaps. Check server trends first. ~~ Jeremiah
						totalvalue = totalvalue + ZGV.Gold.servertrends.items[target[2]].p_lo * numInBags
					else
						totalvalue = totalvalue + ZGV.Gold.Scan:GetPrice(target[2]) * numInBags
					end
				end
			end
		end
...
GOALTYPES['learnmount'] = {
	...
	gettext = function(self) return L["stepgoal_learnmount"]:format(COLOR_ITEM(getSpellName(self.spellid))) end,
}
...
GOALTYPES['learnpet'] = {
	...
	gettext = function(self) return L["stepgoal_learnpet"]:format(COLOR_ITEM(ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcId),self.pet))) end,
}
...
GOALTYPES['petlevel'] = {
	...
	gettext = function(self) return L["stepgoal_petlevel"]:format(COLOR_ITEM(ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcId),self.pet)),self.level) end,
}
...
GOALTYPES['learnspell'] = {
	...
	gettext = function(self) return L["stepgoal_learnspell"]:format(COLOR_ITEM(getSpellName(self.spellid))) end,
}
...
GOALTYPES['learnpetspell'] = {
	...
	gettext = function(self) return L["stepgoal_learnpetspell"]:format(COLOR_ITEM(getSpellName(self.spellid))) end,
}
...
GOALTYPES['abandon'] = {
	parse = function(self,params)
		self.quests = ParseRanges(params)
		self.questsKeyed = ParseRanges(params,"askeys")
		self.questid = self.quests[1]
	end,
	onclick = function(self)
		for i,questData in ipairs(ZGV.quests) do
			if self.questsKeyed[questData.id] then
				questData.useless=true
			end
		end
		ZGV:AbandonUselessQuests()
	end,
	iscomplete = function(self)
		return not ZGV.Parser.ConditionEnv.haveq(unpack(self.quests)),true
	end,
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		local title = (type(self.quest)=="string" and self.quest) or (type(self.quest)=="table" and self.quest.title) or Lretrydots(self)
		return L["stepgoal_abandon"]:format(COLOR_QUEST(L['questtitle']:format(title))) 
	end,
}
...
GOALTYPES['turnin'] = {
	...
	iscomplete = function(self)
		...
		return turned, turned or completed, 0, not turned and quest and quest.inlog and not quest.complete and #quest.goals>0
	end,
	onenter = function(self)
...
GOALTYPES['noquest'] = {
	...
	gettext = function(self)
		return (L["stepgoal_noquest"]):format(COLOR_NPC(ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npc)))
	end,
	click_to_complete = true,
}
...
GOALTYPES['scenariostart'] = {
	parse = function(self,params)
		self.scenario_name,self.scenario_id = ParseID(params)
		if self.scenario_name=="" then self.scenario_name=nil end
	end,
	iscomplete = function(self)
		return ZGV.Parser.ConditionEnv.inscenario(self.scenario_id,self.scenario_name),true
	end,
...
GOALTYPES['scenariogoal'] = {
	...
	iscomplete = function(self)
		...
		if errortext then return false,C_Scenario.IsInScenario(), nil,nil, errortext,not self.future and isbad end
		if needed>0 and self.count and self.count>needed then self.count=needed end
		local cur,nee=norm_nums(current,needed)
		return (needed>0 and remaining<=0) , needed>0 , cur,nee, "scenario goal OK",not self.future and isbad
	end,
	gettext = function(self)
...
GOALTYPES['useany'] = {
	parse = function(self,params)
		self.items = ParseRanges(params)

		-- precache names
		for _,item in ipairs(self.items) do ZGV:GetItemInfo(item) end
		self.defaultitem = self.items[1]
		self.defaultitemname = ZGV:GetItemInfo(self.defaultitem)

		self.itemuse=true
		self.updatescriptfun = function()
			if self.itemid then
				local count = C_Item.GetItemCount(self.itemid)
			...
			for _,itemid in ipairs(self.items) do
				...
				if count>0 then
					self.itemid = itemid
					self.item = ZGV:GetItemInfo(itemid)
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) return GenericText(brief,"use",COLOR_ITEM,remaining or self.count,self.item or self.defaultitemname,true,self.plural,complete and "_done" or "",self.the) end,
}
...
GOALTYPES['talk'] = {
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) return (brief and "%s" or L["stepgoal_talk to"..(complete and "_done" or "")]):format(COLOR_NPC(plural(ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npc),self.plural))) end,
}
	...
	parse = function(self,params)
		...
		if self.targets then
			for i,v in ipairs(self.targets) do
				self.gossipids[tonumber(v[2])] = true
			end
		end
		self.gossipids[tonumber(self.targetid)] = true

		self.target = nil
		self.targetid = nil
		self.targets = nil
	end,
	autoscript = function(self)
...
GOALTYPES['vendor'] = {
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) return (brief and "%s" or L["stepgoal_vendor"..(complete and "_done" or "")]):format(COLOR_NPC(plural(ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npc),self.plural))) end,
}
...
GOALTYPES['skillmax'] = {
	...
	gettext = function(self)
		...
		if spell and spell>0 then
			return L["stepgoal_skillmax2"]:format(COLOR_ITEM(getSpellName(spell)))
		else
...
GOALTYPES['create'] = {
	parse = function(self,params)
		...
		if spell then
			...
			self.spell = getSpellName(tonumber(self.spellid))
			...
			if ZGV.IsRetail then
				...
				if C_TradeSkillUI.GetRecipeQualityItemIDs then
					local variants = C_TradeSkillUI.GetRecipeQualityItemIDs(self.spellid)
					if variants then
	...
	iscomplete = function(self)
		...
		elseif self.count and self.recipedata then
			return C_Item.GetItemCount(self.recipedata.itemid or 0)>=self.count,true
		end
...
GOALTYPES['loadguide'] = {
	parse = function(self,params)
		...
		params = params:gsub("^%s*\"(.-)\"%s*$","%1") -- strip quotes

		self.params = params

		self.loadguidestep,self.loadguide = ZGV.StepProto:GetJumpDestination(params)
		...
		if self.loadguide then
			local guide = ZGV:GetGuideByTitle(self.loadguide)
			if guide then
				self.prefixthe = "the "
				self.guidetitle = guide.title_short
				if self.guidetitle:lower():sub(1,4)=="the " then self.prefixthe="" end -- avoid Load the "The Title" guide
			else
				self.loadguide = nil
				self.loadguidestep = nil
			end
		else
			self.guidetitle = ""
			self.prefixthe = ""
	...
	gettext = function(self) return L["stepgoal_loadguide"]:format(self.prefixthe,self.guidetitle) end,
}
...
GOALTYPES['equipped'] = {
	...
	iscomplete = function(self)
		if C_Item.GetItemCount(self.targetid or self.itemid)==0 then return false,false,0 end  -- not even in the bags
...
GOALTYPES['unequipped'] = {
	...
	iscomplete = function(self)
		if C_Item.GetItemCount(self.targetid or self.itemid)==0 then return true,true,0 end  -- not even in the bags
		for i,slot in pairs(invslots) do
			local slotid,_ = GetInventorySlotInfo(slot)
			if slotid then
				local id = GetInventoryItemID("player",slotid)
...
GOALTYPES['repcollect'] = {
	parse = function(self,params)
		self.params=params
		self.item,self.turnincount,self.turningain,self.faction,self.rep = params:match("^(.*),(.*),(.*),(.*),(.*)$")
		GOALTYPES['_item'].parse(self,self.item)

		local _,race = UnitRace("player")
		if race=="Human" then self.turningain=self.turningain*1.1 end -- 10% racial bonus

		self.rep=ZGV.Faction.StandingNums[self.rep]  or  error("Unknown rep standing "..self.rep)
		self.repfaction = ZGV:GetReputation(self.faction)
		self.repname = self.repfaction:GetStandingName(self.rep)

		if ZGV.BFL[self.faction] then self.faction=ZGV.BFL[self.faction] end
	end,
	iscomplete = function(self)
		if not self.repfaction then return false,false end

		if self.repfaction.standing>=self.rep then return true, true end -- we are already ther

		local repneeded = self.repfaction:CalcTotalTo(self.rep)
		if not repneeded or repneeded==0 then return false,false end -- something is wrong
		self.count = math.ceil(repneeded/tonumber(self.turningain)) * tonumber(self.turnincount)

		return GOALTYPES['get'].iscomplete(self)
	end,
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		if complete then 
			return L["stepgoal_collectrep_done"]:format(COLOR_ITEM(ZGV:GetItemInfo(self.targetid)),self.repname,self.faction)
		else
			local goalcountnow = C_Item.GetItemCount(self.targetid or 0)
			local remaining = self.count-goalcountnow
			if remaining<1 then remaining=self.count end
			local progtext = L["completion_goal"]:format(goalcountnow,self.count)			
			return L["stepgoal_collectrep"]:format(COLOR_COUNT(remaining),COLOR_ITEM(ZGV:GetItemInfo(self.targetid)),self.repname,self.faction),nil,progtext
		end
	end,
	help = "repcollect <item##12345>,<count per turnin>,<rep per turnin>,<faction>,<rep level> -- completes when user has farmed enough items to reach standing (string) with faction (string) via X turnins granting Y rep each. "
}

GOALTYPES['invehicle'] = {
	parse = function(self,params) end,
	iscomplete = function(self)
		return (UnitInVehicle and UnitInVehicle("player")) or UnitOnTaxi("player"),true
	end,
...
GOALTYPES['outvehicle'] = {
	parse = function(self,params) end,
	iscomplete = function(self)
		return not (UnitInVehicle and UnitInVehicle("player")) and not UnitOnTaxi("player"),true
	end,
...
GOALTYPES['havebuff'] = {
	parse = function(self,params)
		...
		self.spell,self.spellid = ParseID(objinfo)

		if not self.spellid then
			self.texture = objinfo
		end

	end,
	iscomplete = function(self)
		return ZGV.Parser.ConditionEnv.hasbuff(self.spellid or self.texture,self.count),true
	end,
	gettext = function(self)
		if self.spellid and not self.qspellname then self.qspellname = getSpellName(self.spellid) end
		return L["stepgoal_havebuff"]:format(COLOR_ITEM(self.qspellname or self.spell or self.texture))
	end,
	help = "havebuff spellname##spellID or texture:12345  -- completes when user has buff with given spellid or texture fileid. "
}
...
GOALTYPES['nobuff'] = {
	...
	gettext = function(self)
		if self.qspellid and not self.qspellname then self.qspellname = getSpellName(self.qspellid) end
		return L["stepgoal_nobuff"]:format(COLOR_ITEM(self.qspellname or self.buff))
	end,
	help = "nobuff spellname##spellID or texture:12345  -- completes when user does not have buff with given spellid or texture fileid. "
}
...
GOALTYPES['clicknpc'] = {
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) return GenericText(brief,self.action,COLOR_ITEM,remaining or self.count,self.npcid and ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npc),	true,self.plural,complete and "_done" or "") end,
}
...
GOALTYPES['petaction'] = {
	parse = function(self,params)
		if tonumber(params) then
			self.petaction = params
		else
			self.petaction,self.petactionid = ParseID(params)
		end
		self.params = params
	end,
...
GOALTYPES['achieve'] = {
	...
	iscomplete = function(self)
		...
		if self.achievesub then
			if (GetAchievementNumCriteria(self.achieveid) < self.achievesub) and not (GetAchievementCriteriaInfoByID(self.achieveid,self.achievesub)) then -- Causes errors when blizzard changes crap.
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief)
		...
		if self.achievesub then
			...
			progtext = L["completion_goal"]:format(("%.2f"):format(self.achievecompletenum) or "...",self.achievenumcrit or "...")
		end
		if complete then progtext=nil end
...
GOALTYPES['grind'] = {
	...
	gettext = function(self)
		local text = ""
		local available_w = floor(ZygorGuidesViewerFrame:GetWidth()) - 20

		if not ZGV.db.char.shortgrind then
			local display_w, display_h = 512,32
			if available_w<display_w then
				display_h = display_h*available_w/display_w
				display_w = available_w
			end

			--text = ("|T%s:%d:%d:0:-5|t"):format(ZGV.DIR.."\\Skins\\grindbar",display_h,display_w) .. "|n"
			text = text .. L["grind_popup_title"]
			text = text .. "|n" .. L["grind_popup_text"]

			local fonstrings = ZGV.ButtonSets.TitleButtons.INVISIBLE:GetFontString(16,16,0,-5)
			text = text .. "|n|cfffe6100Click to hide this message|r |T" .. fonstrings[1].."|t"
		end

		return text
	end,
	onclick = function(self)
		ZGV.db.char.shortgrind = not ZGV.db.char.shortgrind
...
GOALTYPES['killboss'] = {
	parse = function(self,params)
		GOALTYPES['_item'].parse(self,params)
		self.target = EJ_GetEncounterInfo(self.targetid)
	end,
	iscomplete = function(self)
		if not tonumber(self.targetid) then return false,false end
		local killed = C_EncounterJournal.IsEncounterComplete(self.targetid)
		return killed,true
	end,
...
GOALTYPES['debugevent'] = { -- use for debugging step/goal completion.
	parse = function(self,params,step)
		self.event = params
		table.insert(GOALTYPES.debugevent.events,self.event)

		self.autoscriptfun = function(self)
	...
	gettext = function(self)
		return self.event.." "..tostring(self.eventcompleted)
	end,
	onclick = function(self)
		self.eventcompleted = false
		ZGV:UpdateFrame()
	...
	iscomplete = function(self)
		return self.eventcompleted, true
	end
}
...
function Goal:IsComplete()
	...
	if self.questid and self.action~="abandon" then
...
function Goal:OnShow()
	local GOAL = GOALTYPES[self.action]
	if GOAL and GOAL.onshow then
		GOAL.onshow(self)
	end

	if self.eventfunc then
		-- we know that events exist, since eventfunc is only created when they do
		for _,event in ipairs(GOAL.events) do
			ZGV:AddEventHandler(event,self.eventfunc)
		end
	end
end

function Goal:OnHide()
	local GOAL = GOALTYPES[self.action]
	if GOAL and GOAL.onhide then
		GOAL.onhide(self)
	end

	if self.eventfunc then
		for _,event in ipairs(GOAL.events) do
			ZGV:RemoveEventHandler(event,self.eventfunc)
		end
	end
end
...
function Goal:OnVisited()
	...
	if self.action == "mapmarker" then return end -- do not cycle from map markers
...
function Goal:GetTooltip()
	...
	if self.blizztooltip then
		if self.achieveid then
			local link = GetAchievementLink(self.achieveid)
			return nil,link
		end
	end
...
function ZGV.FindPetActionInfo(goal)
	if tonumber(goal.petaction) then
		local name, texture, isToken, _, _, _, spellID, _, _= GetPetActionInfo(goal.petaction)
		return goal.petaction,name,isToken and _G[texture] or texture
	end

	for i=1,12 do
		local name, texture, isToken, _, _, _, spellID, _, _= GetPetActionInfo(i)
		if (goal.petactionid and goal.petactionid==spellID) or (name and name:find(goal.petaction)) then
			return i,name,isToken and _G[texture] or texture
		end
	end
end

function Goal:IsActionable()
	return (self.itemid and (C_Item.GetItemCount(self.itemid)>0 or not self.itemuse))
	    or (self.castspellid and IsUsableSpell(self.castspellid))
	    or (self.petaction and ZGV.FindPetActionInfo(self))
...
function Goal:AutoTranslate()
	...
	if self.targetid then
		...
		if self.action=="kill" then
			if ZygorGuidesNPCs then
				local target,tooltip=ZGV.Localizers:GetTranslatedNPC(tonumber(self.targetid),self.target)
				if target then
	...
	if self.targets then
		if self.action=="kill" then
			for i,target in ipairs(self.targets) do
				local id,name = target[2],target[1]
				local target,tooltip=ZGV.Localizers:GetTranslatedNPC(tonumber(id),name)
				if target then
	...
	if self.npcid then
		local npc,tooltip=ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npc)
		--if npc then
...
function Goal:GetText(showcompleteness,brief,showtotals,nocolor)
	...
	elseif self.action=='kill' then
		local mobname=ZGV.Localizers:GetTranslatedNPC(tonumber(self.targetid),self.target)
		if self.targets then
	...
	elseif self.action=='killboss' then
	...
	elseif self.action=='get' or self.action=='craft' or self.action=='farm' then
		goalcountnow = goalcountnow or self.got or C_Item.GetItemCount(self.targetid or 0)
	...
	elseif self.action=='goldcollect' then
		...
		goalcountnow = C_Item.GetItemCount(self.targetid or 0) or goalcountnow
	...
	elseif self.action=='buy' then
		goalcountnow = goalcountnow or C_Item.GetItemCount(self.targetid or 0)
		goalcountneeded = goalcountneeded or self.count or 1
		remaining = remaining or goalcountneeded-goalcountnow
		if remaining<1 then remaining=goalcountneeded end
	...
	elseif self.action=='from' or self.action=='avoid' then
		...
		for i,mob in ipairs(self.mobs) do
			local mobname
			if mob.id then mobname=ZGV.Localizers:GetTranslatedNPC(tonumber(mob.id),mob.name) end
			if mobname then
				mob.name = mobname
		...
		if self.skill then
			...
			if self:IsComplete() then
				...
				if self.skilllevel then
					...
					if self.recipedata and (self.recipedata.productid or self.recipedata.variants) then
						have = C_Item.GetItemCount(self.targetid or self.recipedata.productid)
						if self.targetid2 then have = have + C_Item.GetItemCount(self.targetid2) end
						if self.targetid3 then have = have + C_Item.GetItemCount(self.targetid3) end
						if have then
							...
							if self.count and have>=self.count then
								...
								-- primary display
								local product = self.recipedata.producttype=="enchant" and getSpellName(self.recipedata.productid) or ZGV:GetItemInfo(self.recipedata.productid)
						...
						if self.targetid  then have = C_Item.GetItemCount(self.targetid) end
						if self.targetid2 then have = have + C_Item.GetItemCount(self.targetid2) end
						if self.targetid3 then have = have + C_Item.GetItemCount(self.targetid3) end
						text = skill_create_text:format(self.count and COLOR_GOAL(self.count-have) or "",COLOR_ITEM(plural(self.spell,self.count and self.count-have or 0)))
...
function Goal:Prepare(reset)
	...
	if self.castspellid then self.castspell=getSpellName(self.castspellid) end
	...
	if self.more and self.targetid then -- record current inventory state
		self.more_base = C_Item.GetItemCount(self.targetid)
	end
...
function Goal:OnEnter()
	...
	if false and "disabled because of Legion script warnings" and ZGV.db.profile.magickey~="" then
		...
		and not self.macrosrc then
			if self.target or self.npcid then
				self.macrosrc = "/target "..(self.target or ZGV.Localizers:GetTranslatedNPC(tonumber(self.npcid),self.npcid))
	...
	and (not self.macro or not GetMacroInfo(self.macro)) --[[and ZGV.db.profile.tweaks_domacros--]] then
		...
		if not InCombatLockdown() then
			...
			self.macro = macro
			if macro then
				if self.macrospell then SetMacroSpell(macro,getSpellName(self.macrospell)) end
...
function Goal:OnClick(button)
	...
	if self.x and not self.force_noway  and not self.parentStep:IsCurrentlySticky()  then
		...
		if self.action=="mapmarker" then
			...
			ZGV.Pointer:FindTravelPath(way)
