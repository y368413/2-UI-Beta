...
function ZGV.Garrison_GetBuildingState(id_or_name,fixit)
	...
	for i,building in ipairs(C_Garrison.GetBuildings(garr60)) do
...
function ZGV:GetItemInfo(id)
	...
	local live_result={C_Item.GetItemInfo(id)}
...
function ZGV:GetItemInfoWipe()
	...
	for i=1,10000 do C_Item.GetItemInfo(i) end
end
...
function ZGV:GET_ITEM_INFO_RECEIVED(event,id,success)
	...
	if success and gii_cache and gii_cache[id] then
		gii_cache[id] = {C_Item.GetItemInfo(id)}
		gii_cache[id].timestamp=time()
	end
end
...
-- UIFrameFade* replacement. Use this to prevent UIFrameFadeIn/Out taint issues.
	...
	function ZGV.UIFrameFade.UIFrameFadeIn(frame, timeToFade, startAlpha, endAlpha,callback)
		local fadeInfo = {};
		fadeInfo.mode = "IN";
		fadeInfo.timeToFade = timeToFade or 5;
		fadeInfo.startAlpha = startAlpha or 0;
		fadeInfo.endAlpha = endAlpha or 1;
		fadeInfo.finishedFunc = callback
		ZGV.UIFrameFade.UIFrameFade(frame, fadeInfo);
	end

	-- Convenience function to do a simple fade out
	function ZGV.UIFrameFade.UIFrameFadeOut(frame, timeToFade, startAlpha, endAlpha,callback)
		local fadeInfo = {};
		fadeInfo.mode = "OUT";
		fadeInfo.timeToFade = timeToFade or 5;
	...
	-- our addition to fade functions
	function ZGV.UIFrameFade.UIFrameFadeAbort(frame) 
		for i,v in pairs(FADEFRAMES) do
			if v==frame then table.remove(FADEFRAMES,i) end
		end
		frame:SetAlpha(1);
	end
...
function IL.Strip(itemlink)
	if tonumber(itemlink) then itemlink="item:"..itemlink end
...
function ZGV.F.CutsceneCancel()
	...
	ZGV:Print(text)
	if ZGV.db.profile.nc_cutscenes then ZGV.NotificationCenter:AddEntry("cutscene",ZGV.L['cinematic_cancelled_short'],"") end
end
...
function ZGV.F.MovieCancel()
	...
	if CinematicFinished then CinematicFinished(Enum.CinematicType.GameMovie, true) end
	
	ZGV:Print(ZGV.L['cinematic_cancelled'])
	if ZGV.db.profile.nc_cutscenes then ZGV.NotificationCenter:AddEntry("cutscene",ZGV.L['cinematic_cancelled_short'],"") end
end
...
function ZGV.F.GetTimeFromSeconds(seconds,mode)
...
function ZGV.F.IsBoosted(expansion)
	...
	--Level 40 - ID 9
	--Level 45 - ID 14884
	--Level 50 - ID 14782 (10 - pre Shadowlands)
	--level 60 - ID 14783

	local achidates = {}
	local count = 0

	--No proper datetime stamp, so this is not perfect. It also won't detect partial boosts, e.g. from 40 to 60, as this can be easily power leveled within a day.
	local _,_,_,_,m10,d10,y10 = GetAchievementInfo(6)
	local _,_,_,_,m20,d20,y20 = GetAchievementInfo(7)
	local _,_,_,_,m30,d30,y30 = GetAchievementInfo(8)
	local _,_,_,_,m40,d40,y40 = GetAchievementInfo(9)
	local _,_,_,_,m45,d45,y45 = GetAchievementInfo(14884)
	local _,_,_,_,m50,d50,y50 = GetAchievementInfo(14782)
	local _,_,_,_,m60,d60,y60 = GetAchievementInfo(14783)

	table.insert(achidates, tostring(m10..d10..y10))
	table.insert(achidates, tostring(m20..d20..y20))
	table.insert(achidates, tostring(m30..d30..y30))
	table.insert(achidates, tostring(m40..d40..y40))
	table.insert(achidates, tostring(m45..d45..y45))
	table.insert(achidates, tostring(m50..d50..y50))
	table.insert(achidates, tostring(m60..d60..y60))

	for i,date in ipairs(achidates) do
		if date ~= tostring(m10..d10..y10) then
			break
		else
			count = count + 1
		end
...
local function getExpFromMob(level)
	...
	local baseexp = (CharLevel*5) + (continentBase[continent] or 580)
...
function ZGV.F.TrackKills(_,event,message)
	...
	ZGV:Debug("TrackKills: %s",lastkills[1])
	lastkills[6]=nil
end
	...
	if not ZGV.Languages.ActiveLanguage then
		for _,spell in ipairs(ZGV.Languages.Spells) do 
			if IsSpellKnown(spell) then
				ZGV.Languages.ActiveLanguage = spell
				break
			end
		end
	end
...
ZGV.Replacements = {}
function ZGV.Replacements:UpdateVisibility(force)
	if ZGV.db.profile.hidetracker then
	...
	if force then ZGV.Replacements.ObjectiveTracker:Show() end

	if ZGV.db.profile.hideexptracker then
		if ZGV.Frame:IsVisible() and ZGV.db.profile.progressbarmode=="exp" and (not ZGV.IsRetail or ZGV.Replacements.ExpBar.shownBarIndex==4) then
			ZGV.Replacements.ExpBar:Hide()
		else
			ZGV.Replacements.ExpBar:Show()
		end
	end
	if force then ZGV.Replacements.ExpBar:Show() end
end
...
function ZGV.Replacements:Startup()
	...
	ZGV.Replacements.ObjectiveTracker:HookScript("OnShow",function() ZGV.Replacements:UpdateVisibility() end)
	ZGV.Replacements.ExpBar:HookScript("OnShow",function() ZGV.Replacements:UpdateVisibility() end)

	ZGV:AddMessageHandler("ZGV_FRAME_VISIBILITY",function() ZGV.Replacements:UpdateVisibility() end)
	ZGV:AddMessageHandler("CLICKED_PROGRESSBAR",function() ZGV.Replacements:UpdateVisibility() end)

	ZGV.Replacements:UpdateVisibility()
end

ZGV.ThrottlerData = {}
function ZGV:Throttler(ident,delay,callback,params)
	if ZGV.ThrottlerData[ident] then
		ZGV:Debug("&throttler prevented %s",ident)
		if ZGV.ThrottlerData[ident].callback then 
			ZGV:Debug("&throttler will retry %s",ident)
			ZGV.ThrottlerData[ident].retry = true 
		end
		return true
	end

	ZGV.ThrottlerData[ident] = {callback=callback,params=params}
	ZGV:Debug("&throttler started %s",ident)
	ZGV.ThrottlerData[ident].timer = ZGV:ScheduleTimer(function() 
		local callback = ZGV.ThrottlerData[ident].callback
		local retry = ZGV.ThrottlerData[ident].retry
		local params = ZGV.ThrottlerData[ident].params

		ZGV:CancelTimer(ZGV.ThrottlerData[ident].timer)
		ZGV.ThrottlerData[ident] = nil

		if retry then
			ZGV:Debug("&throttler queued call %s",ident)
			callback(params and unpack(params))
		end
		ZGV:Debug("&throttler ended %s",ident)
	end, delay)
end