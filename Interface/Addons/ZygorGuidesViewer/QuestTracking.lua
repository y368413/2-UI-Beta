...
OPTIONAL_QUEST_OBJECTIVE_DESCRIPTION_PATTERN = OPTIONAL_QUEST_OBJECTIVE_DESCRIPTION and OPTIONAL_QUEST_OBJECTIVE_DESCRIPTION:gsub("%(","%%("):gsub("%)","%%)"):gsub("%%s","(.*)")
local function ParseLeaderBoard(leaderboard,type)
	...
	local optional = (OPTIONAL_QUEST_OBJECTIVE_DESCRIPTION_PATTERN and leaderboard:match(OPTIONAL_QUEST_OBJECTIVE_DESCRIPTION_PATTERN)) and true or false
	...
	if (item) then
		if type=="faction" then
			return item,num,numneeded,optional  -- not really nums
		else
			return item,tonumber(num),tonumber(numneeded),optional
		end
	else
		return leaderboard,nil,nil,optional
	end
end
...
function ZGV:GetQuestLeaderBoards(questindex,questID)
	...
	for g=1,numgoals do
		...
		if leaderboard then
			local item,num,needed,optional = ParseLeaderBoard(leaderboard,type)
			...
			goals[g] = { item=item, num=num, needed=needed, type=type, complete=complete, leaderboard=leaderboard, optional=optional }
		else
...
function ZGV:QuestTracking_CacheQuestLog(from)
	local interacting = (QuestFrame and QuestFrame:IsVisible()) or (GossipFrame and GossipFrame:IsVisible())

	if not interacting then
		if ZGV:Throttler("QuestTracking_CacheQuestLog",0.5,ZGV.QuestTracking_CacheQuestLog,{ZGV,"throttler"}) then return end
	end
