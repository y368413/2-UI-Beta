...
function Tabs:Initialize()
	...
	Tabs.TabMenuFrame = CHAIN(CreateFrame("Frame",nil,ZGV.Frame,"UIDropDownForkTemplate"))
		:SetFrameStrata("DIALOG")
		:Hide()
	.__END
...
function Tabs:UpdateCurrentTab(mode,step)
	...
	tab:SetAsCurrent()
	tab:AssignGuide(ZGV.CurrentGuide.title,step or ZGV.CurrentGuide.CurrentStepNum,ZGV.CurrentGuide.type=="SHARED",mode)
end
...
function Tabs:CreateTab()
	...
	tab.Button = CHAIN(ui:Create("Button",ZGV.Frame.Border.TabContainer,"ZGVTab"..#Tabs.Pool))
		...
		:SetScript("OnMouseUp",function() tab:HandleClick() end)
	...
	tab.ButtonsContainer = CHAIN(ui:Create("Frame",tab.Button))
		:SetSize(icon_size+1,icon_size-1)
		:SetPoint("RIGHT",-decorwidth+1,0)
	.__END
	...
	tab.Change = CHAIN(CreateFrame("Button", nil, tab.ButtonsContainer, nil))
		...
		:SetScript("OnClick",function(self) 
			ZGV.GuideMenu:Show(ZGV.CurrentGuide.guidepath) 
...
function Tabs:HandleClick()
	if self.isActive then
		if DropDownForkList1 and DropDownForkList1:IsShown() and DropDownForkList1.dropdown==Tabs.TabMenuFrame then CloseDropDownForks() return end
		GameTooltip:Hide()
		...
		table.insert(menu,{
			text=L["tabsmenu_recent"],
			isTitle=true,
		})
		local used = 0
		for i,v in ipairs(ZGV.db.char.guides_history) do
			if not active[v[1]] and used<3 then
				local guide = ZGV:GetGuideByTitle(v[1])
				if guide then
					table.insert(menu,{
						text=guide.title_short,
						iconset=ZGV.IconSets.TabsIcons,
						iconkey=guide.type,
						func=function() self:AssignGuide(guide) self:ActivateGuide() end,
						notCheckable=1,
					})
					used = used + 1
				end
		...
		for i,guide in ipairs(ZGV.registeredguides) do
			...
			if type=="LEVELING" and not active[guide.title] then
		...
		table.insert(menu,{
			text=L["tabsmenu_new_guide"],
			iconset=ZGV.ButtonSets.TitleButtons,
			iconkey="PLUS",
			iconcolor={0,1,0},
			func=function() ZGV.GuideMenu:Show() ZGV.GuideMenu.UseTab = self end,
			notCheckable=1,
		})
