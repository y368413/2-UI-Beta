...
function ActionButton:UpdateCooldown()
	...
	if self:GetAttribute("type")=="spell" then
		local cooldown = GetSpellCooldown(button:GetAttribute("spell"))
		starts,dur,ends = cooldown.startTime, cooldown.duration, cooldown.startTime+cooldown.duration
...
function ActionButton:UpdateTexture()
	...
	if self:GetAttribute("type")=="spell" then
		local spellData = GetSpellInfo(self:GetAttribute("spell"))
