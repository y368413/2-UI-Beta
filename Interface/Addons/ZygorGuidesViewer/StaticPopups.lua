...
function Popup:returnMinimizeSettings()
	...
	return nil,title,notifcationText
end
...
function Popup.private:Minimize(popup,time,hideDelay)
	...
	local notiftype,title,text,data = popup:returnMinimizeSettings()
	if notiftype then
		data = data or {}
		data.shown = true
		data.reviewed = true
		ZGV.NotificationCenter:AddEntry(notiftype,title,text,data)
	end
...
function Popup.private:Close(popup)
	...
	if StaticPopup_DisplayedFrames then StaticPopup_DisplayedFrames[6]=nil end
...
function PopupHandler:CreatePopup(name,skin)
	...
	local popup = CHAIN(ui:Create("Frame",UIParent,name))
		...
		:SetScript("OnShow", function(self)
			...
			self.holder:Show()
			if StaticPopup_DisplayedFrames then StaticPopup_DisplayedFrames[6]=self.holder end
	...
	popup.minimize:SetScript("OnClick",function() popup.private:Minimize(popup) PopupHandler:ReturnToast() end)
	...
	function PopupHandler:ReturnToast() end
...
function PopupHandler:QueuePush(popup)
	...
	if not self.IsPopupVisible then
		self:QueuePop()
	end
end
