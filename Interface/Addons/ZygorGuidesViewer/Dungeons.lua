...
Dungeons.maxlevel = GetMaxLevelForExpansionLevel(expansion)
...
setmetatable(Dungeons,{
	__index=function(t,id)
		...
		local name, typeID, subtypeID, minLevel, maxLevel, recLevel, minRecLevel, maxRecLevel, expansionLevel, groupID, textureFilename, difficulty, maxPlayers, description, isHoliday, min_ilevel, attunement_achieve, attunement_quest, attunement_queston, bonusRepAmount,isTimeWalker,minPlayers,name2, phase, buff
		...
		if name and typeID~=4 then
			...
			dungeon.id = id
			dungeon.name = name
			dungeon.difficulty = isTimeWalker and 24 or difficulty -- timewalk gets reported as diff 2
			...
			dungeon.maxScaleLevel = isTimeWalker and Dungeons.maxlevel or dungeon.maxScaleLevel -- timewalk bypases normal scaling limits
