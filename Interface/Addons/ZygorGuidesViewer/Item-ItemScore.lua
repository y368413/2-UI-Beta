...
function ItemScore:Initialise()
	...
	if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK or ZGV.IsClassicCATA then
		...
		ZGV:AddEventHandler("CONSOLE_MESSAGE",ItemScore.OnEvent)
	end
...
function ItemScore:OnEvent(event,arg1,arg2,...)
	...
	elseif event == "CONSOLE_MESSAGE" then -- on classic, skills changed, so user may have learned new weapon skill
		ItemScore:UpdateEquipmentSkills(arg1,arg2,...)
...
function ItemScore:SetStatWeights(playerclass,playerspec,playerlevel)
	...
	if ZGV.IsRetail or ZGV.IsClassicCATA then
		...
		self.playerspecName = (select(2,GetSpecInfoFunc(self.playerspec)))
	...
	for name,rulesets in pairs(self.ActiveRuleSets) do
		...
		for spec,ruleset in pairs(rulesets) do
			...
			for statname,statvalue in pairs(stats) do
				...
				if caps and caps[statname] and not C_Item.IsEquippedItem(itemlink) then -- don't cap stats on already equipped items
		...
		for powerindex, powerid in pairs(tierdata.azeritePowerIDs) do
			...
			local _,powericon
  			if powerinfo then
				...
				local spellData = GetSpellInfo(powerspell)
				powername,powericon = spellData.name, spellData.iconID
				if location then
...
local function ItemScore_SetTooltipData(tooltip)
	...
	if not ZGV.ItemScore.Upgrades:IsEnabled() then return end
	...
	if not ItemScore.TooltipPatched then
		...
		if valid or validfuture then
			...
			if change_1 < 0 then
				t = ZGV.IconSets.AuctionToolsPriceIcons.DOWN1:GetFontString(width,height,nil,nil,255,0,0)
			elseif change_1 > 0 and change_1 < 100 then
				t = ZGV.IconSets.AuctionToolsPriceIcons.UP1:GetFontString(width,height,nil,nil,0,255,0)
			elseif change_1 > 99 and change_1 < 200 then
				t = ZGV.IconSets.AuctionToolsPriceIcons.UP2:GetFontString(width,height,nil,nil,0,255,0)
			elseif change_1 > 199 then
				t = ZGV.IconSets.AuctionToolsPriceIcons.UP3:GetFontString(width,height,nil,nil,0,255,0)
			else
				t = ZGV.IconSets.AuctionToolsPriceIcons.BULLET:GetFontString(width,height,nil,nil,155,155,155)
			end

			if change_2 then
				if change_2 > 0 and change_2 < 100 then
					v = ZGV.IconSets.AuctionToolsPriceIcons.UP1:GetFontString(width,height,nil,nil,0,255,0)
				elseif change_2 > 99 and change_2 < 200 then
					v = ZGV.IconSets.AuctionToolsPriceIcons.UP2:GetFontString(width,height,nil,nil,0,255,0)
				elseif change_2 > 199 then
					v = ZGV.IconSets.AuctionToolsPriceIcons.UP3:GetFontString(width,height,nil,nil,0,255,0)
			...
			local hotkeys = {{"IsShiftKeyDown","Shift"},{"IsControlKeyDown","Control"},{"IsAltKeyDown","Alt"}}
			local mousekeys = {{"LeftButton","left click"},{"RightButton","right click"}}
			
			for i,v in ipairs(hotkeys) do
				if v[1] == ZGV.db.profile.gearreportkey then hotkey = v[2] end
			end
			for i,v in ipairs(mousekeys) do
				if v[1] == ZGV.db.profile.gearreportmouse then mousekey = v[2] end
			end

			local report = "|r  |cffa8a8a8"..hotkey.." + "..mousekey.." to report a wrong suggestion"
			...
			if (equipped_item_1 and equipped_item_1.score) or (equipped_item_2 and equipped_item_2.score) and score then -- item can replace something
				...
				if equipped_item_1.score then
					if itemlink~=equipped_item_1.itemlink then
						local comment = "|r  " .. slotinfo1 .. futureprefix .. ((change_1 > 0) and "Upgrade: |T"..t.."|t |cff00ff00" or "Downgrade: |T"..t.."|t|cffff0000")
						if ZGV.db.profile.debug then comment = comment..(score-equipped_item_1.score).." " end
						comment = comment  .. futuresuffix
						tooltip:AddLine(comment..(change_1>999 and "999+" or change_1).."% ")
						if not slot_2 and ZGV.db.profile.badupgradehotkey then tooltip:AddLine(report) end
					else
						tooltip:AddLine("|r  "..slotinfo1.."Equipped")
					end
				else
					tooltip:AddLine("|r  " .. slotinfo1 .. futureprefix .. "Upgrade: |T"..t.."|t |cff00ff00100% " .. futuresuffix)
					if ZGV.db.profile.badupgradehotkey then tooltip:AddLine(report) end
				end

				if slot_2 and equipped_item_2.score then
					if itemlink~=equipped_item_2.itemlink then
						local comment = "|r  " .. slotinfo2 .. futureprefix .. ((change_2 > 0) and "Upgrade: |T"..v.."|t |cff00ff00" or "Downgrade: |T"..v.."|t |cffff0000")
						if ZGV.db.profile.debug then comment = comment..(score-equipped_item_2.score).." " end
						comment = comment  .. futuresuffix
						tooltip:AddLine(comment..(change_2>999 and "999+" or change_2).."% ")
						tooltip:AddLine(report_key)
					else
						tooltip:AddLine("|r  " .. slotinfo2 .. "Equipped")
					end
				elseif slot_2 then
					tooltip:AddLine("|r  " .. slotinfo2 .. futureprefix .. "Upgrade: |T"..v.."|t |cff00ff00100% " .. futuresuffix)
					if ZGV.db.profile.badupgradehotkey then tooltip:AddLine(report) end
				end
			elseif (score or 0)>0 then
				tooltip:AddLine(" ")
				tooltip:AddLine("|cfffe6100Zygor ItemScore"..spec..":|r")
				tooltip:AddLine("|r  " .. futureprefix .. "Upgrade: |T"..t.."|t |cff00ff00100% " .. futuresuffix)
				if ZGV.db.profile.badupgradehotkey then tooltip:AddLine(report) end
			end
			...
			if item.type~="" then
				...
				if item.type=="INVTYPE_TRINKET" then
					tooltip:AddLine("   Trinkets are not part of the Zygor scoring system.")
					return
				end

				if ZGV.IsRetail then
					tooltip:AddLine("|r  Not a valid item for "..(ItemScore.playerspecName or ItemScore.ActiveRuleSet.specname or "current spec"))
				else
...
function ItemScore:UpdateEquipmentSkills(msg)
	...
	if id and ItemScore.SkillNamesByID[id] then
		if from == 0 then
			...
			ItemScore.Skills[ItemScore.SkillNamesByID[id]] = to
		end
	end
end
