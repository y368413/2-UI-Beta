...
ConRO.Flags = {};
ConRO.SpellsGlowing = {};
ConRO.WindowsGlowing = {};
ConRO.DefGlowing = {};
ConRO.DefWindowsGlowing = {};
...
ConRO.TauntFrames = {};
ConRO.WindowFramePool = {};
ConRO.WindowFrames = {};
ConRO.DefWindowFramePool = {};
ConRO.DefWindowFrames = {};
...
function ConRO:DisplayToggleFrame()
	...
	local frame = CreateFrame("Frame", "ConROButtonFrame", UIParent)
	local vert = 2;
	local hori = 1;
...
function ConRO:SlashUnlock()
	...
	ConRODefenseWindow:SetMovable(ConRO.db.profile.unlockWindow);
...
function ConRO:DisplayWindowFrame()
	...
	local _, _, spellTexture = GetSpellInfo(247007);
		frame:SetMovable(true)
		frame:SetClampedToScreen(true)
		frame:RegisterForDrag("LeftButton")
	...
	local t = frame.texture;
		...
		if ConRO.db.profile.enableWindowBackground then
			t:Show();
		else 
			t:Hide();
		end
...
function ConRO:CreateDisplayWindow(parent, id)
	local frame = tremove(self.WindowFramePool);
	local spellName, _, spellTexture = GetSpellInfo(id);
	local _, Class = UnitClass("player");
	local Color = RAID_CLASS_COLORS[Class];
		if not frame then
			frame = CreateFrame('Frame', 'ConRO_WindowSpell_' .. id, parent);
			fontstring = frame:CreateFontString(nil, "ARTWORK", "GameFontHighlight");
			fontkey = frame:CreateFontString("ConRO_HotKey_" .. id, "ARTWORK", "GameFontHighlight");
		end

	frame:SetParent("ConROWindow");
	frame:SetFrameStrata('MEDIUM');
	...
	fontstring:SetWidth(ConRO.db.profile.windowIconSize + 20);
	fontstring:SetHeight(ConRO.db.profile.windowIconSize);
	...
	fontkey:SetText(ConRO:FindKeybinding(id));
...
function ConRO:CreateDefWindow(parent, id)
	local frame = tremove(self.DefWindowFramePool);
	local spellName, _, spellTexture = GetSpellInfo(id);
	local _, Class = UnitClass("player");
	local Color = RAID_CLASS_COLORS[Class];
		if not frame then
			frame = CreateFrame('Frame', 'ConRO_DefWindowSpell_' .. id, parent);
			fontstring = frame:CreateFontString(nil, "ARTWORK", "GameFontHighlight");
			fontkey = frame:CreateFontString("ConRO_DefHotKey_" .. id, "ARTWORK", "GameFontHighlight");
		end

	frame:SetParent("ConRODefenseWindow");
	frame:SetFrameStrata('MEDIUM');
	frame:SetFrameLevel('6');
	frame:SetPoint('CENTER', 0, 0);
	frame:SetSize(ConRO.db.profile.windowIconSize * .75, ConRO.db.profile.windowIconSize * .75);
...
function ConRO:DestroyWindowFrames()
	local frame;
	for key, frame in pairs(self.WindowFrames) do
		frame:GetParent().ConROWindowFrames = nil;
...
function ConRO:DestroyDefWindowFrames()
	local frame;
	for key, frame in pairs(self.DefWindowFrames) do
		frame:GetParent().ConRODefWindowFrames = nil;
		frame:ClearAllPoints();
		frame:Hide();
		frame:SetParent(UIParent);
		frame.width = nil;
...
function ConRO:WindowGlow(frame, id)
	if frame.ConROWindowFrames and frame.ConROWindowFrames[id] then
		frame.ConROWindowFrames[id]:Show();
	else
		if not frame.ConROWindowFrames then
			frame.ConROWindowFrames = {};
		end

		frame.ConROWindowFrames[id] = self:CreateDisplayWindow(frame, id);
		frame.ConROWindowFrames[id]:Show();
	end
end

function ConRO:DefWindowGlow(frame, id)
	if frame.ConRODefWindowFrames and frame.ConRODefWindowFrames[id] then
		frame.ConRODefWindowFrames[id]:Show();
	else
		if not frame.ConRODefWindowFrames then
			frame.ConRODefWindowFrames = {};
		end

		frame.ConRODefWindowFrames[id] = self:CreateDefWindow(frame, id);
		frame.ConRODefWindowFrames[id]:Show();
	end
end
...
function ConRO:HideWindowGlow(frame, id)
	if frame.ConROWindowFrames and frame.ConROWindowFrames[id] then
		frame.ConROWindowFrames[id]:Hide();
	end
end

function ConRO:HideDefWindowGlow(frame, id)
	if frame.ConRODefWindowFrames and frame.ConRODefWindowFrames[id] then
		frame.ConRODefWindowFrames[id]:Hide();
	end
end
...
function ConRO:WindowSpell(spellID)
	if self.Spells[spellID] ~= nil then
		for k, button in pairs(self.Spells[spellID]) do
...
function ConRO:DefWindowSpell(spellID)
	if self.DefSpells[spellID] ~= nil then
...
function ConRO:GlowNextWindow(spellID)
	self:WindowClear();
	self:WindowSpell(spellID);
end

function ConRO:GlowNextDefWindow(spellID)
	self:DefWindowClear();
