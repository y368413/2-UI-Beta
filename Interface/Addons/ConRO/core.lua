	...
	args = {
		...
		unlockWindow = {
			...
			set = function(info, val)
				...
				if val == true or ConRO.db.profile.enableWindowBackground then
					ConROWindow.texture:Show();
					ConRODefenseWindow.texture:Show();
				else 
					ConROWindow.texture:Hide();
					ConRODefenseWindow.texture:Hide();
				end				
			end,
		...
		enableWindowBackground = {
			...
			desc = 'Show background texture behind Display Windows.',
			type = 'toggle',
			width = 'normal',
			order = 47,
			set = function(info, val)
				ConRO.db.profile.enableWindowBackground = val;		
				if val == true or ConRO.db.profile.unlockWindow then
					ConROWindow.texture:Show();
					ConRODefenseWindow.texture:Show();
				else 
					ConROWindow.texture:Hide();
			...
			get = function(info) return ConRO.db.profile.enableWindowBackground end
		},
		transparencyWindow = {
		...
		resetExtraWindows = {
			...
			func = function(info)
				...
				ConRODefenseWindow:SetPoint("BOTTOMRIGHT", "ConROWindow", "BOTTOMLEFT", -50, 5);
