...
Skada:AddLoadableModule("Damage", nil, function(Skada, L)
	...
	local function log_damage(set, dmg)
		...
		if player then
			...
			if not player.damagespells[dmg.spellname] then player.damagespells[dmg.spellname] = {id = dmg.spellid, totalhits = 0, damage = 0, school = dmg.school} end
			...
			if spell.max == nil  or amount > spell.max                     then spell.max = amount end
			if (spell.min == nil or amount < spell.min) and not dmg.missed then spell.min = amount end
			...
			if dmg.critical then
				...
				if not spell.criticalmax or amount > spell.criticalmax then spell.criticalmax = amount end
				if not spell.criticalmin or amount < spell.criticalmin then spell.criticalmin = amount end
			...
			elseif dmg.crushing then
				...
				if not spell.hitmax or amount > spell.hitmax then spell.hitmax = amount	end
				if not spell.hitmin or amount < spell.hitmin then spell.hitmin = amount	end
			...
			if set == Skada.current and dmg.dstname and amount > 0 then
				...
				if not player.damaged[dmg.dstname] then	player.damaged[dmg.dstname] = 0	end
	...
	local function SpellDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName,
				   dstFlags, spellId, spellName, spellSchool, samount, soverkill, sschool,
				   sresisted, sblocked, sabsorbed, scritical, sglancing, scrushing, soffhand)
	...
	local function SpellAbsorbed(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
		...
		if type(chk) == "number" then
			...
			local spellId, spellName, spellSchool, _, _, _, _, _, _, _, aAmount = ...

			if aAmount then	SpellDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, aAmount) end
		else

			local _, _, _, _, _, _, _, aAmount = ...

			if aAmount then	SwingDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, aAmount) end
		end
	end
	...
	function mod:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			...
			if player.damage > 0 then
				...
				if player.damage > max then max = player.damage end
		...
		if player then
			...
			if spell then
				...
				if spell.school then
					...
					-- =====================================
					-- | API   : C_Spell.GetSchoolString   |
					-- | valid : + 10.2.6 / 4.4.0 / 1.15.2 |
					-- =====================================
					if c then tooltip:AddLine(C_Spell.GetSchoolString(spell.school), c.r, c.g, c.b)	end
				end
			...
			if player then
				...
				for spellname, spell in pairs(player.damagespells) do
					...
					if spellInfo then
						...
						if spell.damage > max then max = spell.damage end
	...
	function spellmod:Update(win, set)
		...
		if player then
			...
			if spell then
				...
				if spell.hit and spell.hit           > 0 then add_detail_bar(win, L["Hit"]     , spell.hit)	 end
				if spell.critical and spell.critical > 0 then add_detail_bar(win, L["Critical"], spell.critical) end
				if spell.glancing and spell.glancing > 0 then add_detail_bar(win, L["Glancing"], spell.glancing) end

				if spell.crushing and spell.crushing > 0 then add_detail_bar(win, L["Crushing"], spell.crushing) end
				if spell.ABSORB and spell.ABSORB     > 0 then add_detail_bar(win, L["Absorb"]  , spell.ABSORB)	 end
				if spell.BLOCK and spell.BLOCK       > 0 then add_detail_bar(win, L["Block"]   , spell.BLOCK)	 end

				if spell.DEFLECT and spell.DEFLECT   > 0 then add_detail_bar(win, L["Deflect"] , spell.DEFLECT)	 end
				if spell.DODGE and spell.DODGE       > 0 then add_detail_bar(win, L["Dodge"]   , spell.DODGE)	 end
				if spell.EVADE and spell.EVADE       > 0 then add_detail_bar(win, L["Evade"]   , spell.EVADE)	 end

				if spell.IMMUNE and spell.IMMUNE     > 0 then add_detail_bar(win, L["Immune"]  , spell.IMMUNE)	 end
				if spell.MISS and spell.MISS         > 0 then add_detail_bar(win, L["Missed"]  , spell.MISS)	 end
				if spell.PARRY and spell.PARRY       > 0 then add_detail_bar(win, L["Parry"]   , spell.PARRY)	 end

				if spell.REFLECT and spell.REFLECT   > 0 then add_detail_bar(win, L["Reflect"] , spell.REFLECT)	 end
				if spell.RESIST and spell.RESIST     > 0 then add_detail_bar(win, L["Resist"]  , spell.RESIST)	 end
			end
		end
	end
	...
	function mod:OnEnable()
		...
		Skada:AddFeed(L["Damage: Personal DPS"],
			      ...
			      function()
				      ...
				      if Skada.current then
					      ...
					      if player then return Skada:FormatNumber(getDPS(Skada.current, player)) .. " " .. L["DPS"] end
				      end
 			      end

			     )	

		Skada:AddFeed(L["Damage: Raid DPS"],

			      function()

				      if Skada.current then return Skada:FormatNumber(getRaidDPS(Skada.current)) .. " " .. L["RDPS"] end
	...
	function mod:AddPlayerAttributes(player)
		...
		if not player.damaged then player.damaged = {} end
	end
	...
	function mod:AddSetAttributes(set)
		...
		if not set.damage then set.damage = 0 end
	end
