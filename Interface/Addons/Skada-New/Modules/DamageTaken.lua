...
Skada:AddLoadableModule("DamageTaken", nil, function(Skada, L)
	...
	local function log_damage_taken(set, dmg)
		...
		if player then
			...
			if not player.damagetakenspells[dmg.spellname] or not player.damagetakenspells[dmg.spellname]['absorbed'] then player.damagetakenspells[dmg.spellname] =
			...
			if spell.max == nil or dmg.amount > spell.max then spell.max = dmg.amount end

			if dmg.crushing then spell.crushing = spell.crushing + 1            end
			if dmg.blocked  then spell.blocked  = spell.blocked  + dmg.blocked  end
			if dmg.absorbed then spell.absorbed = spell.absorbed + dmg.absorbed end
			if dmg.critical then spell.critical = spell.critical + 1            end
			if dmg.resisted then spell.resisted = spell.resisted + dmg.resisted end
			if dmg.glancing then spell.glancing = spell.glancing + 1            end

			if (spell.min == nil or dmg.amount < spell.min) then spell.min = dmg.amount end
	...
	function spellplayers:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			...
			if player.damagetaken > 0 and player.damagetakenspells[self.spellname] then
				...
				if player.damagetakenspells[self.spellname].damage > max then max = player.damagetakenspells[self.spellname].damage end
	...
	function mod:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			...
			if player.damagetaken > 0 then
				...
				if player.damagetaken > max then max = player.damagetaken end
