...
Skada:AddLoadableModule("CC", nil, function(Skada, L)
			...
			if Skada.db.profile.modules.ccannounce and IsInRaid() and UnitInRaid(srcName) and not (instanceType == "pvp") then
				...
				if Skada.db.profile.modules.ccignoremaintanks then
					...
					for i = 1, 40 do
						...
						if name == srcName and role == "maintank" then return end
					end
				end


				if petid ~= srcGUID then srcName = petname .. " (" .. srcName .. ")" end
	...
	function mod:AddPlayerAttributes(player)
		...
		if not player.ccbreaks then player.ccbreaks = 0	end
	end
	...
	function mod:AddSetAttributes(set)
		...
		if not set.ccbreaks then set.ccbreaks = 0 end
	end
	...
	function mod:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			...
			if player.ccbreaks > 0 then
				...
				if player.ccbreaks > max then max = player.ccbreaks end
