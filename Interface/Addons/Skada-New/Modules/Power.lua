...
Skada:AddLoadableModule("Power", nil, function(Skada, L)
	...
	local function log_gain(set, gain)
		...
		if player then
			...
			if not player.power[gain.type] then player.power[gain.type] = {spells = {}, amount = 0}	end
			if not set.power[gain.type] then set.power[gain.type] = 0 end
			...
			if not player.power[gain.type].spells[gain.spellid] then player.power[gain.type].spells[gain.spellid] = 0 end
	...
	function basemod:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			...
			if player.power[self.power].amount > max then max = player.power[self.power].amount end
	...
	function manamod:AddPlayerAttributes(player)
		...
		if not player.power then player.power = {} end
	end
	...
	function manamod:AddSetAttributes(set)
		...
		if not set.power then set.power = {} end
	end
