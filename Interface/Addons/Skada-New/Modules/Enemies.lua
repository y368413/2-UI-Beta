...
Skada:AddLoadableModule("Enemies", nil, function(Skada, L)
	...
	local function log_healspell(mob, entry, spellname, healing, overheal, crit)
		...
		if crit then spell.crits = spell.crits + 1 end
		...
		if not spell.min or healing < spell.min then spell.min = healing end
		if not spell.max or healing > spell.max then spell.max = healing end
	end
	...
	local function log_healing(set, dmg, ...)
		...
		local healing = math.max(0,amount - overheal)
		...
		return function (self, win, set)
			...
			for name, mob in pairs(set.mobs) do
				...
				if (mob[stat] or 0) > 0 then
					...
					if mob[stat] > max then	max = mob[stat]	end
		...
		if mob then
			...
			for name, player in pairs(mob.players) do
				...
				if player.done > 0 then
					...
					if player.done > max then max = player.done end
		...
		if mob then
			...
			for name, player in pairs(mob.players) do
				...
				if player.taken > 0 then
					...
					if player.taken > max then max = player.taken end
			...
			if mob then
				...
				for name, info in pairs(mob[entry]) do
					...
					if info.hits > 0 then
						...
						if info.healing > max then max = info.healing end
...
function done:GetSetSummary(set)
	...
	function done:AddSetAttributes(set)
		...
		if not set.mobhdone then set.mobhdone = 0 end
	end
