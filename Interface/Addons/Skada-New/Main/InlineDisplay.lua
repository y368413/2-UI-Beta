...
function mod:Create(window, isnew)
	...
	if not window.frame then
		...
		if window.db.height==15 then window.db.height = 23 end
	...
	window.frame:SetScript("OnMouseDown",

			       function(frame, button)

				       if button == "RightButton" then window:RightClick() end
			       end
	...
	window.frame:SetScript("OnDragStart",
	...
	window.frame:SetScript("OnDragStop",
...
function barlibrary:CreateBar(uuid, win)
	...
	if uuid then self.nextuuid = self.nextuuid + 1 end
...
function mod:Update(win)
	...
	for i=#win.dataset, 1, -1 do
		...
		if win.dataset[i].label==nil then table.remove(win.dataset, i) end
	end
