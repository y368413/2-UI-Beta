...
local function tooltipHandler(win, tooltip)
	...
	if win.db.useframe then	Skada:SetTooltipPosition(tooltip, win.frame) end
	...
	if #win.dataset > 0 then
		...
		for i, data in ipairs(win.dataset) do
			...
			if data.id and not data.ignore and i < 30 then
				...
				if win.metadata.showspots and Skada.db.profile.showranks then label = (("%2u. %s"):format(i, label)) end
...
function mod:Create(win, isnew)
	...
	if not win.frame then
		...
		win.frame:SetScript("OnDragStart",
		...
		win.frame:SetScript("OnDragStop",
...
function mod:Show(win)
	...
	if win.db.useframe then win.frame:Show() end
end
...
function mod:Hide(win)
	...
	if win.db.useframe then	win.frame:Hide() end
end
...
function mod:Update(win)
	...
	if win.obj then	win.obj.text = "" end
	...
	if #win.dataset > 0 then
		...
		if data.id then
			...
			if win.obj then	win.obj.text = label end
			if win.db.useframe then	win.frame.title:SetText(label)
