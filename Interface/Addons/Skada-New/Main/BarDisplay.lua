...
--		window.bargroup:AddButton(L["Report"], L["Report description"], "Interface\\Buttons\\UI-GuildButton-MOTD-Up", "Interface\\Buttons\\UI-GuildButton-MOTD-Up", function() Skada:OpenReportWindow(bargroup.win) end)
		...
		window.bargroup:AddButton(L["Stop"], L["Stop description"], "Interface\\Buttons\\Arrow-Down-Down", "Interface\\Buttons\\Arrow-Down-Down",
	...
	window.bargroup.button:SetScript("OnEnter", function(win, button) window.bargroup:SetButtonsOpacity(1.0) end)
	window.bargroup.button:SetScript("OnLeave", function(win, button) if not window.bargroup.button:IsMouseOver() then window.bargroup:SetButtonsOpacity(0.25) end end)
	...
	if not mod.class_icon_tcoords then
...
local function showmode(win, id, label, mode)
	...
	if win.selectedmode then tinsert(win.history, win.selectedmode)	end
...
local function BarClickIgnore(bar, button)
	...
	if button == "RightButton" then	win:RightClick() end
end
...
function mod:Update(win)
	...
	if win.metadata.showspots then table_sort(win.dataset, value_sort) end
	...
	for i, data in ipairs(win.dataset) do
		...
		if (data.icon and not data.ignore) or (data.class and win.db.classicons) or (data.role and win.db.roleicons) then hasicon = true end
	end


	if hasicon and not win.bargroup.showIcon then win.bargroup:ShowIcon() end
	if not hasicon and win.bargroup.showIcon then win.bargroup:HideIcon() end
		...
		if data.id then
			...
			if bar then
				...
				elseif data.class and win.db.classcolorbars then
					...
					if color then bar:SetColorAt(0, color.r, color.g, color.b, color.a or 1) end
				else
				...
				if Skada.db.profile.showself and data.id and data.id == UnitGUID("player") then	bar.fixed = true end
			end


			if win.metadata.ordersort then bar.order = i end
			...
			if win.metadata.wipestale then bar.checked = true end
			...
			if data.backgroundcolor then bar.bgtexture:SetVertexColor(data.backgroundcolor.r, data.backgroundcolor.g, data.backgroundcolor.b, data.backgroundcolor.a or 1) end
			...
			if not data.ignore then	nr = nr + 1 end
		end
	end
	...
	if win.metadata.wipestale then
		...
		for name, bar in pairs(bars) do
			...
			if not bar.checked then	win.bargroup:RemoveBar(bar) end
		end
	end
...
function mod:ApplySettings(win)
	...
	if p.title.textcolor then fo:SetTextColor(p.title.textcolor.r, p.title.textcolor.g, p.title.textcolor.b, p.title.textcolor.a) end
...

					{
						 ...
						 desc = L["Sets the font flags."],
					       values = {[""] = L["None"], ["OUTLINE"] = L["Outline"], ["THICKOUTLINE"] = L["Thick outline"],
								 ["MONOCHROME"] = L["Monochrome"], ["OUTLINEMONOCHROME"] = L["Outlined monochrome"]},
						  get = function()
