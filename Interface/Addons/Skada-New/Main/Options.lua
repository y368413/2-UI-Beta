...
function Skada:AddColumnOptions(mod)
	...
	if mod.metadata and mod.metadata.columns then
		...
		for colname, value in pairs(mod.metadata.columns) do
			...
			if db[c] ~= nil then mod.metadata.columns[colname] = db[c] end
					    ...
					    order = 4,
					     ...
					     args =
						   ...
						   tentativecombatstart = {
										 ...
										 desc = L["Skada usually uses a very conservative (simple) combat detection scheme that works best in raids." ..
											  " With this option Skada attempts to emulate other damage meters. Useful for running dungeons."     ..
											  " Meaningless on boss encounters."],
										order = 10,
										  get = function()
...

									{
										 ...
										 desc = L["Choose which data feed to show in the DataBroker view. This requires an LDB display addon, such as Titan Panel."],
									       values = function()
												local feeds = {}
												feeds[""]   = L["None"]
											 	for name, func in pairs(Skada:GetFeeds()) do
													feeds[name] = name
												end
												return feeds
											end,
