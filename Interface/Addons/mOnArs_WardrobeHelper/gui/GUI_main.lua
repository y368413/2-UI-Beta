...
local function compMenuItemsName(reverse)
	return function(a, b)
		if a.index == b.index then
			return cmpAlphabetical(a.name, b.name, reverse)
		else
...
local function compMenuItemsProgress(reverse)
	return function(a, b)
		local indexA = a.index * 10
		local indexB = b.index * 10
		if indexA % 10 == 2 then
			indexA = indexA - 1
		end
		if indexB % 10 == 2 then
		...
		if indexA == indexB then
			local instA = o.instances[a.name]
			local instB = o.instances[b.name]
			...
			if instA ~= nil then
				if mOnWDSave.disableProgress then
					percA = instA.total - instA.collected
				else
					percA = math.floor((instA.collected / instA.total * 100) + 0.5)
					if instA.total == 0 or percA > 100 then
						percA = 100
					end
				end
			end
			...
			if instB ~= nil then
				if mOnWDSave.disableProgress then
					percB = instB.total - instB.collected
				else
					percB = math.floor((instB.collected / instB.total * 100) + 0.5)
					if instB.total == 0 or percB > 100 then
						percB = 100
					end
				end
			end
...
local fff = CreateFrame("Button", nil, f)
fff:Show()
f.col1Header = fff
fff:SetWidth(120)
fff:SetHeight(24)
fff:SetPoint("LEFT", f.ColL1, "RIGHT")
fff:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight", "ADD")
fff:RegisterForClicks("LeftButtonUp", "RightButtonUp")
fff:SetScript(
	"OnClick",
	function(self, button)
		if button == "LeftButton" then
			if (mOnWDSave.sortColumn == 0) then
				mOnWDSave.sortDirection = -mOnWDSave.sortDirection
			else
				mOnWDSave.sortColumn = 0
				mOnWDSave.sortDirection = 1
			end
		end
...
f.ColTT1 = fs
fs:SetText("")
fs:SetPoint("RIGHT", f.ColR1, -20, 0)
...
fff:SetWidth(220)
fff:SetHeight(24)
fff:SetPoint("LEFT", f.ColL2, "RIGHT")
fff:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight", "ADD")
fff:RegisterForClicks("LeftButtonUp", "RightButtonUp")
fff:SetScript(
	"OnClick",
	function(self, button)
		if button == "LeftButton" then
			if (mOnWDSave.sortColumn == 1) then
				mOnWDSave.sortDirection = -mOnWDSave.sortDirection
			else
				mOnWDSave.sortColumn = 1
				mOnWDSave.sortDirection = 1
			end
		end
		o.GUImainPage(o.CURRENT_PAGE)
...
f.ColTT2 = fs
fs:SetText("^")
fs:SetPoint("RIGHT", f.ColR2, -20, 0)
...
local function CreateTableRow(parent, rowHeight, N)
		...
		function(self, button)
			...
			if button == "RightButton" and IsShiftKeyDown() then
				if mOnWDSave.favoriteInstances[row.text2:GetText()] == 8 then
					mOnWDSave.favoriteInstances[row.text2:GetText()] = nil
				elseif mOnWDSave.favoriteInstances[row.text2:GetText()] ~= nil then
					if mOnWDSave.favoriteInstances[row.text2:GetText()] == true then
	...
	row:SetScript(
		...
		function()
			if o.instances[row.text2:GetText()] then
				local count = 0
				for k, v in pairs(o.instances[row.text2:GetText()]["difficulties"]) do
					count = count + 1
				end
				if count > 1 then
					...
					for k, v in orderedPairs(o.instances[row.text2:GetText()]["difficulties"]) do
						...
						if total == 0 or percent > 100 then
							percent = 100
						end
						GameTooltip:AddDoubleLine(
							k,
							mOnWDSave.disableProgress and total or (collected .. " / " .. total) .. " (" .. percent .. "%)"
						)
					end
					GameTooltip:Show()
				end
	...
	c:Hide()
	row.star = c]] local c =
		row.status:CreateTexture(nil, "OVERLAY")
...
b:SetScript(
	...
	function()
		...
		GameTooltip:AddDoubleLine(
			o.strings["Shift"] .. " + " .. o.strings["Right Click"],
			o.strings["Main Shift RightClick Help"]
		)
		GameTooltip:AddDoubleLine(
			o.strings["Shift"] .. " + " .. o.strings["Left Click"],
			o.strings["Main Shift LeftClick Help"]
		)
		GameTooltip:Show()
	end
)
...
local function everythingNeededExists(i)
	return mOnWD_MainFrame and mOnWD_MainFrame.ListFrame and mOnWD_MainFrame.ListFrame.RowFrame and
		mOnWD_MainFrame.ListFrame.RowFrame.rows and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i] and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i].text1 and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i].text2 and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i].saved and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i].percent and
		mOnWD_MainFrame.ListFrame.RowFrame.rows[i].status
end
				...
				if item.category then
					...
					if o.tiers[item.name] then
						...
						for k, v in pairs(t) do
							for j = 0, #v do
								if o.instances[v[j]] then
									total = total + o.instances[v[j]]["total"]
								end
							end
						end
						if mOnWDSave.disableProgress then
							...
							for k, v in pairs(t) do
								for j = 0, #v do
									if o.instances[v[j]] then
										collected = collected + o.instances[v[j]]["collected"]
									end
								end
							end
							local percent = math.floor((collected / total * 100) + 0.5)
...
o.GUI_sort = function()
	if (mOnWDSave.sortDirection == 1) then
		f.ColTT1:SetText("^")
		f.ColTT2:SetText("^")
	else
		f.ColTT1:SetText("v")
	...
	if mOnWDSave.sortColumn == 0 then
		...
		if (mOnWDSave.sortDirection == 1) then
			table.sort(o.menuItems, compMenuItemsProgress(false))
		else
			table.sort(o.menuItems, compMenuItemsProgress(true))
		end
	elseif mOnWDSave.sortColumn == 1 then
		f.ColTT1:Hide()
		f.ColTT2:Show()

		if (mOnWDSave.sortDirection == 1) then
			table.sort(o.menuItems, compMenuItemsName(false))
		else
			table.sort(o.menuItems, compMenuItemsName(true))
		end
	end
end
