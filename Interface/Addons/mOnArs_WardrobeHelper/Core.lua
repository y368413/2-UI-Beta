...
o.defaultSettings.sortColumn = 0
o.defaultSettings.sortDirection = 1
...
if o.customCategories == nil then
	o.customCategories = {}
end
		...
		if sources[i].isCollected then
			if itemID then
				local i1, i2, b1, i3, b2, itemString, visualString, sourceType =
					C_TransmogCollection.GetAppearanceSourceInfo(sources[i].sourceID)
...
o.isBlacklisted = function(id)
	for i = 1, #o.blacklist do
		if o.blacklist[i] == id then
			return true
		end
	end

	for i = 1, #mOnWDSave.blacklist do
		if mOnWDSave.blacklist[i] == id then
			return true
		end
	end
							...
							for i = 1, #boss do
								...
								if sources then
									...
									if collected then
										...
										newInstances[categoryName]["difficulties"][difficultyName]["collected"] =
											newInstances[categoryName]["difficulties"][difficultyName]["collected"] + 1
									else
									...
									if category["#allSources"] and (collected == false or mOnWDSave.completionistMode) then
										for j = 1, #sources do
											local i1, i2, b1, i3, b2, itemString, visualString, sourceType =
												C_TransmogCollection.GetAppearanceSourceInfo(sources[j].sourceID)
											local id = tonumber(string.match(itemString, "item:([^:]*):"))
									...
									newInstances[categoryName]["difficulties"][difficultyName]["total"] =
										newInstances[categoryName]["difficulties"][difficultyName]["total"] + 1
								else
		...
		for boss, items in pairs(bosses["bosses"]) do
			for i = #items["items"], 1, -1 do
				...
				if o.isBlacklisted(items["items"][i].id) then
					...
					o.instances[instance]["difficulties"][difficulty]["total"] =
						o.instances[instance]["difficulties"][difficulty]["total"] - 1
				elseif o.isCollected(sources, mOnWDSave.completionistMode and items["items"][i].id or nil) then
					...
					o.instances[instance]["difficulties"][difficulty]["collected"] =
						o.instances[instance]["difficulties"][difficulty]["collected"] + 1
				end
			end
		end
	end
...
o.GUItoggle = function()
	if mOnWD_MainFrame:IsVisible() then
		o.GUIclose()
	else
		o.GUIopen()
	end
...
o.fixSettings = function()
	if mOnWDSave == nil then
		mOnWDSave = {}
	end
	for k, v in pairs(o.defaultSettings) do
		if mOnWDSave[k] == nil then
			mOnWDSave[k] = v
		end
	end
end
...
o.updateSavedInstances = function()
	...
	for i = 1, GetNumSavedInstances() do
		local name,
			...
			difficultyName,
			numEncounters,
			encounterProgress = GetSavedInstanceInfo(i)
		if locked then
	...
	if WardrobeCollectionFrame then
		if WardrobeCollectionFrame.NavigationFrame then
			if mOnWD_MainFrame_bShow == nil then
				local b =
					CreateFrame("BUTTON", "mOnWD_MainFrame_bShow", WardrobeCollectionFrame.NavigationFrame, "UIPanelButtonTemplate")
