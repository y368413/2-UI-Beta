...
local MAJOR_VERSION = "LibCustomGlow-2.0"
...
local function addFrameAndTex(r,color,name,key,N,xOffset,yOffset,texture,texCoord,desaturated,frameLevel)
    ...
    PixelUtil.SetPoint(f,"TOPLEFT",r,"TOPLEFT",-xOffset,yOffset)
    PixelUtil.SetPoint(f,"BOTTOMRIGHT",r,"BOTTOMRIGHT",xOffset,-yOffset)
    f:Show()
...
local  pUpdate = function(self,elapsed)
    ...
    if self:IsShown() then
        if not (self.masks[1]:IsShown()) then
            self.masks[1]:Show()
            PixelUtil.SetPoint(self.masks[1],"TOPLEFT",self,"TOPLEFT",self.info.th,-self.info.th)
            PixelUtil.SetPoint(self.masks[1],"BOTTOMRIGHT",self,"BOTTOMRIGHT",-self.info.th,self.info.th)
        end
        if self.masks[2] and not(self.masks[2]:IsShown()) then
            self.masks[2]:Show()
            PixelUtil.SetPoint(self.masks[2],"TOPLEFT",self,"TOPLEFT",self.info.th+1,-self.info.th-1)
            PixelUtil.SetPoint(self.masks[2],"BOTTOMRIGHT",self,"BOTTOMRIGHT",-self.info.th-1,self.info.th+1)
        end
        if self.bg and not(self.bg:IsShown()) then
            self.bg:Show()
        end
        for k,line  in pairs(self.textures) do
			PixelUtil.SetPoint(line,"TOPLEFT",self,"TOPLEFT",pCalc1((progress+self.info.step*(k-1))%1,width,self.info.th,self.info.pTLx),-pCalc2((progress+self.info.step*(k-1))%1,height,self.info.th,self.info.pTLy))
            PixelUtil.SetPoint(line,"BOTTOMRIGHT",self,"TOPLEFT",self.info.th+pCalc2((progress+self.info.step*(k-1))%1,width,self.info.th,self.info.pBRx),-height+pCalc1((progress+self.info.step*(k-1))%1,height,self.info.th,self.info.pBRy))
        end
    end
end
...
function lib.PixelGlow_Start(r,color,N,frequency,length,th,xOffset,yOffset,border,key,frameLevel)
    ...
    PixelUtil.SetPoint(f.masks[1],"TOPLEFT",f,"TOPLEFT",th,-th)
    PixelUtil.SetPoint(f.masks[1],"BOTTOMRIGHT",f,"BOTTOMRIGHT",-th,th)
    ...
    if not(border==false) then
        ...
        PixelUtil.SetPoint(f.masks[2],"TOPLEFT",f,"TOPLEFT",th+1,-th-1)
        PixelUtil.SetPoint(f.masks[2],"BOTTOMRIGHT",f,"BOTTOMRIGHT",-th-1,th+1)
    ...
    for k=1,4 do
        ...
        for i = 1,self.info.N do
            ...
            if position>self.info.bottomlim then
                PixelUtil.SetPoint(self.textures[texIndex],"CENTER",self,"BOTTOMRIGHT",-position+self.info.bottomlim,0)
            elseif position>self.info.rightlim then
                PixelUtil.SetPoint(self.textures[texIndex],"CENTER",self,"TOPRIGHT",0,-position+self.info.rightlim)
            elseif position>self.info.height then
                PixelUtil.SetPoint(self.textures[texIndex],"CENTER",self,"TOPLEFT",position-self.info.height,0)
            else
                PixelUtil.SetPoint(self.textures[texIndex],"CENTER",self,"BOTTOMLEFT",0,position)
            end
        end
    end
end
...
function lib.ButtonGlow_Start(r,color,frequency,frameLevel)
    ...
    if r._ButtonGlow then
        ...
        PixelUtil.SetPoint(f,"TOPLEFT", r, "TOPLEFT", -width * 0.2, height * 0.2)
        PixelUtil.SetPoint(f,"BOTTOMRIGHT", r, "BOTTOMRIGHT", width * 0.2, -height * 0.2)
