...
local rwiconsize = 16			-- RaidWarning icon size, because 12 is small for me. Works only if croprwicons=true
local buttonsize = 21
local shadow = true  --true for shadow style, false for 1px style
local StatusBarTexture = "Interface\\AddOns\\_ShiGuang\\Media\\Modules\\Raid\\normTex"
...
if DBMskin then
	...
	DBMSkin:SetScript("OnEvent", function()
		...
		self.shadow:SetPoint("BOTTOMRIGHT", 2, -2)
			...
			edgeFile = "Interface\\AddOns\\_ShiGuang\\Media\\Modules\\Raid\\glowTex", 
					edgeSize = 5,
		...
		self.Border:SetBackdropBorderColor(65/255, 74/255, 79/255,1)
					...
					if not bar.injected then
						bar.ApplyStyle=function()
							...
							if not frame.styled then
								...
								frame:SetHeight(buttonsize/1.4)
								if shadow then
							...
							if not name.styled then
								...
								name:SetWidth(165)
								name:SetHeight(8)
								name:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
							...
							if not timer.styled then	
								...
								timer:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
				...
				if not anchor.styled then
					local header={anchor:GetRegions()}
						if header[1]:IsObjectType("FontString") then
							header[1]:SetFont(GameTooltipText:GetFont(), 14, "OUTLINE")
			...
			local SkinBoss=function()
				...
				while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
					...
					progress:SetStatusBarTexture("Interface\\AddOns\\_ShiGuang\\Media\\Modules\\Raid\\normTex")
	...
	--[[if IsAddOnLoaded("DBM-VPMao") then
		DBM_AllSavedOptions["Default"]["CountdownVoice"] = "VP:Mao"
		DBM_AllSavedOptions["Default"]["CountdownVoice2"] = "VP:Mao"
		DBM_AllSavedOptions["Default"]["CountdownVoice3"] = "VP:Mao"
		DBM_AllSavedOptions["Default"]["ChosenVoicePack"] = "Mao"
	end]]