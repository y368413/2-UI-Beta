...
todoTable.init = {}; -- adds init table to addon namespace

local config = todoTable.config;
local itemsFrame = todoTable.itemsFrame;
local init = todoTable.init;
...
function Todo:PLAYER_LOGIN()
  local disabled = config.database.options.args.main.args.chat.args.groupWarnings.args.hourlyReminder.disabled
  if (Todo.db.global.UI_reloading) then -- just to be sure that it wasn't a reload, but a genuine player log in
    Todo.db.global.UI_reloading = false

    if (Todo.db.global.warnTimerRemaining > 0) then -- this is for the special case where we logged in, but reloaded before the 20 sec timer activated, so we just try it again
      Todo.warnTimer = self:ScheduleTimer(function() -- after reloading, we restart the warn timer from where we left off before the reload
        if (Todo.db.profile.hourlyReminder and not disabled()) then -- without forgetting that it's the hourly reminder timer this time
          Todo:Warn()
        end
        Todo.warnTimer = self:ScheduleRepeatingTimer(function()
          if (Todo.db.profile.hourlyReminder and not disabled()) then
            Todo:Warn()
          end
        end, warnTimerTime)
      end, Todo.db.global.warnTimerRemaining)
      return;
    end
  end

  Todo.db.global.warnTimerRemaining = 0
...
init.commands = {
  ...
  [L["info"]] = function()
    ...
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["toggle"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["categories"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["favorites"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["descriptions"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["hyperlinks"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["rename"])
    str = str.." -- "..string.format("|cff%s%s|r", hex, "/todo "..L["tutorial"])
    config:PrintForced(str)
  end,

  [L["toggle"]] = function()
    config:PrintForced(L["To toggle the list, you have several ways:"]..'\n- '..L["minimap button (the default)"]..'\n- '..L["a normal Todo button"]..'\n- '..L["databroker plugin (eg. titan panel)"]..'\n- '..L["the '/todo' command"]..'\n- '..L["key binding"]..'\n'..L["You can go to the addon options in the game's interface settings to customize this."])
  end,
...
local function HandleSlashCommands(str)
  ...
  if (#str == 0) then
    -- we just entered "/todo" with no additional args.
    path[""]();
    return;
  end
...
function Todo:OnInitialize()
    -- Called when the addon is loaded

    -- Register new Slash Command
    SLASH_Todo1 = "/todo";
    SlashCmdList.Todo = HandleSlashCommands;

    -- Saved variable database
    self.db = LibStub("AceDB-3.0"):New("TodoDB", config.database.defaults)
    ...
    hooksecurefunc("ReloadUI", function()
      Todo.db.global.UI_reloading = true
      Todo.db.global.warnTimerRemaining = self:TimeLeft(Todo.warnTimer) -- if we are reloading, we keep in mind how much time there was left to our repeating warn timer
    end) -- this is for knowing when the addon is loading, if it was a UI reload or the player logging in
    hooksecurefunc("ChatEdit_InsertLink", function(...) return Todo:EditBoxInsertLink(...) end) -- this is for adding hyperlinks in my addon edit boxes
    ...
    Todo:InitializeOptionsWidthRecursive(config.database.options.args.main.args, wDef)
    ...
    Todo:CreateTodoButton();
    Todo:CreateMinimapButton();
    ...
    local hex = config:RGBToHex(config.database.theme2);
    --config:Print(L["addon loaded!"]..' ('..string.format("|cff%s%s|r", hex, "/todo "..L["info"])..')');
    addonLoaded = true;
end

function Todo:AddonUpdated()
