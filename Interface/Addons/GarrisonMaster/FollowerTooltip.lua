if GetLocale() == "zhCN" then
	SmartFollowerManagerTextShift = "按下Shift显示其他可能的技能"
	SmartFollowerManagerTextAlt = "按下Alt显示可能的技能组合"
	SmartFollowerManagerShiftDown = "其他可能的技能"
	SmartFollowerManagerAltDown = "可能的技能组合"
	SmartFollowerManagerAbilityCountFormat = "%s(%d/%d)"
	SmartFollowerManagerAbilityPair = "此组合: 激活%d  总计%d"
elseif GetLocale() == "zhTW" then
	SmartFollowerManagerTextShift = "按下Shift顯示其他可能的技能"
	SmartFollowerManagerTextAlt = "按下Alt顯示可能的技能組合"
	SmartFollowerManagerShiftDown = "其他可能的技能"
	SmartFollowerManagerAltDown = "可能的技能組合"
	SmartFollowerManagerAbilityCountFormat = "%s(%d/%d)"
	SmartFollowerManagerAbilityPair = "此組合: 任用%d  總計%d"
else --enUS
	SmartFollowerManagerTextShift = "Press SHIFT to show other abilities..."
	SmartFollowerManagerTextAlt = "Press ALT to show ability pairs..."
	SmartFollowerManagerShiftDown = "Other Abilities"	--need review
	SmartFollowerManagerAltDown = "Ability Pairs"
	SmartFollowerManagerAbilityCountFormat = "%s(%d/%d)"
	SmartFollowerManagerAbilityPair = "This Pair: Active %d  Total %d"
end

local _, SmartFollowerManager = ...

local API = SmartFollowerManager.API
local GarrisonFollowerTooltip_Owner = SmartFollowerManager.GarrisonFollowerTooltip_Owner
local Override = SmartFollowerManager.Override
local FGFTData = {isMaxLevel = false}
local GFTData = {isMaxLevel = false}
local AbilitiesData
local strLPair = SmartFollowerManagerAbilityPair
...
local function ModifyFollowerTooltip(tooltipFrame, data)
	...
	local abiSpec = SmartFollowerManager.abiSpec[data.spec]
	...
	if data.quality <= 4 then
		TooltipMode = KeyMode
		TooltipModeText = SmartFollowerManagerTextShift..'|n'..SmartFollowerManagerTextAlt
	else
		TooltipMode = (KeyMode == 1) and 1 or 0
		TooltipModeText = SmartFollowerManagerTextShift
	end
	]]
	TooltipMode = KeyMode
	TooltipModeText = SmartFollowerManagerTextShift..'|n'..SmartFollowerManagerTextAlt
	...
	if #otherAbilities > 0 and detailed then
		...
		if TooltipMode == 1 then
			...
			label:SetText(SmartFollowerManagerShiftDown)
		...
		elseif TooltipMode == 2 then
			...
			label:SetText(SmartFollowerManagerAltDown)
			color = NORMAL_FONT_COLOR
			label:SetVertexColor(color.r, color.g, color.b)
...
function SmartFollowerManager.LoadWithUI.FollowerTooltip()
